    ...
    return switch (c) {
        ...
        '\t' => "\\t"[0..],
            ...
            if (std.ascii.isCntrl(c))
                return std.fmtgen.bufPrint(char_buf[0..], "\\x{x:0<2}", .{c}) catch unreachable
            else
                return std.fmtgen.bufPrint(char_buf[0..], "{c}", .{c}) catch unreachable;
        },
    };
}
    ...
    if (tok.id == .IntegerLiteral) {
        if (lit_bytes.len > 2 and lit_bytes[0] == '0') {
            switch (lit_bytes[1]) {
                '0'...'7' => {
                    // Octal
                    lit_bytes = try std.fmtgen.allocPrint(c.a(), "0o{}", .{lit_bytes});
                },
                'X' => {
                    // Hexadecimal with capital X, valid in C but not in Zig
                    lit_bytes = try std.fmtgen.allocPrint(c.a(), "0x{}", .{lit_bytes[2..]});
                },
                else => {},
            }
        }
