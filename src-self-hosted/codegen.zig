...
pub fn CodeGen(comptime linker: link.File.Tag) type {
    return struct{
        pub fn generateSymbol(
            ...
            dbg_info_type_relocs: comptime switch (linker) {
                .elf => *link.File.Elf.DbgInfoTypeRelocsTable,
                .macho => *link.File.MachO.DbgInfoTypeRelocsTable,
                .c, .wasm => @compileError("C and Wasm backends use their own implementation"),
            },
                    ...
                    if (typed_value.val.cast(Value.Payload.DeclRef)) |payload| {
                        ...
                        switch (linker) {
                            .elf => {
                                ...
                                const elf = @fieldParentPtr(link.File.Elf, "base", bin_file);
                                // TODO handle the dependency of this symbol on the decl's vaddr.
                                // If the decl changes vaddr, then this symbol needs to get regenerated.
                                const vaddr = elf.local_symbols.items[decl.link.elf.local_sym_index].st_value;
                            ...
                            .macho => return Result{
                                .fail = try ErrorMsg.create(
                                    bin_file.allocator,
                                    src,
                                    "TODO implement targeting Mach-O linker",
                                    .{},
                                )
                            },
                            .c, .wasm => unreachable,
                        ...
                        .x86_64 => {
                            ...
                            if (inst.func.cast(ir.Inst.Constant)) |func_inst| {
                                if (func_inst.val.cast(Value.Payload.Function)) |func_val| {
                                    ...
                                    switch (linker) {
                                        .elf => {
                                            const elf = @fieldParentPtr(link.File.Elf, "base", self.bin_file);
                                            const got = &elf.program_headers.items[elf.phdr_got_index.?];
                                            const got_addr = @intCast(u32, got.p_vaddr + func.owner_decl.link.elf.offset_table_index * ptr_bytes);
                                            mem.writeIntLittle(u32, self.code.addManyAsArrayAssumeCapacity(4), got_addr);
                                        },
                                        .macho => return self.fail(self.src, "TODO implement targeting Mach-O linker", .{}),
                                        .c, .wasm => unreachable,
                                    }
