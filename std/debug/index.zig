...
var stderr_stream: ?io.OutStreamInterface(os.File) = null;
...
pub fn getStderrStream() !io.OutStreamInterface(os.File) {
    if (stderr_stream) |st| {
        return st;
    } else {
        stderr_file = try io.getStdErr();
        const st = stderr_file.outStreamInterface();
        stderr_stream = st;
        return st;
    }
}
...
fn openSelfDebugInfoLinux(allocator: mem.Allocator) !DwarfInfo {
    const S = struct {
        ...
        var self_exe_seekable_stream: io.SeekableStreamInterface(os.File) = undefined;
        var self_exe_in_stream: io.InStreamInterface(os.File) = undefined;
    };
    S.self_exe_file = try os.openSelfExe();
    errdefer S.self_exe_file.close();

    S.self_exe_seekable_stream = S.self_exe_file.seekableStreamInterface();
    S.self_exe_in_stream = S.self_exe_file.inStreamInterface();

    return openElfDebugInfo(
        allocator,
        S.self_exe_file.seekableStream(),
        S.self_exe_file.inStream(),
    );
}
...
pub const DwarfInfo = struct {
    ...
    pub fn interface(self: DwarfInfo) mem.Allocator {
        return self.abbrev_table_list.allocator;
    }

    pub fn readString(self: *DwarfInfo) ![]u8 {
        return readStringRaw(self.interface(), self.dwarf_in_stream);
    }
};
...
pub const DebugInfo = switch (builtin.os) {
    builtin.Os.macosx => struct {
        ...
        pub fn interface(self: DebugInfo) mem.Allocator {
...
fn parseAbbrevTable(di: *DwarfInfo) !AbbrevTable {
    var result = AbbrevTable.init(di.interface());
    while (true) {
        ...
        try result.append(AbbrevTableEntry{
            ...
            .attrs = ArrayList(AbbrevAttr).init(di.interface()),
        });
...
fn parseDie(di: *DwarfInfo, abbrev_table: *const AbbrevTable, is_64: bool) !Die {
    ...
    var result = Die{
        ...
        .attrs = ArrayList(Die.Attr).init(di.interface()),
    };
    try result.attrs.resize(table_entry.attrs.len);
    for (table_entry.attrs.toSliceConst()) |attr, i| {
        result.attrs.items[i] = Die.Attr{
            .id = attr.attr_id,
            .value = try parseFormValue(di.interface(), di.dwarf_in_stream, attr.form_id, is_64),
        };
    }
    return result;
}
...
fn getLineNumberInfoMacOs(di: *DebugInfo, symbol: MachoSymbol, target_address: usize) !LineInfo {
    ...
    var include_directories = ArrayList([]const u8).init(di.interface());
    ...
    var file_entries = ArrayList(FileEntry).init(di.interface());
...
fn getLineNumberInfoDwarf(di: *DwarfInfo, compile_unit: CompileUnit, target_address: usize) !LineInfo {
    ...
    while (this_offset < debug_line_end) : (this_index += 1) {
        ...
        const standard_opcode_lengths = try di.interface().alloc(u8, opcode_base - 1);
        ...
        var include_directories = ArrayList([]u8).init(di.interface());
...
fn scanAllCompileUnits(di: *DwarfInfo) !void {
    ...
    while (this_unit_offset < debug_info_end) {
        ...
        const compile_unit_die = try di.interface().create(try parseDie(di, abbrev_table, is_64));
