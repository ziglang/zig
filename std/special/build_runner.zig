...
pub fn main() !void {
    ...
    var stderr_file_stream: io.OutStreamInterface(os.File) = undefined;
    var stderr_stream = if (stderr_file) |f| x: {
        stderr_file_stream = f.outStreamInterface();
        break :x stderr_file_stream;
    } else |err| err;

    var stdout_file = io.getStdOut();
    var stdout_file_stream: io.OutStreamInterface(os.File) = undefined;
    var stdout_stream = if (stdout_file) |f| x: {
        stdout_file_stream = f.outStreamInterface();
        break :x stdout_file_stream;
    } else |err| err;
