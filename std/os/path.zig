...
fn joinSep(allocator: var, separator: u8, paths: []const []const u8) ![]u8 {
...
pub fn joinWindows(allocator: var, paths: []const []const u8) ![]u8 {
    return joinSep(allocator, sep_windows, paths);
}

/// Naively combines a series of paths with the native path seperator.
/// Allocates memory for the result, which must be freed by the caller.
pub fn joinPosix(allocator: var, paths: []const []const u8) ![]u8 {
...
pub fn resolve(allocator: var, paths: []const []const u8) ![]u8 {
    if (is_windows) {
...
pub fn resolveWindows(allocator: var, paths: []const []const u8) ![]u8 {
    if (paths.len == 0) {
...
/// Note: all usage of this function should be audited due to the existence of symlinks.
/// Without performing actual syscalls, resolving `..` could be incorrect.
pub fn resolvePosix(allocator: var, paths: []const []const u8) ![]u8 {
    if (paths.len == 0) {
...
pub fn relative(allocator: var, from: []const u8, to: []const u8) ![]u8 {
    if (is_windows) {
...
pub fn relativeWindows(allocator: var, from: []const u8, to: []const u8) ![]u8 {
...
pub fn relativePosix(allocator: var, from: []const u8, to: []const u8) ![]u8 {
...
pub fn realAlloc(allocator: var, pathname: []const u8) ![]u8 {
