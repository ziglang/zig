...
pub const ChildProcess = struct {
    ...
    fn cleanupAfterWait(self: *ChildProcess, status: i32) !Term {
        ...
        // Write std.math.maxInt(ErrInt) to the write end of the err_pipe. This is after
        // waitpid, so this write is guaranteed to be after the child
        // pid potentially wrote an error. This way we can do a blocking
        // read on the error pipe and either get std.math.maxInt(ErrInt) (no error) or
        // an error code.
        try writeIntFd(self.err_pipe[1], std.math.maxInt(ErrInt));
