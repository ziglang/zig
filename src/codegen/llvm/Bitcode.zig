...
pub const AbbrevOp = union(enum) {
    ...
    pub fn childCode(self: AbbrevOp) u3 {
        return switch (self) {
            .array_fixed, .fixed => 1,
            .array_vbr, .vbr => 2,
            .array_char6, .char6 => 4,
...
pub fn init(allocator: std.mem.Allocator) Bitcode {
    return .{ .buffer = std.ArrayList(u32).init(allocator) };
}

pub fn deinit(self: Bitcode) void {
    self.buffer.deinit();
}

pub fn toSlice(self: Bitcode) []const u32 {
    std.debug.assert(self.bit_count == 0);
    return self.buffer.items;
}

pub fn length(self: Bitcode) usize {
    std.debug.assert(self.bit_count == 0);
    return self.buffer.items.len;
}

pub fn writeBits(self: *Bitcode, value: anytype, bits: u16) Error!void {
    if (bits == 0) return;
...
pub fn writeVBR(self: *Bitcode, value: anytype, comptime vbr_bits: usize) Error!void {
    comptime std.debug.assert(vbr_bits > 1);
...
pub fn write6BitChar(self: *Bitcode, c: u8) Error!void {
    try self.writeBits(charTo6Bit(c), 6);
}

pub fn alignTo32(self: *Bitcode) Error!void {
    if (self.bit_count == 0) return;
...
pub fn enterTopBlock(self: *Bitcode, comptime SubBlock: type) Error!BlockWriter(SubBlock) {
...
fn BlockWriter(comptime Block: type) type {
    return struct {
        ...
        const abbrev_len = std.math.log2_int_ceil(
            u6,
        ...
        pub fn init(bitcode: *Bitcode, comptime parent_abbrev_len: u6) Error!Self {
        ...
        pub fn writeAbbrevAdapted(
            ...
            try self.bitcode.writeBits(comptime blk: {
                inline for (Block.abbrevs, 0..) |abbrev, i| {
                    if (Abbrev == abbrev) break :blk i + 4;
                }
        ...
        fn defineAbbrev(bitcode: *Bitcode, comptime ops: []const AbbrevOp) Error!void {
            ...
            try bitcode.writeVBR(blk: {
                ...
                inline for (ops) |op| {
                    count += switch (op) {
                        .literal, .fixed, .vbr, .char6, .blob => 1,
                        .array_fixed, .array_vbr, .array_char6 => 2,
                    };
                }
                break :blk count;
            }, 5);
            ...
            inline for (ops) |op| {
                switch (op) {
                    ...
                    .fixed, .vbr => |width| {
                        ...
                        try bitcode.writeBits(op.code(), 3);
                    ...
                    .array_fixed, .array_vbr => |width| {
                        ...
                        // Fixed or VBR op
                        try bitcode.writeBits(0, 1);
                        try bitcode.writeBits(op.childCode(), 3);
                        try bitcode.writeVBR(width, 5);
                    },
                    .array_char6 => {
                        // Array op
