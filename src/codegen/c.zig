    ...
    fn writeCValue(f: *Function, w: anytype, c_value: CValue, location: ValueRenderLocation) !void {
        switch (c_value) {
            .constant => |val| try f.object.dg.renderValue(
                w,
                Type.fromInterned(f.object.dg.zcu.intern_pool.typeOf(val)),
...
pub const DeclGen = struct {
    ...
    fn fail(dg: *DeclGen, comptime format: []const u8, args: anytype) error{ AnalysisFail, OutOfMemory } {
        ...
        const src_loc = src.toSrcLoc(decl, zcu);
    ...
    fn renderAnonDeclValue(
        ...
        if (Value.fromInterned(decl_val).getExternFunc(zcu)) |extern_func| {
            _ = extern_func;
            _ = ptr_val;
            _ = location;
            @panic("TODO");
        ...
        assert(Value.fromInterned(decl_val).getVariable(zcu) == null);

        // We shouldn't cast C function pointers as this is UB (when you call
        // them).  The analysis until now should ensure that the C function
        // pointers are compatible.  If they are not, then there is a bug
        // somewhere and we should let the C compiler tell us about it.
        const need_typecast = if (ty.castPtrToFn(zcu)) |_| false else !ty.childType(zcu).eql(decl_ty, zcu);
        if (need_typecast) {
            try writer.writeAll("((");
    ...
    fn renderDeclValue(
        ...
        // Render an undefined pointer if we have a pointer to a zero-bit or comptime type.
        if (ty.isPtrAtRuntime(zcu) and !decl.ty.isFnOrHasRuntimeBits(zcu)) {
            return dg.writeCValue(writer, .{ .undef = ty });
        }
        ...
        // pointers are compatible.  If they are not, then there is a bug
        // somewhere and we should let the C compiler tell us about it.
        const need_typecast = if (ty.castPtrToFn(zcu)) |_| false else !ty.childType(zcu).eql(decl.ty, zcu);
        if (need_typecast) {
            try writer.writeAll("((");
            try dg.renderType(writer, ty);
            try writer.writeByte(')');
    ...
    fn renderParentPtr(
        ...
        const zcu = dg.zcu;
        const ptr_ty = Type.fromInterned(zcu.intern_pool.typeOf(ptr_val));
        ...
        switch (ptr.addr) {
            ...
            .eu_payload, .opt_payload => |base| {
                const ptr_base_ty = Type.fromInterned(zcu.intern_pool.typeOf(base));
                ...
                const ptr_payload_ty = try zcu.adjustPtrTypeChild(ptr_base_ty, payload_ty);
            ...
            .elem => |elem| {
                const ptr_base_ty = Type.fromInterned(zcu.intern_pool.typeOf(elem.base));
                const elem_ty = ptr_base_ty.elemType2(zcu);
                const ptr_elem_ty = try zcu.adjustPtrTypeChild(ptr_base_ty, elem_ty);
                ...
                try writer.writeAll("&(");
                if (zcu.intern_pool.indexToKey(ptr_base_ty.toIntern()).ptr_type.flags.size == .One)
                    try writer.writeByte('*');
                try dg.renderParentPtr(writer, elem.base, location);
                try writer.print(")[{d}]", .{elem.index});
            },
            .field => |field| {
                const ptr_base_ty = Type.fromInterned(zcu.intern_pool.typeOf(field.base));
                const base_ty = ptr_base_ty.childType(zcu);
                // Ensure complete type definition is visible before accessing fields.
                _ = try dg.typeToIndex(base_ty, .complete);
                const field_ty = switch (zcu.intern_pool.indexToKey(base_ty.toIntern())) {
                    .anon_struct_type, .struct_type, .union_type => base_ty.structFieldType(@as(usize, @intCast(field.index)), zcu),
                ...
                const ptr_field_ty = try zcu.adjustPtrTypeChild(ptr_base_ty, field_ty);
                ...
                switch (fieldLocation(ptr_base_ty, ptr_ty, @as(u32, @intCast(field.index)), zcu)) {
                    ...
                    .byte_offset => |byte_offset| {
                        ...
                        const byte_offset_val = try zcu.intValue(Type.usize, byte_offset);
                    ...
                    .end => {
                        ...
                        try dg.renderParentPtr(writer, field.base, location);
                        try writer.print(") + {})", .{
                            try dg.fmtIntLiteral(Type.usize, try zcu.intValue(Type.usize, 1), .Other),
                        });
                    },
                }
            },
    ...
    fn renderValue(
            ...
            switch (ty.zigTypeTag(zcu)) {
                ...
                .Struct => switch (ty.containerLayout(zcu)) {
                    .Auto, .Extern => {
                        ...
                        try writer.writeByte('{');
                        var empty = true;
                        for (0..ty.structFieldCount(zcu)) |field_i| {
                            if (ty.structFieldIsComptime(field_i, zcu)) continue;
                ...
                .Array, .Vector => {
                    ...
                    if (ai.elem_type.eql(Type.u8, zcu)) {
                        ...
                        while (index < c_len) : (index += 1) {
                            ...
                            try dg.renderValue(writer, ty.childType(zcu), val, initializer_type);
                        }
                        return writer.writeByte('}');
                    }
                },
        ...
        switch (ip.indexToKey(val.ip_index)) {
            ...
            .error_union => |error_union| {
                ...
                try dg.renderValue(
                    ...
                    Value.fromInterned(switch (error_union.val) {
                        .err_name => try zcu.intern(.{ .undef = payload_ty.ip_index }),
            ...
            .float => {
                ...
                const repr_val = try zcu.intValue_big(repr_ty, repr_val_big.toConst());
            ...
            .opt => |opt| {
                ...
                try dg.renderValue(writer, payload_ty, Value.fromInterned(switch (opt.val) {
                    .none => try zcu.intern(.{ .undef = payload_ty.ip_index }),
                        ...
                        if (eff_num_fields == 0) {
                            ...
                            for (field_types, 0..) |field_ty, field_i| {
                                ...
                                if (bit_offset != 0) {
                                    ...
                                    try writer.writeAll(", ");
                                    const bit_offset_val = try zcu.intValue(bit_offset_ty, bit_offset);
                                ...
                                bit_offset += Type.fromInterned(field_ty).bitSize(zcu);
            ...
            .un => |un| {
                ...
                if (un.tag == .none) {
                    ...
                    const field_i = zcu.unionTagFieldIndex(union_obj, Value.fromInterned(un.tag)).?;
                    ...
                    if (field_ty.hasRuntimeBits(zcu)) {
                        ...
                        if (!Type.fromInterned(this_field_ty).hasRuntimeBits(zcu)) continue;
    ...
    fn renderFunctionSignature(
        ...
        const fn_info = zcu.typeToFunc(fn_decl.ty).?;
        if (fn_info.cc == .Naked) {
            switch (kind) {
    ...
    fn typeToIndex(dg: *DeclGen, ty: Type, kind: CType.Kind) !CType.Index {
        return dg.ctypes.typeToIndex(dg.gpa, ty, dg.zcu, kind);
    }

    fn typeToCType(dg: *DeclGen, ty: Type, kind: CType.Kind) !CType {
        return dg.ctypes.typeToCType(dg.gpa, ty, dg.zcu, kind);
    }

    fn byteSize(dg: *DeclGen, cty: CType) u64 {
        return cty.byteSize(dg.ctypes.set, dg.zcu.getTarget());
    }
    ...
    fn declIsGlobal(dg: *DeclGen, tv: TypedValue) bool {
        const zcu = dg.zcu;
        return switch (zcu.intern_pool.indexToKey(tv.val.ip_index)) {
    ...
    fn renderFwdDecl(
        ...
        const decl = dg.zcu.declPtr(decl_index);
        ...
        const maybe_exports = dg.zcu.decl_exports.get(decl_index);
        ...
        mangled: {
            const external_name = dg.zcu.intern_pool.stringToSlice(if (maybe_exports) |exports|
    ...
    fn renderCTypeForBuiltinFnName(dg: *DeclGen, writer: anytype, cty: CType) !void {
        switch (cty.tag()) {
                ...
                if (cty.isBool())
                    ...
                    signAbbrev(cty.signedness(dg.zcu.getTarget()))
                else if (cty.isFloat())
                    @as(u8, 'f')
                ...
                if (cty.isFloat()) cty.floatActiveBits(dg.zcu.getTarget()) else dg.byteSize(cty) * 8,
            }),
    ...
    fn renderBuiltinInfo(dg: *DeclGen, writer: anytype, ty: Type, info: BuiltinInfo) !void {
        ...
        try writer.print(", {}", .{try dg.fmtIntLiteral(
            bits_ty,
            try zcu.intValue(bits_ty, int_info.bits),
            .FunctionArgument,
        )});
    }
...
pub fn genErrDecls(o: *Object) !void {
    ...
    if (zcu.global_error_set.keys().len > 1) {
        ...
        for (zcu.global_error_set.keys()[1..], 1..) |name_nts, value| {
            const name = zcu.intern_pool.stringToSlice(name_nts);
    ...
    for (zcu.global_error_set.keys()) |name_nts| {
        const name = zcu.intern_pool.stringToSlice(name_nts);
    ...
    for (zcu.global_error_set.keys(), 0..) |name_nts, value| {
        ...
        const len_val = try zcu.intValue(Type.usize, name.len);
...
fn genExports(o: *Object) !void {
    ...
    const decl = zcu.declPtr(decl_index);
    const tv: TypedValue = .{ .ty = decl.ty, .val = Value.fromInterned((try decl.internValue(zcu))) };
...
pub fn genLazyFn(o: *Object, lazy_fn: LazyFnMap.Entry) !void {
    ...
    switch (key) {
        .tag_name => {
            ...
            try w.writeAll(") {\n switch (tag) {\n");
            for (enum_ty.enumFields(zcu), 0..) |name_ip, index_usize| {
                ...
                const int_val = try tag_val.intFromEnum(enum_ty, zcu);
...
pub fn genDecl(o: *Object) !void {
    ...
    if (!tv.ty.isFnOrHasRuntimeBitsIgnoreComptime(zcu)) return;
    if (tv.val.getExternFunc(zcu)) |_| {
        const fwd_decl_writer = o.dg.fwd_decl.writer();
        try fwd_decl_writer.writeAll("zig_extern ");
        try o.dg.renderFunctionSignature(fwd_decl_writer, decl_index, .forward, .{ .export_index = 0 });
        try fwd_decl_writer.writeAll(";\n");
        try genExports(o);
    } else if (tv.val.getVariable(zcu)) |variable| {
...
pub fn genDeclValue(
    ...
    if (zcu.intern_pool.stringToSliceUnwrap(link_section)) |s|
        try w.print("zig_linksection(\"{s}\", ", .{s});
...
fn genBodyInner(f: *Function, body: []const Air.Inst.Index) error{ AnalysisFail, OutOfMemory }!void {
    ...
    for (body) |inst| {
        ...
        const result_value = switch (air_tags[@intFromEnum(inst)]) {
            ...
            .mod       => try airBinBuiltinCall(f, inst, "zcu", .none),
...
fn airTrunc(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    if (dest_bits >= 8 and std.math.isPowerOfTwo(dest_bits)) {
        ...
        .unsigned => {
            const mask_val = try inst_scalar_ty.maxIntScalar(zcu, scalar_ty);
    ...
    callee: {
        known: {
            const fn_decl = fn_decl: {
                ...
                break :fn_decl switch (zcu.intern_pool.indexToKey(callee_val.ip_index)) {
...
fn airDbgInline(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    const zcu = f.object.dg.zcu;
    const writer = f.object.writer();
    const owner_decl = zcu.funcOwnerDeclPtr(ty_fn.func);
    try writer.print("/* dbg func:{s} */\n", .{
        zcu.intern_pool.stringToSlice(owner_decl.name),
    });
    return .none;
}
...
fn airSwitchBr(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    for (0..switch_br.data.cases_len) |case_i| {
        ...
        for (items) |item| {
            ...
            try f.object.dg.renderValue(writer, condition_ty, (try f.air.value(item, zcu)).?, .Other);
            try writer.writeByte(':');
        }
        try writer.writeByte(' ');
...
fn asmInputNeedsLocal(f: *Function, constraint: []const u8, value: CValue) bool {
    const target = f.object.dg.zcu.getTarget();
    return switch (constraint[0]) {
        '{' => true,
        'i', 'r' => false,
        'I' => !target.cpu.arch.isArmOrThumb(),
        else => switch (value) {
            .constant => |val| switch (f.object.dg.zcu.intern_pool.indexToKey(val)) {
                .ptr => |ptr| switch (ptr.addr) {
                    .decl => false,
                    else => true,
                },
...
fn airIsNull(
    ...
    const rhs = if (!payload_ty.hasRuntimeBitsIgnoreComptime(zcu))
        ...
        TypedValue{ .ty = optional_ty, .val = try zcu.getCoerced(Value.null, optional_ty) }
    else if (payload_ty.zigTypeTag(zcu) == .ErrorSet)
        TypedValue{ .ty = err_int_ty, .val = try zcu.intValue(err_int_ty, 0) }
        ...
        break :rhs TypedValue{ .ty = opt_slice_ptr_ty, .val = try zcu.nullValue(opt_slice_ptr_ty) };
    } else rhs: {
        try writer.writeAll(".is_null");
...
fn airFieldParentPtr(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    switch (fieldLocation(container_ptr_ty, field_ptr_ty, extra.field_index, zcu)) {
        ...
        .end => {
            try f.writeCValue(writer, field_ptr_val, .Other);
            try writer.print(" - {}", .{try f.fmtIntLiteral(Type.usize, try zcu.intValue(Type.usize, 1))});
        },
    }
...
fn airStructFieldVal(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    const field_name: CValue = switch (zcu.intern_pool.indexToKey(struct_ty.ip_index)) {
...
fn airUnwrapErrUnionErr(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    if (!payload_ty.hasRuntimeBits(zcu)) {
        ...
        if (!error_ty.errorSetIsEmpty(zcu))
            ...
            const err_int_ty = try zcu.errorIntType();
            try f.object.dg.renderValue(writer, err_int_ty, try zcu.intValue(err_int_ty, 0), .Initializer);
        }
    }
    try writer.writeAll(";\n");
    return local;
...
fn airErrUnionPayloadPtrSet(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    if (!payload_ty.hasRuntimeBitsIgnoreComptime(zcu)) {
        try f.writeCValueDeref(writer, operand);
        try writer.writeAll(" = ");
        try f.object.dg.renderValue(writer, err_int_ty, try zcu.intValue(err_int_ty, 0), .Other);
        try writer.writeAll(";\n ");
...
fn airArrayToSlice(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    const writer = f.object.writer();
    const local = try f.allocLocal(inst, inst_ty);
    const array_ty = f.typeOf(ty_op.operand).childType(zcu);
    ...
    } else if (array_ty.hasRuntimeBitsIgnoreComptime(zcu)) {
        try writer.writeAll("&(");
        try f.writeCValueDeref(writer, operand);
        try writer.print(")[{}]", .{try f.fmtIntLiteral(Type.usize, try zcu.intValue(Type.usize, 0))});
    } else try f.writeCValue(writer, operand, .Initializer);
    try writer.writeAll("; ");

    const len_val = try zcu.intValue(Type.usize, array_ty.arrayLen(zcu));
    try f.writeCValueMember(writer, local, .{ .identifier = "len" });
...
fn airFloatCast(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    else if (inst_ty.isInt(zcu) and operand_ty.isRuntimeFloat())
        if (inst_ty.isSignedInt(zcu)) "fix" else "fixuns"
    else if (inst_ty.isRuntimeFloat() and operand_ty.isInt(zcu))
        if (operand_ty.isSignedInt(zcu)) "float" else "floatun"
    else
        unreachable;
    ...
    try writer.writeAll(" = ");
    if (inst_ty.isInt(zcu) and operand_ty.isRuntimeFloat()) {
        try writer.writeAll("zig_wrap_");
    ...
    try writer.writeAll("zig_");
    try writer.writeAll(operation);
    try writer.writeAll(compilerRtAbbrev(operand_ty, zcu));
    try writer.writeAll(compilerRtAbbrev(inst_ty, zcu));
    try writer.writeByte('(');
    try f.writeCValue(writer, operand, .FunctionArgument);
    try writer.writeByte(')');
    if (inst_ty.isInt(zcu) and operand_ty.isRuntimeFloat()) {
...
fn airCmpBuiltinCall(
    ...
    if (!ref_ret) try writer.print(" {s} {}", .{
        ...
        try f.fmtIntLiteral(Type.i32, try zcu.intValue(Type.i32, 0)),
    });
    try writer.writeAll(";\n");
    try v.end(f, inst, writer);
...
fn airMemset(f: *Function, inst: Air.Inst.Index, safety: bool) !CValue {
    ...
    if (elem_abi_size > 1 or dest_ty.isVolatilePtr(zcu)) {
        ...
        try f.writeCValue(writer, index, .Other);
        try writer.writeAll(" = ");
        try f.object.dg.renderValue(writer, Type.usize, try zcu.intValue(Type.usize, 0), .Initializer);
        try writer.writeAll("; ");
        try f.writeCValue(writer, index, .Other);
        try writer.writeAll(" != ");
        switch (dest_ty.ptrSize(zcu)) {
...
fn airShuffle(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    for (0..extra.mask_len) |index| {
        try f.writeCValue(writer, local, .Other);
        try writer.writeByte('[');
        try f.object.dg.renderValue(writer, Type.usize, try zcu.intValue(Type.usize, index), .Other);
        try writer.writeAll("] = ");
    ...
    switch (inst_ty.zigTypeTag(zcu)) {
        ...
        .Struct => switch (inst_ty.containerLayout(zcu)) {
            .Auto, .Extern => for (resolved_elements, 0..) |element, field_i_usize| {
                ...
                if (inst_ty.structFieldIsComptime(field_i, zcu)) continue;
                const field_ty = inst_ty.structFieldType(field_i, zcu);
                if (!field_ty.hasRuntimeBitsIgnoreComptime(zcu)) continue;
                ...
                try f.writeCValueMember(writer, local, if (inst_ty.isSimpleTuple(zcu))
                    ...
                    .{ .identifier = ip.stringToSlice(inst_ty.legacyStructFieldName(field_i, zcu)) });
                try a.assign(f, writer);
                try f.writeCValue(writer, element, .Other);
                try a.end(f, writer);
            },
...
fn airUnionInit(f: *Function, inst: Air.Inst.Index) !CValue {
    ...
    const field: CValue = if (union_ty.unionTagTypeSafety(zcu)) |tag_ty| field: {
        ...
        if (layout.tag_size != 0) {
            ...
            const int_val = try tag_val.intFromEnum(tag_ty, zcu);
...
fn compilerRtAbbrev(ty: Type, zcu: *Zcu) []const u8 {
    const target = zcu.getTarget();
...
const Vectorize = struct {
    ...
    pub fn start(f: *Function, inst: Air.Inst.Index, writer: anytype, ty: Type) !Vectorize {
        ...
        return if (ty.zigTypeTag(zcu) == .Vector) index: {
            const len_val = try zcu.intValue(Type.usize, ty.vectorLen(zcu));

            const local = try f.allocLocal(inst, Type.usize);

            try writer.writeAll("for (");
            try f.writeCValue(writer, local, .Other);
            try writer.print(" = {d}; ", .{try f.fmtIntLiteral(Type.usize, try zcu.intValue(Type.usize, 0))});
            try f.writeCValue(writer, local, .Other);
            try writer.print(" < {d}; ", .{
                try f.fmtIntLiteral(Type.usize, len_val),
            });
            try f.writeCValue(writer, local, .Other);
            try writer.print(" += {d}) {{\n", .{try f.fmtIntLiteral(Type.usize, try zcu.intValue(Type.usize, 1))});
            f.object.indent_writer.pushIndent();
