...
pub fn emit(
    ...
    try emitInstructionsForward(w, mir.prologue);
    try emitInstructionsBackward(w, mir.body);
    const body_end: u32 = @intCast(w.end);
    try emitInstructionsBackward(w, mir.epilogue);
    try w.splatByteAll(0, Instruction.size * literals_align_gap);
    try w.writeAll(@ptrCast(mir.literals));
    mir_log.debug("", .{});
