...
pub const Object = struct {
        ...
        switch (ty.zigTypeTag(mod)) {
            ...
            .Array => {
                const debug_array_type = try o.builder.debugArrayType(
                    ...
                    .none, // Scope
                    0, // Line
                    try o.lowerDebugType(ty.childType(mod), required_by_runtime),
                    ty.abiSize(mod) * 8,
                    (ty.abiAlignment(mod).toByteUnits() orelse 0) * 8,
                    try o.builder.debugTuple(&.{
            ...
            .Vector => {
                ...
                const debug_elem_type = switch (elem_ty.zigTypeTag(mod)) {
                    ...
                    else => try o.lowerDebugType(ty.childType(mod), required_by_runtime),
                };
    ...
    fn genNamespaces(o: *Object) !void {
        ...
        while (unresolved_i < unresolved.count()) : (unresolved_i += 1) {
            ...
            const owner_decl_index = ty.getOwnerDeclOrNull(mod);
            const owner_decl: ?*Module.Decl =
                if (owner_decl_index) |owner| o.module.declPtr(owner) else null;

            const file = if (owner_decl) |owner|
                try o.getDebugFile(mod.namespacePtr(owner.src_namespace).file_scope)
            else
                .none;
            const scope = if (owner_decl) |owner|
                ...
                o.debug_compile_unit;
            const line = if (owner_decl) |owner| owner.typeSrcLine(mod) + 1 else 0;
            ...
            const ns = if (ty.getNamespace(mod)) |n| n.unwrap() else null;
            if (ns) |ns_id| {
            ...
            const res = switch (ty.zigTypeTag(mod)) {
                .Enum => res: {
    ...
    fn namespaceToDebugScope(o: *Object, namespace_index: InternPool.NamespaceIndex) !Builder.Metadata {
        ...
        return o.lowerDebugType(mod.declPtr(namespace.decl_index).val.toType(), false);
    }
