...
pub const Object = struct {
    ...
    pub fn emit(o: *Object, pt: Zcu.PerThread, options: EmitOptions) !void {
        const zcu = pt.zcu;
        const comp = zcu.comp;
            ...
            .aggregate => switch (ip.indexToKey(ty.toIntern())) {
                .struct_type => {
                    ...
                    for (struct_type.field_types.get(ip), 0..) |field_ty, field_index| {
                        ...
                        const field_val = try o.lowerValueToInt(pt, llvm_int_ty, (try val.fieldValue(pt, field_index)).toIntern());
    ...
    fn lowerNavRefValue(o: *Object, pt: Zcu.PerThread, nav_index: InternPool.Nav.Index) Allocator.Error!Builder.Constant {
        ...
        const llvm_global = if (is_fn_body)
            (try o.resolveLlvmFunction(pt, owner_nav_index)).ptrConst(&o.builder).global
        else
            (try o.resolveGlobalNav(pt, owner_nav_index)).ptrConst(&o.builder).global;
    ...
    fn getCmpLtErrorsLenFunction(o: *Object, pt: Zcu.PerThread) !Builder.Function.Index {
        ...
        var attributes: Builder.FunctionAttributes.Wip = .{};
        defer attributes.deinit(&o.builder);
        try o.addCommonFnAttributes(pt, &attributes, zcu.root_mod, zcu.root_mod.omit_frame_pointer);
...
pub const FuncGen = struct {
    ...
    fn buildSimplePanic(fg: *FuncGen, panic_id: Zcu.PanicId) !void {
        ...
        const msg_ptr = try o.lowerValue(pt, msg_nav.status.resolved.val);
    ...
    fn airRetAddr(self: *FuncGen, inst: Air.Inst.Index) !Builder.Value {
        ...
        const pt = self.ng.pt;
        const llvm_usize = try o.lowerType(pt, Type.usize);
        if (!target_util.supportsReturnAddress(self.ng.pt.zcu.getTarget())) {
    ...
    fn safeWasmMemset(
        ...
        const o = self.ng.object;
        const pt = self.ng.pt;
        const usize_zero = try o.builder.intValue(try o.lowerType(pt, Type.usize), 0);
...
fn lowerFnRetTy(o: *Object, pt: Zcu.PerThread, fn_info: InternPool.Key.FuncType) Allocator.Error!Builder.Type {
    ...
    switch (fn_info.cc) {
        .Unspecified,
        .Inline,
        => return if (returnTypeByRef(zcu, target, return_type)) .void else o.lowerType(pt, return_type),

        .C => {
            switch (target.cpu.arch) {
                .mips, .mipsel => return o.lowerType(pt, return_type),
                .x86 => return if (isByRef(return_type, zcu)) .void else o.lowerType(pt, return_type),
                .x86_64 => switch (target.os.tag) {
                    .windows => return lowerWin64FnRetTy(o, pt, fn_info),
                    else => return lowerSystemVFnRetTy(o, pt, fn_info),
                },
                .wasm32 => {
        ...
        .Win64 => return lowerWin64FnRetTy(o, pt, fn_info),
        .SysV => return lowerSystemVFnRetTy(o, pt, fn_info),
        .Stdcall => return if (isScalar(zcu, return_type)) o.lowerType(pt, return_type) else .void,
        else => return o.lowerType(pt, return_type),
    }
}
