...
pub const Object = struct {
    ...
    /// - *Zcu.File => *DIFile
    /// - *Zcu.Decl (Fn) => *DISubprogram
    /// - *Zcu.Decl (Non-Fn) => *DIGlobalVariable
    ...
    fn genErrorNameTable(o: *Object) Allocator.Error!void {
        ...
        for (llvm_errors[1..], error_name_list[1..]) |*llvm_error, name| {
            const name_string = try o.builder.string(zcu.intern_pool.stringToSlice(name));
    ...
    fn resolveExportExternCollisions(object: *Object) !void {
        ...
        for (object.extern_collisions.keys()) |decl_index| {
            ...
            const decl_name = object.builder.stringIfExists(zcu.intern_pool.stringToSlice(zcu.declPtr(decl_index).name)) orelse continue;
    ...
    fn resolveGlobalCollisions(
        ...
        for (export_list) |exp| {
            ...
            const exp_name = object.builder.stringIfExists(zcu.intern_pool.stringToSlice(exp.opts.name)) orelse continue;
    ...
    pub fn updateDecl(self: *Object, module: *Zcu, decl_index: InternPool.DeclIndex) !void {
    ...
    pub fn updateExports(
        ...
        if (decl.isExtern(zcu)) {
            const decl_name = decl_name: {
                const decl_name = zcu.intern_pool.stringToSlice(decl.name);

                if (zcu.getTarget().isWasm() and try decl.isFunction(zcu)) {
                    if (zcu.intern_pool.stringToSliceUnwrap(decl.getOwnedExternFunc(zcu).?.lib_name)) |lib_name| {
            ...
            const main_exp_name = try self.builder.string(
                zcu.intern_pool.stringToSlice(exports[0].opts.name),
            );
            ...
            const fqn = try self.builder.string(
                zcu.intern_pool.stringToSlice(try decl.getFullyQualifiedName(zcu)),
            );
    ...
    fn updateExportedValue(
        ...
        const global_index = i: {
            ...
            const variable_index = try o.builder.addVariable(
                main_exp_name,
                try o.lowerType(Type.fromInterned(zcu.intern_pool.typeOf(exported_value))),
                llvm_addr_space,
            );
    ...
    fn updateExportedGlobal(
        ...
        if (zcu.intern_pool.stringToSliceUnwrap(exports[0].opts.section)) |section|
        ...
        for (exports[1..]) |exp| {
            const exp_name = try o.builder.string(zcu.intern_pool.stringToSlice(exp.opts.name));
    ...
    fn getDIFile(o: *Object, gpa: Allocator, file: *const Zcu.File) !*llvm.DIFile {
    ...
    fn lowerDebugTypeImpl(
        ...
        switch (ty.zigTypeTag(zcu)) {
            ...
            .Int => {
                ...
                const di_bits = ty.abiSize(zcu) * 8; // lldb cannot handle non-byte sized types
            ...
            .Enum => {
                ...
                const di_file = try o.getDIFile(gpa, zcu.namespacePtr(owner_decl.src_namespace).file_scope);
                ...
                const enum_di_ty = dib.createEnumerationType(
                    ...
                    ty.abiSize(zcu) * 8,
                    ty.abiAlignment(zcu).toByteUnits(0) * 8,
                    enumerators.ptr,
            ...
            .Pointer => {
                ...
                if (ty.isSlice(zcu)) {
                    ...
                    const full_di_ty = dib.createStructType(
                        ...
                        di_file,
                        line,
                        ty.abiSize(zcu) * 8, // size in bits
                        ty.abiAlignment(zcu).toByteUnits(0) * 8, // align in bits
                        0, // flags
                        null, // derived from
                        &fields,
                        fields.len,
                ...
                const ptr_di_ty = dib.createPointerType(
                    elem_di_ty,
                    target.ptrBitWidth(),
                    ty.ptrAlignment(zcu).toByteUnits(0) * 8,
                    name,
                );
                // The recursive call to `lowerDebugType` means we can't use `gop` anymore.
            ...
            .Opaque => {
                ...
                const opaque_di_ty = dib.createForwardDeclType(
                    ...
                    try o.getDIFile(gpa, zcu.namespacePtr(owner_decl.src_namespace).file_scope),
                    owner_decl.src_node + 1,
                );
            ...
            .Array => {
                const array_di_ty = dib.createArrayType(
                    ty.abiSize(zcu) * 8,
                    ty.abiAlignment(zcu).toByteUnits(0) * 8,
                    try o.lowerDebugType(ty.childType(zcu), .full),
                    @intCast(ty.arrayLen(zcu)),
                );
                // The recursive call to `lowerDebugType` means we can't use `gop` anymore.
            ...
            .Vector => {
                ...
                // vectors properly.
                const elem_di_type = switch (elem_ty.zigTypeTag(zcu)) {
                    .Int => blk: {
                    ...
                    else => try o.lowerDebugType(ty.childType(zcu), .full),
                };

                const vector_di_ty = dib.createVectorType(
                    ty.abiSize(zcu) * 8,
                    @intCast(ty.abiAlignment(zcu).toByteUnits(0) * 8),
                    elem_di_type,
                    ty.vectorLen(zcu),
                );
                // The recursive call to `lowerDebugType` means we can't use `gop` anymore.
            ...
            .Union => {
                ...
                if (!union_type.haveFieldTypes(ip) or !ty.hasRuntimeBitsIgnoreComptime(zcu)) {
                ...
                for (0..union_obj.field_names.len) |field_index| {
                    ...
                    const field_size = Type.fromInterned(field_ty).abiSize(zcu);
                    const field_align = zcu.unionFieldNormalAlignment(union_obj, @intCast(field_index));
    ...
    fn makeEmptyNamespaceDIType(o: *Object, decl_index: InternPool.DeclIndex) !*llvm.DIType {
        ...
        return o.di_builder.?.createStructType(
            di_scope,
            zcu.intern_pool.stringToSlice(decl.name), // TODO use fully qualified name
            try o.getDIFile(o.gpa, zcu.namespacePtr(decl.src_namespace).file_scope),
            decl.src_line + 1,
            0, // size in bits
            0, // align in bits
            0, // flags
    ...
    fn getStackTraceType(o: *Object) Allocator.Error!Type {
        ...
        const builtin_str = try zcu.intern_pool.getOrPutString(zcu.gpa, "builtin");
        ...
        const builtin_decl = std_namespace.decls
            .getKeyAdapted(builtin_str, Zcu.DeclAdapter{ .zcu = zcu }).?;

        const stack_trace_str = try zcu.intern_pool.getOrPutString(zcu.gpa, "StackTrace");
        // buffer is only used for int_type, `builtin` is a struct.
        const builtin_ty = zcu.declPtr(builtin_decl).val.toType();
        const builtin_namespace = builtin_ty.getNamespace(zcu).?;
        const stack_trace_decl_index = builtin_namespace.decls
            .getKeyAdapted(stack_trace_str, Zcu.DeclAdapter{ .zcu = zcu }).?;
    ...
    fn resolveGlobalDecl(
        ...
        const variable_index = try o.builder.addVariable(
            try o.builder.string(zcu.intern_pool.stringToSlice(
                if (is_extern) decl.name else try decl.getFullyQualifiedName(zcu),
            )),
    ...
    fn lowerType(o: *Object, t: Type) Allocator.Error!Builder.Type {
        ...
        if (std.debug.runtime_safety and o.builder.useLibLlvm() and false) check: {
            ...
            if (t.zigTypeTag(zcu) == .Opaque) break :check;
            if (!t.hasRuntimeBits(zcu)) break :check;
            if (!try ty.isSized(&o.builder)) break :check;

            const zig_size = t.abiSize(zcu);
            ...
            if (llvm_size != zig_size) {
                log.err("when lowering {}, Zig ABI size = {d} but LLVM ABI size = {d}", .{
                    t.fmt(o.zcu), zig_size, llvm_size,
                });
            }
        }
        return ty;
            ...
            => unreachable,
                ...
                .struct_type => |struct_type| {
                    ...
                    const name = try o.builder.string(ip.stringToSlice(
                        try zcu.declPtr(struct_type.decl.unwrap().?).getFullyQualifiedName(zcu),
                    ));
                ...
                .union_type => |union_type| {
                    ...
                    const name = try o.builder.string(ip.stringToSlice(
                        try zcu.declPtr(union_obj.decl).getFullyQualifiedName(zcu),
                    ));
                    const ty = try o.builder.opaqueType(name);
                    gop.value_ptr.* = ty; // must be done before any recursive calls
                ...
                .opaque_type => |opaque_type| {
                    ...
                    if (!gop.found_existing) {
                        const name = try o.builder.string(ip.stringToSlice(
                            try zcu.opaqueFullyQualifiedName(opaque_type),
                        ));
    ...
    fn lowerIntAsPtr(o: *Object, val: InternPool.Index) Allocator.Error!Builder.Constant {
        const zcu = o.zcu;
        switch (zcu.intern_pool.indexToKey(val)) {
            .undef => return o.builder.undefConst(.ptr),
            .int => {
                var bigint_space: Value.BigIntSpace = undefined;
                const bigint = Value.fromInterned(val).toBigInt(&bigint_space, zcu);
    ...
    fn lowerParentPtr(o: *Object, ptr_val: Value) Error!Builder.Constant {
        ...
        return switch (ptr.addr) {
            ...
            .eu_payload => |eu_ptr| {
                ...
                const eu_ty = Type.fromInterned(ip.typeOf(eu_ptr)).childType(zcu);
                ...
                const err_int_ty = try zcu.errorIntType();
                const payload_align = payload_ty.abiAlignment(zcu);
                const err_align = err_int_ty.abiAlignment(zcu);
            ...
            .opt_payload => |opt_ptr| {
                ...
                const opt_ty = Type.fromInterned(ip.typeOf(opt_ptr)).childType(zcu);
                const payload_ty = opt_ty.optionalChild(zcu);
                if (!payload_ty.hasRuntimeBitsIgnoreComptime(zcu) or
                    payload_ty.optionalReprIsPayload(zcu))
                {
            ...
            .elem => |elem_ptr| {
                ...
                const elem_ty = Type.fromInterned(ip.typeOf(elem_ptr.base)).elemType2(zcu);
            ...
            .field => |field_ptr| {
                ...
                switch (parent_ty.zigTypeTag(zcu)) {
                    .Union => {
                        if (parent_ty.containerLayout(zcu) == .Packed) {
                            return parent_ptr;
                        }

                        const layout = parent_ty.unionGetLayout(zcu);
                        if (layout.payload_size == 0) {
                    ...
                    .Struct => {
                        if (zcu.typeToPackedStruct(parent_ty)) |struct_type| {
                            const ptr_info = Type.fromInterned(ptr.ty).ptrInfo(zcu);
                            if (ptr_info.packed_offset.host_size != 0) return parent_ptr;

                            const parent_ptr_info = parent_ptr_ty.ptrInfo(zcu);
                            const bit_offset = zcu.structPackedFieldBitOffset(struct_type, field_index) + parent_ptr_info.packed_offset.bit_offset;
                        ...
                        return o.builder.gepConst(
                            ...
                            if (o.llvmFieldIndex(parent_ty, field_index)) |llvm_field_index| &.{
                                ...
                                try o.builder.intConst(.i32, @intFromBool(
                                    parent_ty.hasRuntimeBitsIgnoreComptime(zcu),
                                )),
                            },
                        );
                    },
                    .Pointer => {
                        assert(parent_ty.isSlice(zcu));
                        const parent_llvm_ty = try o.lowerType(parent_ty);
                        return o.builder.gepConst(.inbounds, parent_llvm_ty, parent_ptr, null, &.{
    ...
    fn lowerAnonDeclRef(
        ...
        const llvm_addr_space = toLlvmAddressSpace(orig_ty.ptrAddressSpace(zcu), target);
        const alignment = orig_ty.ptrAlignment(zcu);
        ...
        return o.builder.convConst(if (ptr_ty.isAbiInt(zcu)) switch (ptr_ty.intInfo(zcu).signedness) {
            .signed => .signed,
            .unsigned => .unsigned,
    ...
    fn lowerDeclRefValue(o: *Object, ty: Type, decl_index: InternPool.DeclIndex) Allocator.Error!Builder.Constant {
        ...
        if ((!is_fn_body and !decl.ty.hasRuntimeBits(zcu)) or
            (is_fn_body and zcu.typeToFunc(decl.ty).?.is_generic)) return o.lowerPtrToVoid(ty);
        ...
        return o.builder.convConst(if (ty.isAbiInt(zcu)) switch (ty.intInfo(zcu).signedness) {
            .signed => .signed,
            .unsigned => .unsigned,
    ...
    fn lowerPtrToVoid(o: *Object, ptr_ty: Type) Allocator.Error!Builder.Constant {
        ...
        const int: u64 = ptr_ty.ptrInfo(zcu).flags.alignment.toByteUnitsOptional() orelse
...
pub const DeclGen = struct {
    ...
    fn todo(dg: *DeclGen, comptime format: []const u8, args: anytype) Error {
        ...
        const gpa = o.gpa;
        const zcu = o.zcu;
        const src_loc = LazySrcLoc.nodeOffset(0).toSrcLoc(dg.decl, zcu);
...
pub const FuncGen = struct {
    ...
    fn resolveValue(self: *FuncGen, tv: TypedValue) Error!Builder.Constant {
        ...
        if (!isByRef(tv.ty, zcu)) return llvm_val;
        ...
        variable_index.setMutability(.constant, &o.builder);
        variable_index.setUnnamedAddr(.unnamed_addr, &o.builder);
        variable_index.setAlignment(tv.ty.abiAlignment(zcu).toLlvm(), &o.builder);
    ...
    fn resolveNullOptUsize(self: *FuncGen) Error!Builder.Constant {
        ...
        if (o.null_opt_usize == .no_init) {
            const ty = try zcu.intern(.{ .opt_type = .usize_type });
            o.null_opt_usize = try self.resolveValue(.{
                .ty = Type.fromInterned(ty),
                .val = Value.fromInterned((try zcu.intern(.{ .opt = .{ .ty = ty, .val = .none } }))),
            });
        }
    ...
    fn buildSimplePanic(fg: *FuncGen, panic_id: Zcu.PanicId) !void {
        ...
        const msg_len = msg_decl.ty.childType(zcu).arrayLen(zcu);
        const msg_ptr = try o.lowerValue(try msg_decl.internValue(zcu));
        ...
        const fn_info = zcu.typeToFunc(panic_decl.ty).?;
    ...
    fn airRet(self: *FuncGen, inst: Air.Inst.Index) !Builder.Value {
        ...
        const fn_info = zcu.typeToFunc(self.dg.decl.ty).?;
    ...
    fn airDbgInlineEnd(self: *FuncGen, inst: Air.Inst.Index) !Builder.Value {
        ...
        const zcu = o.zcu;
        const decl = zcu.funcOwnerDeclPtr(ty_fn.func);
        const di_file = try o.getDIFile(self.gpa, zcu.namespacePtr(decl.src_namespace).file_scope);
        self.di_file = di_file;
    ...
    fn airDbgVarPtr(self: *FuncGen, inst: Air.Inst.Index) !Builder.Value {
        ...
        const di_local_var = dib.createAutoVariable(
            ...
            self.di_file.?,
            self.prev_dbg_line,
            try o.lowerDebugType(ptr_ty.childType(zcu), .full),
    ...
    fn airArg(self: *FuncGen, inst: Air.Inst.Index) !Builder.Value {
        ...
        if (o.di_builder) |dib| {
            ...
            const di_local_var = dib.createParameterVariable(
                self.di_scope.?,
                zcu.getParamName(func_index, src_index).ptr, // TODO test 0 bit args
                self.di_file.?,
                lbrace_line,
    ...
    fn airErrorSetHasValue(self: *FuncGen, inst: Air.Inst.Index) !Builder.Value {
        ...
        for (names) |name| {
            const err_int = zcu.global_error_set.getIndex(name).?;
    ...
    fn nextInner(it: *ParamTypeIterator, ty: Type) Allocator.Error!?Lowering {
        ...
        switch (it.fn_info.cc) {
            .Unspecified, .Inline => {
                it.zig_index += 1;
                it.llvm_index += 1;
                if (ty.isSlice(zcu) or (ty.zigTypeTag(zcu) == .Optional and ty.optionalChild(zcu).isSlice(zcu))) {
                    it.llvm_index += 1;
                    return .slice;
