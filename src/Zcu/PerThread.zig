...
fn deinitFile(pt: Zcu.PerThread, file_index: Zcu.File.Index) void {
    ...
    log.debug("deinit File {s}", .{file.displayPath(zcu)});
...
pub fn updateFile(
    ...
    var source_file = try std.fs.openFileAbsolute(file.abs_path, .{});
    defer source_file.close();

    const stat = try source_file.stat();

    const want_local_cache = file.mod == zcu.main_mod;
    const hex_digest = Cache.HashHelper.oneShot(file.abs_path);
    ...
    var lock: std.fs.File.Lock = switch (file.status) {
        .never_loaded, .retryable_failure => lock: {
            ...
            log.debug("AstGen checking cache: {s} (local={}, digest={s})", .{
                file.abs_path, want_local_cache, &hex_digest,
            });
        ...
        .astgen_failure, .success => lock: {
            ...
            if (unchanged_metadata) {
                log.debug("unmodified metadata of file: {s}", .{file.abs_path});
                return;
            }

            log.debug("metadata changed: {s}", .{file.abs_path});
    ...
    const need_update = while (true) {
        ...
        switch (result) {
            .success => {
                log.debug("AstGen cached success: {s}", .{file.abs_path});
                break false;
            },
            .invalid => {},
            .truncated => log.warn("unexpected EOF reading cached ZIR for {s}", .{file.abs_path}),
            .stale => log.debug("AstGen cache stale: {s}", .{file.abs_path}),
        }
    ...
    if (need_update) {
            ...
            .zig => {
                ...
                Zcu.saveZirCache(gpa, cache_file, stat, file.zir.?) catch |err| switch (err) {
                    error.OutOfMemory => |e| return e,
                        ...
                        file.abs_path, cache_directory, &hex_digest, @errorName(err),
                    }),
                };
            },
            .zon => {
        ...
        log.debug("AstGen fresh success: {s}", .{file.abs_path});
    }
...
pub fn discoverImport(
    ...
    introspect.assertAbsolute(importer_path);
    ...
    introspect.assertAbsolute(resolved_path);

    // We're about to do a GOP on `import_table`, so we need the mutex.
    zcu.comp.mutex.lock();
    defer zcu.comp.mutex.unlock();

    const gop = try zcu.import_table.getOrPutAdapted(gpa, resolved_path, Zcu.ImportTableAdapter{ .zcu = zcu });
    errdefer _ = zcu.import_table.pop();
    if (gop.found_existing) {
        gpa.free(resolved_path); // we didn't need it for `File.abs_path`
    ...
    const new_file_index = try zcu.intern_pool.createFile(gpa, pt.tid, .{
        .bin_digest = Cache.HashHelper.oneShotBin(resolved_path),
        .file = new_file,
        .root_type = .none,
    });
...
pub fn doImport(
    ...
    return .{
        .file = zcu.import_table.getKeyAdapted(resolved_path, Zcu.ImportTableAdapter{ .zcu = zcu }).?,
        .module_root = null,
    };
}
...
pub fn populateModuleRootTable(pt: Zcu.PerThread) Allocator.Error!void {
    const zcu = pt.zcu;
    const gpa = zcu.gpa;
...
pub fn computeAliveFiles(pt: Zcu.PerThread) Allocator.Error!bool {
    ...
    while (live_check_idx < zcu.alive_files.count()) {
        ...
        for (0..extra.data.imports_len) |_| {
            ...
            const new_sub_path: ?[]const u8 = sub_path: {
                const abs_root_path = try std.fs.path.resolve(gpa, &.{
                    comp.cwd,
                    imported_mod.root.root_dir.path orelse ".",
                    imported_mod.root.sub_path,
                });
                defer gpa.free(abs_root_path);
                const abs_path = imported_file.abs_path; // externally managed, same lifetime as the file
                // We're about to do some direct string comparisons on the paths above. This is
                // okay because they are both from `std.fs.path.resolve` so we know there are no
                // redundant `.` or `..` components, no duplicate slashes, no trailing slashes.
                if (!std.mem.startsWith(u8, abs_path, abs_root_path)) {
                    break :sub_path null;
                }
                const after_root = abs_path[abs_root_path.len..];
                if (after_root.len < 2) break :sub_path null;
                if (after_root[0] != std.fs.path.sep) break :sub_path null;
                break :sub_path after_root[1..];
            };
...
pub fn updateBuiltinModule(pt: Zcu.PerThread, opts: Builtin) Allocator.Error!void {
    ...
    if (gop.found_existing) {
        // The `File` is up-to-date. Let's make sure the file on disk is still valid.
        const mod = gop.value_ptr.*;
        const file_index = zcu.module_roots.get(mod).?.unwrap().?;
        Builtin.updateFileOnDisk(zcu.fileByIndex(file_index)) catch |err| comp.setMiscFailure(
            .write_builtin_zig,
            "unable to write '{}{c}builtin.zig': {s}",
            .{ mod.root, std.fs.path.sep, @errorName(err) },
        );
        return;
    }
    errdefer _ = zcu.builtin_modules.pop();

    const mod: *Module = try .createBuiltin(comp.arena, opts, comp.global_cache_directory);
    ...
    const file_gop = try zcu.import_table.getOrPutAdapted(gpa, resolved_path, Zcu.ImportTableAdapter{ .zcu = zcu });
    if (file_gop.found_existing) {
        @panic("TODO error on user import of builtin.zig");
    }
    errdefer _ = zcu.import_table.pop();
    ...
    Builtin.updateFileOnDisk(file) catch |err| comp.setMiscFailure(
        .write_builtin_zig,
        "unable to write '{}{c}builtin.zig': {s}",
        .{ mod.root, std.fs.path.sep, @errorName(err) },
    );
...
pub fn embedFile(
    ...
    if (introspect.isUpDir(sub_file_path) or std.fs.path.isAbsolute(sub_file_path)) {
