...
fn createFileRootStruct(
    ...
    codegen_type: {
        ...
        if (file.mod.debug_format == .none or file.mod.debug_format == .symbols) break :codegen_type;
        // This job depends on any resolve_type_fully jobs queued up before it.
...
fn semaCau(pt: Zcu.PerThread, cau_index: InternPool.Cau.Index) !SemaCauResult {
    ...
    queue_codegen: {
        ...
        if (!try decl_ty.hasRuntimeBitsSema(pt)) {
            ...
            if (file.mod.debug_format == .none or file.mod.debug_format == .symbols) break :queue_codegen;
        }
    ...
    for (fn_info.param_body[0..src_params_len]) |inst| {
        ...
        sema.air_instructions.appendAssumeCapacity(.{
            ...
            .data = .{ .arg = .{
                ...
                .name = if (inner_block.ownerModule().debug_format == .none or inner_block.ownerModule().debug_format == .symbols)
                    .none
                else
