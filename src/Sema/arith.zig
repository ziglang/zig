...
//!
//! Functions starting with `int` or `float` are low-level primitives which operate on defined
//! scalar values; generally speaking, they are at the bottom of this file and non-`pub`.
...
fn intAdd(sema: *Sema, lhs: Value, rhs: Value, ty: Type) !struct { overflow: bool, val: Value } {
    const result = intAddInner(sema, lhs, rhs, ty) catch |err| switch (err) {
        error.Overflow => {
            const result = intAddInner(sema, lhs, rhs, .comptime_int) catch |err1| switch (err1) {
...
fn intAddInner(sema: *Sema, lhs: Value, rhs: Value, ty: Type) !Value {
    const pt = sema.pt;
    const zcu = pt.zcu;
    if (ty.toIntern() != .comptime_int_type) {
        const res = try intAddWithOverflow(sema, lhs, rhs, ty);
        switch (res.overflow_bit.toUnsignedInt(zcu)) {
            0 => return res.wrapped_result,
            1 => return error.Overflow,
            else => unreachable,
        }
    }
...
fn intSub(sema: *Sema, lhs: Value, rhs: Value, ty: Type) !struct { overflow: bool, val: Value } {
    const result = intSubInner(sema, lhs, rhs, ty) catch |err| switch (err) {
        error.Overflow => {
            const result = intSubInner(sema, lhs, rhs, .comptime_int) catch |err1| switch (err1) {
                error.Overflow => unreachable,
                else => |e| return e,
            };
            return .{ .overflow = true, .val = result };
...
fn intSubInner(sema: *Sema, lhs: Value, rhs: Value, ty: Type) !Value {
    const pt = sema.pt;
    const zcu = pt.zcu;
    if (ty.toIntern() != .comptime_int_type) {
...
fn intMul(sema: *Sema, lhs: Value, rhs: Value, ty: Type) !struct { overflow: bool, val: Value } {
    const result = intMulInner(sema, lhs, rhs, ty) catch |err| switch (err) {
        error.Overflow => {
            const result = intMulInner(sema, lhs, rhs, .comptime_int) catch |err1| switch (err1) {
                error.Overflow => unreachable,
                else => |e| return e,
            };
            return .{ .overflow = true, .val = result };
...
fn intMulInner(sema: *Sema, lhs: Value, rhs: Value, ty: Type) !Value {
    const pt = sema.pt;
    const zcu = pt.zcu;
    if (ty.toIntern() != .comptime_int_type) {
