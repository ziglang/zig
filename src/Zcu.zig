    ...
    pub fn resolveBaseNode(base_node_inst: InternPool.TrackedInst.Index, zcu: *Zcu) ?struct { *File, Ast.Node.Index } {
        ...
        const base_node: Ast.Node.Index = switch (inst.tag) {
            ...
            .extended => switch (inst.data.extended.opcode) {
                ...
                .enum_reify => zir.extraData(Zir.Inst.Reify, inst.data.extended.operand).data.node,
                .pointer_reify => zir.extraData(Zir.Inst.Reify, inst.data.extended.operand).data.node,
                .struct_reify => zir.extraData(Zir.Inst.Reify, inst.data.extended.operand).data.node,
                .union_reify => zir.extraData(Zir.Inst.Reify, inst.data.extended.operand).data.node,
                else => unreachable,
            },
            else => unreachable,
        };
