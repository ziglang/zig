...
pub fn flushModule(self: *Plan9, arena: Allocator, prog_node: *std.Progress.Node) link.File.FlushError!void {
    ...
    var foff = hdr_size;
    iovecs[0] = .{ .ptr = hdr_slice.ptr, .len = hdr_slice.len };
    ...
    // text
    {
        ...
        while (it_file.next()) |fentry| {
            ...
            while (it.next()) |entry| {
                ...
                foff += out.code.len;
                iovecs[iovecs_i] = .{ .ptr = out.code.ptr, .len = out.code.len };
                iovecs_i += 1;
    ...
    // the text lazy symbols
    {
        ...
        while (it.next()) |kv| {
            ...
            foff += code.len;
            iovecs[iovecs_i] = .{ .ptr = code.ptr, .len = code.len };
            iovecs_i += 1;
            const off = self.getAddr(text_i, .t);
            text_i += code.len;
    ...
    iovecs[iovecs_i] = .{ .ptr = got_table.ptr, .len = got_table.len };
    iovecs_i += 1;
    // data
    ...
    iovecs[iovecs_i] = .{ .ptr = syms.ptr, .len = syms.len };
    iovecs_i += 1;
    iovecs[iovecs_i] = .{ .ptr = linecountinfo.items.ptr, .len = linecountinfo.items.len };
    iovecs_i += 1;
    // generate the header
    self.hdr = .{
        .magic = self.magic,
