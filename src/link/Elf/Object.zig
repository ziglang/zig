...
fn formatComdatGroups(ctx: FormatContext, bw: *Writer, comptime unused_fmt_string: []const u8) Writer.Error!void {
    _ = unused_fmt_string;
    const object = ctx.object;
    const elf_file = ctx.elf_file;
    try bw.writeAll("  COMDAT groups\n");
    for (object.comdat_groups.items, 0..) |cg, cg_index| {
        try bw.print("    COMDAT({d})", .{cg_index});
        if (!cg.alive) try bw.writeAll(" : [*]");
        try bw.writeByte('\n');
