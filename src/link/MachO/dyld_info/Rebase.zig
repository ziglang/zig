...
fn finalizeSegment(entries: []const Entry, bw: *std.io.BufferedWriter) anyerror!void {
...
fn rebaseAddAddr(addr: u64, bw: *std.io.BufferedWriter) anyerror!void {
    log.debug(">>> rebase with add: {x}", .{addr});
    try bw.writeByte(macho.REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB);
    try bw.writeLeb128(addr);
}

fn rebaseTimes(count: usize, bw: *std.io.BufferedWriter) anyerror!void {
...
fn rebaseTimesSkip(count: usize, skip: u64, bw: *std.io.BufferedWriter) anyerror!void {
...
pub fn write(rebase: Rebase, bw: *std.io.BufferedWriter) anyerror!void {
