...
pub const WipNav = struct {
    ...
    fn abbrevCode(wip_nav: *WipNav, abbrev_code: AbbrevCode) (UpdateError || Writer.Error)!void {
        try wip_nav.debug_info.ensureTotalCapacity(AbbrevCode.max_bytes);
    ...
    fn bigIntConstValue(
        ...
        if (bits <= 64) {
            ...
            while (bit < bits) {
                ...
                bit += 7;
                try wip_nav.debug_info.writer.writeByte(@as(u8, if (bit < bits) 0x80 else 0x00) | twos_comp_part);
            }
        } else {
            ...
            big_int.writeTwosComplement(
                try wip_nav.debug_info.writer.writableSlice(@intCast(bytes)),
    ...
    switch (ip.indexToKey(value_index)) {
        ...
        .error_union => |error_union| {
            ...
            };
            {
                ...
                try wip_nav.strp("is_error");
                try diw.writeUleb128(err_abi_size);
                dwarf.writeInt(try wip_nav.debug_info.writer.writableSlice(err_abi_size), err_value);
            }
...
fn refAbbrevCode(
    ...
    var debug_abbrev_aw: Writer.Allocating = try .initCapacity(dwarf.gpa, AbbrevCode.max_bytes);
    ...
    if (dwarf.debug_line.section.dirty) {
        ...
        for (dwarf.mods.values(), dwarf.debug_line.section.units.items) |mod_info, *unit| {
            ...
            for (mod_info.files.keys()) |file_index| {
                ...
                hw.splatByteAll(0, dwarf.sectionOffsetBytes()) catch unreachable;
                dwarf.writeInt(
                    hw.writableSlice(dir_index_info.bytes) catch unreachable,
...
const AbbrevCode = enum {
    ...
    const max_bytes = uleb128Bytes(@typeInfo(AbbrevCode).@"enum".fields.len - 1);
    comptime {
