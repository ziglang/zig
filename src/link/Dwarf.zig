    ...
    fn write(loc: Loc, bw: *std.io.BufferedWriter, adapter: anytype) anyerror!void {
        switch (loc) {
            ...
            .addr => |addr| {
                ...
                switch (addr) {
                    .sym => |sym_index| try adapter.addrSym(bw, sym_index),
                }
            },
...
pub const WipNav = struct {
    ...
    pub fn genLocalDebugInfo(
        ...
        assert(wip_nav.func != .none);
        wip_nav.abbrevCode(switch (tag) {
            inline else => |ct_tag| @field(AbbrevCode, @tagName(ct_tag)),
        }) catch |err| return @errorCast(err);
        wip_nav.strp(name) catch |err| return @errorCast(err);
        wip_nav.refType(ty) catch |err| return @errorCast(err);
        wip_nav.infoExprloc(loc) catch |err| return @errorCast(err);
    ...
    fn blockValue(wip_nav: *WipNav, src_loc: Zcu.LazySrcLoc, val: Value) anyerror!void {
        ...
        assert(old_len + bytes == dial.items.len);
    }
...
fn initWipNavInner(
    ...
    switch (nav_key) {
        ...
        .func => |func| if (func.owner_nav != nav_index) {
            ...
            wip_nav.func_high_pc = @intCast(dibw.count);
            try dibw.writeInt(u32, 0, dwarf.endian);
            const target = file.mod.?.resolved_target.result;
            try uleb128(dibw, switch (nav.status.fully_resolved.alignment) {
