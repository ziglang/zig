...
pub fn init(file: std.fs.File, gpa: std.mem.Allocator) !MappedFile {
    ...
    mf.flags = .{
        .block_size = .fromByteUnits(
            std.math.ceilPowerOfTwoAssert(usize, @max(std.heap.pageSize(), stat.blksize)),
        ),
    ...
    try mf.ensureTotalCapacity(@intCast(stat.size));
    return mf;
}

pub fn deinit(mf: *MappedFile, gpa: std.mem.Allocator) void {
    if (mf.contents.len > 0) std.posix.munmap(mf.contents);
    mf.nodes.deinit(gpa);
    mf.large.deinit(gpa);
...
pub const Node = extern struct {
    ...
    pub const Index = enum(u32) {
        ...
        pub fn writer(ni: Node.Index, mf: *MappedFile, gpa: std.mem.Allocator) Writer {
            return .{
                .gpa = gpa,
                .mf = mf,
                .ni = ni,
    ...
    pub const Writer = struct {
        ...
        fn sendFile(
            ...
            switch (file_reader.mode) {
                .positional => {
                    ...
                    const copy_size = w.mf.copyFileRange(
                        file_reader.file,
                        file_reader.pos,
        ...
        fn growingRebase(
            ...
            _ = preserve;
            const w: *Writer = @fieldParentPtr("interface", interface);
            const ensure_capacity = interface.end + unused_capacity;
            if (interface.buffer.len >= ensure_capacity) return;
            w.mf.resizeNode(w.gpa, w.ni, ensure_capacity +| ensure_capacity / 2) catch |err| {
                w.err = err;
                return error.WriteFailed;
            };
            interface.buffer = w.ni.slice(w.mf);
        }
    };
...
fn addNode(mf: *MappedFile, gpa: std.mem.Allocator, opts: struct {
    ...
    };
    {
        ...
        if (offset > opts.parent.location(mf).resolve(mf)[1])
            try mf.resizeNode(gpa, opts.parent, offset);
...
pub fn resizeNode(mf: *MappedFile, gpa: std.mem.Allocator, ni: Node.Index, requested_size: u64) !void {
    const node = ni.get(mf);
    ...
    while (true) {
        ...
        insert_range: {
            ...
            retry: while (true) {
                switch (std.os.linux.E.init(std.os.linux.fallocate(
                    mf.file.handle,
                    std.os.linux.FALLOC.FL_INSERT_RANGE,
        ...
        switch (node.next) {
            ...
            else => |*next_ni| switch (node.flags.fixed) {
                false => {
                    ...
                    if (node.flags.has_content) {
                        ...
                        const old_file_offset: usize = @intCast(parent_file_offset + old_offset);
                        const new_file_offset: usize = @intCast(parent_file_offset + new_offset);
                        // make a copy of this node at the new location
                        try mf.copyRange(old_file_offset, new_file_offset, old_size);
                        // delete the copy of this node at the old location
                        const is_large = old_size >= mf.flags.block_size.toByteUnits() * 2 - 1;
                        punch_hole: {
                            if (is_linux and !mf.flags.fallocate_punch_hole_unsupported and is_large) {
                                retry: while (true) {
                                    switch (std.os.linux.E.init(std.os.linux.fallocate(
                                        mf.file.handle,
                            ...
                            @memset(mf.contents[old_file_offset..][0..@intCast(old_size)], 0);
                        }
                    }
...
fn copyFileRange(
    ...
    if (is_linux and !mf.flags.copy_file_range_unsupported) {
        ...
        retry: while (remaining_size >= mf.flags.block_size.toByteUnits() * 2 - 1) {
            const copy_len = std.os.linux.copy_file_range(
                old_file.handle,
                &old_file_offset_mut,
                mf.file.handle,
                &new_file_offset_mut,
            ...
            switch (std.os.linux.E.init(copy_len)) {
                .SUCCESS => {
                    if (copy_len == 0) break;
...
pub fn ensureTotalCapacityPrecise(mf: *MappedFile, new_capacity: usize) !void {
    ...
    mf.contents = switch (mf.contents.len) {
        0 => try std.posix.mmap(
            null,
            aligned_capacity,
