    ...
    pub fn apply(reloc: *const Reloc, elf: *Elf) void {
        ...
        switch (elf.shdrSlice()) {
            inline else => |shdr, class| {
                ...
                const loc_file_offset = loc_value - loc_sh_addr + loc_sh_offset;
...
fn addSymbolAssumeCapacity(elf: *Elf, opts: struct {
    node: MappedFile.Node.Index = .none,
    name: []const u8 = "",
    size: std.elf.Word = 0,
    type: std.elf.STT,
    ...
    const si: Symbol.Index = @enumFromInt(elf.symtab.items.len);
    elf.symtab.addOneAssumeCapacity().* = .{
        .ni = opts.node,
        .loc_relocs = .none,
        .target_relocs = .none,
    ...
    errdefer _ = elf.symtab.pop();
    try elf.mf.resizeNode(gpa, Symbol.Index.symtab.node(elf), @as(usize, switch (elf.identClass()) {
        .NONE, _ => unreachable,
...
pub fn globalSymbol(
    ...
    if (!sym_gop.found_existing) sym_gop.value_ptr.* = try elf.addSymbolAssumeCapacity(.{
        .name = opts.name,
        .type = opts.type,
        .bind = opts.bind,
        .visibility = opts.visibility,
...
fn navType(
    ...
    if (true) return switch (nav_status) {
...
pub fn navSymbol(elf: *Elf, zcu: *Zcu, nav_index: InternPool.Nav.Index) !Symbol.Index {
    ...
    if (!sym_gop.found_existing) {
        sym_gop.value_ptr.* = try elf.addSymbolAssumeCapacity(.{
...
pub fn uavSymbol(elf: *Elf, uav_val: InternPool.Index) !Symbol.Index {
    ...
    if (!sym_gop.found_existing)
        sym_gop.value_ptr.* = try elf.addSymbolAssumeCapacity(.{ .type = .OBJECT });
    return sym_gop.value_ptr.*;
}
...
fn addSection(
    elf: *Elf,
    segment_ni: MappedFile.Node.Index,
    opts: struct {
        name: []const u8 = "",
    ...
    try elf.mf.resizeNode(gpa, Node.known.shdr, shdr_size);
    ...
    switch (node) {
        ...
        .section => |si| switch (elf.shdrSlice()) {
            inline else => |shdr, class| {
                ...
                const shndx = std.mem.toNative(@TypeOf(sym.shndx), sym.shndx, target_endian);
                const sh = &shdr[shndx];
                const flags: std.elf.SHF = @bitCast(std.mem.toNative(
                    @typeInfo(std.elf.SHF).@"struct".backing_integer.?,
                    @bitCast(sh.flags),
                    target_endian,
                ));
                if (flags.ALLOC) {
