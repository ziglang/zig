...
pub fn flushModule(self: *C, arena: Allocator, prog_node: *std.Progress.Node) !void {
    ...
    f.all_buffers.items[ctypes_index] = .{
        .ptr = if (f.ctypes_buf.items.len > 0) f.ctypes_buf.items.ptr else "",
    ...
    f.all_buffers.items[lazy_index] = .{
        .ptr = if (lazy_fwd_decl_len > 0) self.lazy_fwd_decl_buf.items.ptr else "",
...
const Flush = struct {
    ...
    all_buffers: std.ArrayListUnmanaged(std.io.WriteBuffers) = .{},
    ...
    fn appendBufAssumeCapacity(f: *Flush, buf: []const u8) void {
        if (buf.len == 0) return;
        f.all_buffers.appendAssumeCapacity(.{ .ptr = buf.ptr, .len = buf.len });
        f.file_size += buf.len;
    }
...
pub fn flushEmitH(module: *Module) !void {
    ...
    var all_buffers = try std.ArrayList(std.io.WriteBuffers).initCapacity(module.gpa, num_buffers);
