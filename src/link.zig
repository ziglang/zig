    ...
    pub fn linkAsArchive(base: *File, arena: Allocator, prog_node: std.Progress.Node) FlushError!void {
        ...
        if (!base.disable_lld_caching) {
            ...
            const prev_digest: []u8 = Cache.readSmallFile(
                ...
                log.debug("archive new_digest={s} readFile error: {s}", .{ std.fmt.fmtSliceHex(&digest, .lower), @errorName(err) });
                break :b prev_digest_buf[0..0];
            };
            if (mem.eql(u8, prev_digest, &digest)) {
                log.debug("archive digest={s} match - skipping invocation", .{std.fmt.fmtSliceHex(&digest, .lower)});
