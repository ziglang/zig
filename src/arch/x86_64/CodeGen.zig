    ...
    fn getSymbolIndex(owner: Owner, ctx: *Self) !u32 {
        switch (owner) {
            .func_index => |func_index| {
                const zcu = ctx.bin_file.comp.zcu.?;
                const decl_index = zcu.funcOwnerDeclIndex(func_index);
...
fn gen(self: *Self) InnerError!void {
    const zcu = self.bin_file.comp.zcu.?;
    ...
    if (cc != .Naked) {
        ...
        if (fn_info.is_var_args) switch (cc) {
            .SysV => {
                ...
                const vec_2_f64 = try zcu.vectorType(.{ .len = 2, .child = .f64_type });
...
fn genLazy(self: *Self, lazy_sym: link.File.LazySymbol) InnerError!void {
    const zcu = self.bin_file.comp.zcu.?;
    switch (lazy_sym.ty.zigTypeTag(zcu)) {
        .Enum => {
            const enum_ty = lazy_sym.ty;
            wip_mir_log.debug("{}.@tagName:", .{enum_ty.fmt(zcu)});
            ...
            for (exitlude_jump_relocs, 0..) |*exitlude_jump_reloc, index_usize| {
                ...
                const tag_name = zcu.intern_pool.stringToSlice(enum_ty.enumFields(zcu)[index_usize]);
                const tag_val = try zcu.enumValueFieldIndex(enum_ty, index);
...
fn allocRegOrMemAdvanced(self: *Self, ty: Type, inst: ?Air.Inst.Index, reg_ok: bool) !MCValue {
    ...
    const abi_size = math.cast(u32, ty.abiSize(zcu)) orelse {
        return self.fail("type '{}' too big to fit into stack frame", .{ty.fmt(zcu)});
    };
...
fn airTrunc(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        ...
        const dst_mcv = if (src_mcv.isRegister() and self.reuseOperand(inst, ty_op.operand, 0, src_mcv))
            ...
            break :dst dst_mcv;
        } else return self.fail("TODO implement trunc from {} to {}", .{ src_ty.fmt(zcu), dst_ty.fmt(zcu) });

        if (dst_ty.zigTypeTag(zcu) == .Vector) {
            assert(src_ty.zigTypeTag(zcu) == .Vector and dst_ty.vectorLen(zcu) == src_ty.vectorLen(zcu));
            const dst_info = dst_ty.childType(zcu).intInfo(zcu);
            const src_info = src_ty.childType(zcu).intInfo(zcu);
            const mir_tag = @as(?Mir.Inst.FixedTag, switch (dst_info.bits) {
                8 => switch (src_info.bits) {
                    16 => switch (dst_ty.vectorLen(zcu)) {
                ...
                16 => switch (src_info.bits) {
                    32 => switch (dst_ty.vectorLen(zcu)) {
            ...
            }) orelse return self.fail("TODO implement airTrunc for {}", .{dst_ty.fmt(zcu)});

            const elem_ty = src_ty.childType(zcu);
            const mask_val = try zcu.intValue(elem_ty, @as(u64, math.maxInt(u64)) >> @intCast(64 - dst_info.bits));

            const splat_ty = try zcu.vectorType(.{
            ...
            const splat_val = try zcu.intern(.{ .aggregate = .{
        ...
        if (dst_abi_size <= 8) {
            ...
            const high_ty = try zcu.intType(dst_info.signedness, dst_info.bits - 64);
...
fn airMulDivBinOp(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        ...
        const dst_info = dst_ty.intInfo(zcu);
        const src_ty = try zcu.intType(dst_info.signedness, switch (tag) {
            else => unreachable,
            .mul, .mul_wrap => @max(
                    ...
                    .{ .lib = .{
                        ...
                        .callee = std.fmt.bufPrint(&callee_buf, "__{s}{s}{c}i3", .{
                            ...
                            switch (tag) {
                                ...
                                .div_floor => if (signed) "zcu" else "div",
                                .rem, .mod => "zcu",
                                else => unreachable,
                            },
...
fn airMulSat(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        if (ty.toIntern() == .i128_type) {
            const ptr_c_int = try zcu.singleMutPtrType(Type.c_int);
...
fn genSetFrameTruncatedOverflowCompare(
    ...
    const hi_ty = try zcu.intType(int_info.signedness, hi_bits);

    const limb_bits: u16 = @intCast(if (int_info.bits <= 64) self.regBitSize(ty) else 64);
    const limb_ty = try zcu.intType(int_info.signedness, limb_bits);

    const rest_ty = try zcu.intType(.unsigned, int_info.bits - hi_bits);
...
fn airMulWithOverflow(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = switch (dst_ty.zigTypeTag(zcu)) {
        .Vector => return self.fail("TODO implement airMulWithOverflow for {}", .{dst_ty.fmt(zcu)}),
        .Int => result: {
            const dst_info = dst_ty.intInfo(zcu);
            ...
            const src_ty = try zcu.intType(dst_info.signedness, src_bits);
    ...
    const result: MCValue = result: {
        switch (lhs_ty.zigTypeTag(zcu)) {
            ...
            .Vector => switch (lhs_ty.childType(zcu).zigTypeTag(zcu)) {
                .Int => if (@as(?Mir.Inst.FixedTag, switch (lhs_ty.childType(zcu).intInfo(zcu).bits) {
                ...
                })) |mir_tag| if (try self.air.value(bin_op.rhs, zcu)) |rhs_val| {
                    ...
                    .splat => {
                        ...
                        const mask_ty = try zcu.vectorType(.{ .len = 16, .child = .u8_type });
                        const mask_mcv = try self.genTypedValue(.{
                            .ty = mask_ty,
                            .val = Value.fromInterned((try zcu.intern(.{ .aggregate = .{
                                .ty = mask_ty.toIntern(),
                                .storage = .{ .elems = &([1]InternPool.Index{
                                    (try rhs_ty.childType(zcu).maxIntScalar(zcu, Type.u8)).toIntern(),
                                } ++ [1]InternPool.Index{
                                    (try zcu.intValue(Type.u8, 0)).toIntern(),
                                } ** 15) },
                            } }))),
                        });
                        const mask_addr_reg =
        ...
        return self.fail("TODO implement airShlShrBinOp for {}", .{lhs_ty.fmt(zcu)});
    };
    return self.finishAir(inst, result, .{ bin_op.lhs, bin_op.rhs, .none });
}
        ...
        if (array_ty.isVector(zcu) and elem_ty.bitSize(zcu) == 1) {
            ...
            switch (array_mcv) {
                ...
                .memory, .load_symbol, .load_direct, .load_got, .load_tlv => try self.asmMemoryRegister(
                    ...
                    @tagName(array_mcv), array_ty.fmt(zcu),
                }),
            }
...
fn airSetUnionTag(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const ptr_tag_ty = try zcu.adjustPtrTypeChild(ptr_union_ty, tag_ty);
...
fn airClz(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        ...
        if (has_lzcnt) {
            ...
            } else return self.fail("TODO airClz of {}", .{src_ty.fmt(zcu)});
            break :result dst_mcv;
        }

        if (src_bits > 64)
            return self.fail("TODO airClz of {}", .{src_ty.fmt(zcu)});
    ...
    const result = result: {
        ...
        if (self.hasFeature(.bmi)) {
            if (src_bits <= 64) {
                ...
                try self.asmCmovccRegisterRegister(.nc, dst_reg.to32(), tmp_reg.to32());
            } else return self.fail("TODO airCtz of {}", .{src_ty.fmt(zcu)});
            break :result dst_mcv;
        }

        if (src_bits > 64) return self.fail("TODO airCtz of {}", .{src_ty.fmt(zcu)});
...
fn airPopCount(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = result: {
        ...
        if (src_ty.zigTypeTag(zcu) == .Vector or src_abi_size > 16)
            return self.fail("TODO implement airPopCount for {}", .{src_ty.fmt(zcu)});
        const src_mcv = try self.resolveInst(ty_op.operand);
        ...
        const src_info = src_ty.intInfo(zcu);
        const hi_ty = try zcu.intType(src_info.signedness, (src_info.bits - 1) % 64 + 1);
...
fn genPopCount(
    ...
    if (self.hasFeature(.popcnt)) return self.genBinOpMir(
        ...
        if (src_abi_size > 1) src_mcv else src: {
            ...
            try self.truncateRegister(try src_ty.toUnsigned(zcu), dst_reg);
...
fn floatSign(self: *Self, inst: Air.Inst.Index, operand: Air.Inst.Ref, ty: Type) !void {
    ...
    const result = result: {
        ...
        const vec_ty = try zcu.vectorType(.{
            .len = @divExact(abi_size * 8, scalar_bits),
            .child = (try zcu.intType(.signed, scalar_bits)).ip_index,
        });

        const sign_mcv = try self.genTypedValue(.{ .ty = vec_ty, .val = switch (tag) {
            .neg => try vec_ty.minInt(zcu, vec_ty),
            .abs => try vec_ty.maxInt(zcu, vec_ty),
            else => unreachable,
        } });
        ...
        if (self.hasFeature(.avx)) try self.asmRegisterRegisterMemory(
            switch (scalar_bits) {
                ...
                80 => return self.fail("TODO implement floatSign for {}", .{ty.fmt(zcu)}),
                else => unreachable,
            },
            registerAlias(dst_reg, abi_size),
...
fn genRoundLibcall(self: *Self, ty: Type, src_mcv: MCValue, mode: RoundMode) !MCValue {
    ...
    if (ty.zigTypeTag(zcu) != .Float)
        return self.fail("TODO implement genRound for {}", .{ty.fmt(zcu)});
    ...
    const result: MCValue = result: {
        const mir_tag = @as(?Mir.Inst.FixedTag, switch (ty.zigTypeTag(zcu)) {
            ...
            .Int => switch (ty.abiSize(zcu)) {
                ...
                else => return self.fail("TODO implement abs for {}", .{ty.fmt(zcu)}),
            },
        ...
        }) orelse return self.fail("TODO implement airAbs for {}", .{ty.fmt(zcu)});
...
fn packedLoad(self: *Self, dst_mcv: MCValue, ptr_ty: Type, ptr_mcv: MCValue) InnerError!void {
    ...
    if (val_abi_size > 8) return self.fail("TODO implement packed load of {}", .{val_ty.fmt(zcu)});
...
fn packedStore(self: *Self, ptr_ty: Type, ptr_mcv: MCValue, src_mcv: MCValue) InnerError!void {
    ...
    const limb_abi_bits = limb_abi_size * 8;
    const limb_ty = try zcu.intType(.unsigned, limb_abi_bits);
    ...
    while (limb_i * limb_abi_bits < src_bit_off + src_bit_size) : (limb_i += 1) {
        ...
        } else return self.fail("TODO: implement packed store of {}", .{src_ty.fmt(zcu)});
    }
}
...
fn airStructFieldVal(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = result: {
        ...
        const field_off: u32 = switch (container_ty.containerLayout(zcu)) {
            .Auto, .Extern => @intCast(container_ty.structFieldOffset(index, zcu) * 8),
            .Packed => if (zcu.typeToStruct(container_ty)) |struct_type|
            ...
            .register_pair => |src_regs| {
                ...
                const src_reg = if (field_off + field_bit_size <= 64)
                    ...
                    if (field_bit_size < 128) try self.truncateRegister(
                        try zcu.intType(.unsigned, @intCast(field_bit_size - 64)),
                        dst_regs[1],
                    );
                    break :result if (field_rc.supersetOf(abi.RegisterClass.gp))
                        dst_mcv
...
fn airFieldParentPtr(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const field_offset: i32 = @intCast(parent_ty.structFieldOffset(extra.field_index, zcu));
...
fn genUnOp(self: *Self, maybe_inst: ?Air.Inst.Index, tag: Air.Inst.Tag, src_air: Air.Inst.Ref) !MCValue {
    ...
    if (src_ty.zigTypeTag(zcu) == .Vector)
        return self.fail("TODO implement genUnOp for {}", .{src_ty.fmt(zcu)});
...
fn genUnOpMir(self: *Self, mir_tag: Mir.Inst.FixedTag, dst_ty: Type, dst_mcv: MCValue) !void {
    const zcu = self.bin_file.comp.zcu.?;
    const abi_size: u32 = @intCast(dst_ty.abiSize(zcu));
    if (abi_size > 8) return self.fail("TODO implement {} for {}", .{ mir_tag, dst_ty.fmt(zcu) });
    switch (dst_mcv) {
        .none,
        .unreach,
        .dead,
...
fn genShiftBinOp(
    ...
    assert(rhs_ty.abiSize(zcu) == 1);
    try self.spillEflagsIfOccupied();

    const lhs_abi_size = lhs_ty.abiSize(zcu);
...
fn genMulDivBinOp(
    ...
    if (dst_ty.zigTypeTag(zcu) == .Vector or dst_ty.zigTypeTag(zcu) == .Float) return self.fail(
        ...
        .{ @tagName(tag), src_ty.fmt(zcu), dst_ty.fmt(zcu) },
    );
    const dst_abi_size: u32 = @intCast(dst_ty.abiSize(zcu));
    const src_abi_size: u32 = @intCast(src_ty.abiSize(zcu));
...
fn genBinOp(
    ...
    if (lhs_ty.isRuntimeFloat()) libcall: {
        ...
        const callee = switch (air_tag) {
            ...
            .rem, .mod, .min, .max => std.fmt.bufPrint(&callee_buf, "{s}f{s}{s}", .{
                ...
                @tagName(air_tag), lhs_ty.fmt(zcu),
            }),
        } catch unreachable;
        const result = try self.genCall(.{ .lib = .{
    ...
    if (sse_op and ((lhs_ty.scalarType(zcu).isRuntimeFloat() and
        ...
        return self.fail("TODO implement genBinOp for {s} {}", .{ @tagName(air_tag), lhs_ty.fmt(zcu) });
        ...
        .cmp_lt, .cmp_lte, .cmp_eq, .cmp_gte, .cmp_gt, .cmp_neq => {
            switch (lhs_ty.childType(zcu).zigTypeTag(zcu)) {
                .Int => switch (air_tag) {
                    ...
                    .cmp_neq,
                    => {
                        const unsigned_ty = try lhs_ty.toUnsigned(zcu);
                        const not_mcv = try self.genTypedValue(.{
                            .ty = lhs_ty,
                            .val = try unsigned_ty.maxInt(zcu, unsigned_ty),
                        });
...
fn airArg(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = if (self.liveness.isUnused(inst)) .unreach else result: {
        ...
        const name = zcu.getParamName(self.owner.func_index, src_index);
...
fn genArgDbgInfo(self: Self, ty: Type, name: [:0]const u8, mcv: MCValue) !void {
    ...
    switch (self.debug_output) {
        .dwarf => |dw| {
            ...
            try dw.genArgDbgInfo(name, ty, self.owner.getDecl(zcu), loc);
        },
        .plan9 => {},
        .none => {},
    }
...
fn genVarDbgInfo(
    ...
    switch (self.debug_output) {
        .dwarf => |dw| {
            ...
            // Instead of flattening the owner and passing Decl.Index here we may
            // want to special case LazySymbol in DWARF linker too.
            try dw.genVarDbgInfo(name, ty, self.owner.getDecl(zcu), is_ptr, loc);
        },
        .plan9 => {},
        .none => {},
    }
...
fn genCall(self: *Self, info: union(enum) {
    ...
    // on linking.
    switch (info) {
        .air => |callee| if (try self.air.value(callee, zcu)) |func_value| {
            const func_key = zcu.intern_pool.indexToKey(func_value.ip_index);
            switch (switch (func_key) {
                else => func_key,
                .ptr => |ptr| switch (ptr.addr) {
                    .decl => |decl| zcu.intern_pool.indexToKey(try zcu.declPtr(decl).internValue(zcu)),
                    else => func_key,
                },
            }) {
                .func => |func| {
                    try zcu.markDeclAlive(zcu.declPtr(func.owner_decl));
                ...
                .extern_func => |extern_func| {
                    ...
                    try zcu.markDeclAlive(owner_decl);
                    const lib_name = zcu.intern_pool.stringToSliceUnwrap(extern_func.lib_name);
                    const decl_name = zcu.intern_pool.stringToSlice(owner_decl.name);
...
fn airCmpLtErrorsLen(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const addr_lock = self.register_manager.lockRegAssumeUnused(addr_reg);
    defer self.register_manager.unlockReg(addr_lock);
    try self.genLazySymbolRef(.lea, addr_reg, link.File.LazySymbol.initDecl(.const_data, null, zcu));
...
fn moveStrategy(self: *Self, ty: Type, class: Register.Class, aligned: bool) !MoveStrategy {
    ...
    switch (class) {
        ...
        .sse => switch (ty.zigTypeTag(zcu)) {
                ...
                const classes = mem.sliceTo(&abi.classifySystemV(ty, zcu, .other), .none);
    ...
    return self.fail("TODO moveStrategy for {}", .{ty.fmt(zcu)});
}
    ...
    switch (src_mcv) {
        ...
        .register => |src_reg| if (dst_reg.id() != src_reg.id()) switch (dst_reg.class()) {
            ...
            .sse => switch (src_reg.class()) {
                ...
                .sse => try self.asmRegisterRegister(
                    ...
                    }) orelse return self.fail("TODO implement genSetReg for {}", .{ty.fmt(zcu)}),
                    registerAlias(dst_reg, abi_size),
                    registerAlias(src_reg, abi_size),
                ),
            },
        ...
        => try @as(MoveStrategy, switch (src_mcv) {
            ...
            .load_frame => |frame_addr| try self.moveStrategy(
                ty,
                dst_reg.class(),
                self.getFrameAddrAlignment(frame_addr).compare(.gte, ty.abiAlignment(zcu)),
            ),
    ...
    switch (src_mcv) {
        ...
        .register => |src_reg| {
            ...
            if (src_size > mem_size) {
                ...
                .none => ty.abiAlignment(zcu).check(@as(u32, @bitCast(disp))),
                .reg => |reg| switch (reg) {
                    .es, .cs, .ss, .ds => ty.abiAlignment(zcu).check(@as(u32, @bitCast(disp))),
                    else => false,
                },
                .frame => |frame_index| self.getFrameAddrAlignment(
                    .{ .index = frame_index, .off = disp },
                ).compare(.gte, ty.abiAlignment(zcu)),
                .reloc => false,
            })).write(
                self,
                .{ .base = base, .mod = .{ .rm = .{
...
fn airTagName(self: *Self, inst: Air.Inst.Index) !void {
    ...
    try self.genLazySymbolRef(
        .call,
        .rax,
        link.File.LazySymbol.initDecl(.code, enum_ty.getOwnerDecl(zcu), zcu),
    );
...
fn airSplat(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = result: {
        ...
        return self.fail("TODO implement airSplat for {}", .{vector_ty.fmt(zcu)});
    };
    return self.finishAir(inst, result, .{ ty_op.operand, .none, .none });
}
    ...
    const result: MCValue = result: {
        ...
        if (operand_ty.isVector(zcu) and operand_ty.childType(zcu).toIntern() == .bool_type) {
            ...
            const mask_len = (math.cast(u6, operand_ty.vectorLen(zcu)) orelse
                return self.fail("TODO implement airReduce for {}", .{operand_ty.fmt(zcu)}));
            ...
            switch (reduce.operation) {
                ...
                else => return self.fail("TODO implement airReduce for {}", .{operand_ty.fmt(zcu)}),
            }
        }
        return self.fail("TODO implement airReduce for {}", .{operand_ty.fmt(zcu)});
    };
        ...
        switch (result_ty.zigTypeTag(zcu)) {
            .Struct => {
                ...
                if (result_ty.containerLayout(zcu) == .Packed) {
                    const struct_type = zcu.typeToStruct(result_ty).?;
                    ...
                    for (elements, 0..) |elem, elem_i_usize| {
                        ...
                        const elem_off = zcu.structPackedFieldBitOffset(struct_type, elem_i);
            ...
            .Array, .Vector => {
                ...
                if (result_ty.isVector(zcu) and elem_ty.bitSize(zcu) == 1) {
...
fn airMulAdd(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result = result: {
        ...
        const mir_tag = @as(?Mir.Inst.FixedTag, if (mem.eql(u2, &order, &.{ 1, 3, 2 }) or
            ...
            unreachable) orelse return self.fail("TODO implement airMulAdd for {}", .{ty.fmt(zcu)});
...
fn airVaStart(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const ptr_anyopaque_ty = try zcu.singleMutPtrType(Type.anyopaque);
...
fn airVaArg(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = switch (abi.resolveCallingConvention(
        ...
        .SysV => result: {
            ...
            const classes = mem.sliceTo(&abi.classifySystemV(promote_ty, zcu, .arg), .none);
            switch (classes[0]) {
                .integer => {
...
fn genTypedValue(self: *Self, arg_tv: TypedValue) InnerError!MCValue {
    const zcu = self.bin_file.comp.zcu.?;
    return switch (try codegen.genTypedValue(self.bin_file, self.src_loc, arg_tv, self.owner.getDecl(zcu))) {
        .mcv => |mcv| switch (mcv) {
            .none => .none,
            .undef => .undef,
            .immediate => |imm| .{ .immediate = imm },
    ...
    switch (cc) {
        ...
        .C, .SysV, .Win64 => {
            ...
            if (ret_ty.zigTypeTag(zcu) == .NoReturn) {
                ...
                const classes = switch (resolved_cc) {
                    .SysV => mem.sliceTo(&abi.classifySystemV(ret_ty, zcu, .ret), .none),
            ...
            for (param_types, result.args) |ty, *arg| {
                ...
                const classes = switch (resolved_cc) {
                    .SysV => mem.sliceTo(&abi.classifySystemV(ty, zcu, .arg), .none),
                ...
                const param_align: u31 =
                    @intCast(@max(ty.abiAlignment(zcu).toByteUnitsOptional().?, 8));
        ...
        .Unspecified => {
            ...
            for (param_types, result.args) |ty, *arg| {
                ...
                const param_size: u31 = @intCast(ty.abiSize(zcu));
                const param_align: u31 = @intCast(ty.abiAlignment(zcu).toByteUnitsOptional().?);
                result.stack_byte_count =
                    mem.alignForward(u31, result.stack_byte_count, param_align);
                arg.* = .{ .load_frame = .{
                    .index = stack_frame_base,
    ...
    if (classes.len == 2) for (&parts, classes, 0..) |*part, class, part_i| {
        part.* = switch (class) {
            .integer => switch (part_i) {
                0 => Type.u64,
                1 => part: {
                    const elem_size = ty.abiAlignment(zcu).minStrict(.@"8").toByteUnitsOptional().?;
            ...
            .float => Type.f32,
            .float_combine => try zcu.vectorType(.{ .len = 2, .child = .f32_type }),
            .sse => Type.f64,
            else => break,
        };
...
fn regExtraBits(self: *Self, ty: Type) u64 {
    const zcu = self.bin_file.comp.zcu.?;
    return self.regBitSize(ty) - ty.bitSize(zcu);
}
