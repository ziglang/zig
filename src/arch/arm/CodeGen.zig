...
const DbgInfoReloc = struct {
    ...
    fn genArgDbgInfo(reloc: DbgInfoReloc, function: Self) error{OutOfMemory}!void {
        const zcu = function.bin_file.comp.zcu.?;
        switch (function.debug_output) {
            .dwarf => |dw| {
                ...
                try dw.genArgDbgInfo(reloc.name, reloc.ty, zcu.funcOwnerDeclIndex(function.func_index), loc);
            },
            .plan9 => {},
            .none => {},
        }
    ...
    fn genVarDbgInfo(reloc: DbgInfoReloc, function: Self) !void {
        ...
        switch (function.debug_output) {
            .dwarf => |dw| {
                ...
                try dw.genVarDbgInfo(reloc.name, reloc.ty, zcu.funcOwnerDeclIndex(function.func_index), is_ptr, loc);
            },
            .plan9 => {},
            .none => {},
        }
...
fn airWrapOptional(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = if (self.liveness.isUnused(inst)) .dead else result: {
        ...
        const abi_size: u32 = @intCast(optional_ty.abiSize(zcu));
...
fn arrayElemVal(
    ...
    switch (mcv) {
        ...
        => {
            ...
            const ptr_ty = try zcu.singleMutPtrType(elem_ty);
...
fn store(self: *Self, ptr: MCValue, value: MCValue, ptr_ty: Type, value_ty: Type) InnerError!void {
    const zcu = self.bin_file.comp.zcu.?;
    const elem_size: u32 = @intCast(value_ty.abiSize(zcu));
...
fn airArg(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const name = zcu.getParamName(self.func_index, src_index);
...
fn airCall(self: *Self, inst: Air.Inst.Index, modifier: std.builtin.CallModifier) !void {
    ...
    if (try self.air.value(callee, zcu)) |func_value| {
        if (func_value.getFunction(zcu)) |func| {
        ...
        } else if (func_value.getExternFunc(zcu)) |_| {
    ...
    switch (mcv) {
        ...
        => {
            ...
            if (abi_size <= 4) {
                ...
                const ptr_ty = try zcu.singleMutPtrType(ty);
...
fn airTry(self: *Self, inst: Air.Inst.Index) !void {
    ...
    const result: MCValue = result: {
        ...
        const zcu = self.bin_file.comp.zcu.?;
        const error_union_size: u32 = @intCast(error_union_ty.abiSize(zcu));
        const error_union_align = error_union_ty.abiAlignment(zcu);
...
fn resolveInst(self: *Self, inst: Air.Inst.Ref) InnerError!MCValue {
    ...
    const inst_index = inst.toIndex() orelse return self.genTypedValue(.{
        .ty = inst_ty,
        .val = (try self.air.value(inst, zcu)).?,
    });
