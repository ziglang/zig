...
test "instruction formatting" {
    try expectEqualStrings("addi_d r8 r0 10", std.fmt.comptimePrint("{}", .{Inst{ .opcode = .addi_d, .data = .{
        .DJSk12 = .{ .r8, .r0, 10 },
    } }}));
    try expectEqualStrings("fabs_d f1 f2", std.fmt.comptimePrint("{}", .{Inst{ .opcode = .fabs_d, .data = .{
        .DJ = .{ .f1, .f2 },
    } }}));
    try expectEqualStrings("break 0", std.fmt.comptimePrint("{}", .{Inst{ .opcode = .@"break", .data = .{
        .Ud15 = .{0},
    } }}));
    try expectEqualStrings("fcmp_caf_s fcc0 f1 f2", std.fmt.comptimePrint("{}", .{Inst{ .opcode = .fcmp_caf_s, .data = .{
        .DJK = .{ .fcc0, .f1, .f2 },
    } }}));
}
