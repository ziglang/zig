...
pub fn addDependencies(
    ...
    for (manifest.dependencies.keys(), 0..) |name, i| {
        ...
        const dep_hash = dep.hash orelse return error.MissingDependencyHash;
        const hex_digest = dep_hash[0..hex_multihash_len];
        const pkg_dir_sub_path = "p" ++ fs.path.sep_str ++ hex_digest;

        const build_root = try global_cache_directory.join(gpa, &.{pkg_dir_sub_path});
        errdefer gpa.free(build_root);

        var pkg_dir = global_cache_directory.handle.openDir(pkg_dir_sub_path, .{}) catch |err| switch (err) {
            error.FileNotFound => return error.MissingPackage,
            else => |e| return e,
        };
        ...
        try pkg.add(gpa, name, ptr);
        try root_pkg.add(gpa, fqn, ptr);
