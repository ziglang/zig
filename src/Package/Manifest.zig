    ...
    fn parseDependency(p: *Parse, node: Ast.Node.Index) !Dependency {
        ...
        for (struct_init.ast.fields) |field_init| {
            ...
            if (mem.eql(u8, field_name, "url")) {
                ...
                if (dep.no_unpack != null) {
                    return fail(p, ast.nodeMainToken(field_init), "dependency should specify only one 'no_unpack' field", .{});
                }
                dep.no_unpack = parseString(p, field_init) catch |err| switch (err) {
                    error.ParseFailure => continue,
                    else => |e| return e,
                };
                if (getNoUnpackProblem(dep.no_unpack.?)) |problem| {
                    return fail(p, ast.nodeMainToken(field_init), "no_unpack value {s}", .{problem});
                }
            } else {
                // Ignore unknown fields so that we can add fields in future zig
                // versions without breaking older zig versions.
