...
pub fn createBuiltin(arena: Allocator, opts: Builtin, global_cache_dir: Cache.Directory) Allocator.Error!*Module {
    const root_dir_sub_path = try arena.dupe(u8, "b" ++ std.fs.path.sep_str ++ Cache.binToHex(opts.hash()));
    const new = try arena.create(Module);
    new.* = .{
        .root = .{
            .root_dir = global_cache_dir,
            .sub_path = root_dir_sub_path,
        },
