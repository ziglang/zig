...
/// TODO: how do we handle case-insensitive filesystems?
pub const Path = struct {
    ...
    pub fn fromUnresolved(
        gpa: Allocator,
        dirs: Compilation.Directories,
        /// This function does not takes ownership of `unresolved`.
        unresolved: []const u8,
    ) Allocator.Error!Path {
        const resolved = try introspect.resolvePath(gpa, dirs.cwd, &.{unresolved});
        return .fromResolved(gpa, dirs, resolved);
    }

    /// From a resolved path (`introspect.resolvePath`), construct a canonical `Path`.
    pub fn fromResolved(
        gpa: Allocator,
        dirs: Compilation.Directories,
        /// The path to convert. Must be from `resolvePath`.
        /// This function takes ownership of `resolved`, even on error.
        resolved: []u8,
    ) Allocator.Error!Path {
        errdefer gpa.free(resolved);

        // If, for instance, `dirs.local_cache.path` is within the lib dir, it must take priority,
        // so that we prefer `.root = .local_cache_relative` over `.root = .zig_lib_relative`. The
        // easiest way to do this is simply to prioritize the longest root.
        const PathAndRoot = struct { []const u8, Root };
        var roots: [3]PathAndRoot = .{
            .{ dirs.zig_lib.path orelse "", .zig_lib },
            .{ dirs.global_cache.path orelse "", .global_cache },
            .{ dirs.local_cache.path orelse "", .local_cache },
        };
        std.mem.sort(PathAndRoot, &roots, {}, struct {
            fn lessThan(_: void, lhs: PathAndRoot, rhs: PathAndRoot) bool {
                return lhs[0].len > rhs[0].len; // '>' instead of '<' to sort descending
            }
        }.lessThan);

        for (roots) |path_and_root| {
            const root_path = path_and_root[0];
            if (std.mem.eql(u8, root_path, "")) {
                // This root is the cwd.
                if (!fs.path.isAbsolute(resolved)) {
                    return .{
                        .root = path_and_root[1],
                        .sub_path = resolved,
                    };
                }
                continue;
            ...
            gpa.free(resolved);
            return .{ .root = path_and_root[1], .sub_path = duped };
        }

        // We're not relative to any root, so we will use an absolute path (on targets where they are available).

        if (std.mem.eql(u8, dirs.cwd, "")) {
            // This is a special case to avoid a leading slash.
            assert(builtin.target.os.tag == .wasi);
            return .{
                .root = .none,
                .sub_path = resolved,
            };
        ...
        if (std.mem.eql(u8, resolved, "")) {
            // This is a special case to avoid a trailing slash.
            return .{
                .root = .none,
                .sub_path = try gpa.dupe(u8, dirs.cwd),
            };
        }

        if (fs.path.isAbsolute(resolved)) {
            // `resolved` is already absolute
            return .{
                .root = .none,
                .sub_path = resolved,
            };
    ...
    pub fn fromRoot(
        ...
        // TODO: better implementation! this sucks!!!! just cache prefixes which move between roots and check them!!!!!!!!!!!!!
        // we can't just return .{ .root = root, .sub_path = something }, because `relative_unresolved` might take us into a
        // "preferred" root (i.e. a more deeply nested one).
        // this is actually also a potentially invalid Path, but join doesn't care right now.
        // fix this terrible code pls matthew
        const base: Path = .{ .root = root, .sub_path = "" };
        return base.join(gpa, dirs, parts);
    }

    /// Given a `Path` and a (unresolved) sub path relative to it, construct a `Path` representing the joined path.
    pub fn join(
        p: Path,
        gpa: Allocator,
        dirs: Compilation.Directories,
        ...
        // TODO: better implementation! this sucks!!!! just cache prefixes which move between roots and check them!!!!!!!!!!!!!
        const p_abs = try p.toAbsolute(dirs, gpa);
        defer gpa.free(p_abs);
        const all_parts = try gpa.alloc([]const u8, parts.len + 1);
        defer gpa.free(all_parts);
        all_parts[0] = p_abs;
        @memcpy(all_parts[1..], parts);
        const resolved = try introspect.resolvePath(gpa, dirs.cwd, all_parts);
        // No `errdefer` because `fromResolved` takes ownership of `resolved`.
        return fromResolved(gpa, dirs, resolved);
    }
