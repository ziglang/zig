...
fn cIsPointer(c: *Context, stmt: *const clang.Stmt) bool {
    switch (stmt.getStmtClass()) {
        .ImplicitCastExprClass => {
            const implicitCast = @ptrCast(*const clang.ImplicitCastExpr, stmt);
            const dest_type = getExprQualType(c, @ptrCast(*const clang.Expr, implicitCast));
            return dest_type.getTypePtr().getTypeClass() == .Pointer;
        },
        .BinaryOperatorClass => {
            const bin_op = @ptrCast(*const clang.BinaryOperator, stmt);
            if (bin_op.getOpcode() == .Add) {
                const lhs = @ptrCast(*const clang.Stmt, bin_op.getLHS());
                const rhs = @ptrCast(*const clang.Stmt, bin_op.getRHS());
                return cIsPointer(c, lhs) or cIsPointer(c, rhs);
            }
            return false;
        },
        .UnaryOperatorClass => {
            const unary_op = @ptrCast(*const clang.UnaryOperator, stmt);
            switch (unary_op.getOpcode()) {
                .PostInc, .PostDec, .PreInc, .PreDec => {
                    const op_expr = @ptrCast(*const clang.Stmt, unary_op.getSubExpr());
                    return cIsPointer(c, op_expr);
                },
                .AddrOf => return true,
                else => return false,
            }
        ...
        .ParenExprClass => {
            const op_expr = @ptrCast(*const clang.ParenExpr, stmt).getSubExpr();
            return cIsPointer(c, @ptrCast(*const clang.Stmt, op_expr));
        },
        .DeclRefExprClass => {
            const decl_ref = @ptrCast(*const clang.DeclRefExpr, stmt);
            const value_decl = decl_ref.getDecl();
            const qt = value_decl.getType();
            const ty = qt.getTypePtr();
            return ty.getTypeClass() == .Pointer;
        },
        else => return false,
    }
}
...
fn cIsPointerDiffExpr(c: *Context, stmt: *const clang.BinaryOperator) bool {
    ...
    return stmt.getOpcode() == .Sub and cIsPointer(c, lhs) and cIsPointer(c, rhs);
}
