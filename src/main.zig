...
const RuntimeSafety = struct {
    const no_checks: u64 = 0;
    const check_all: u64 = 0x5555555555555555;
    const extra_all: u64 = 0xaaaaaaaaaaaaaaaa;
    fn setAll(
        ptr: *std.builtin.RuntimeSafety,
        setting: std.builtin.RuntimeSafety.Setting,
    ) void {
        ptr.* = switch (setting) {
            .none => @bitCast(no_checks),
            .check => @bitCast(check_all),
            .extra => @bitCast(extra_all),
        };
    }
    fn parseSafety(args_iter: *ArgsIterator, string: []const u8, arg: []const u8) ?std.builtin.RuntimeSafety.Setting {
        if (mem.eql(u8, arg, string)) {
            const next_arg = args_iter.next() orelse {
                fatal("expected [none|check|extra] after {s}", .{string});
            };
            return std.meta.stringToEnum(std.builtin.RuntimeSafety.Setting, next_arg) orelse {
                fatal("expected [none|check|extra] after {s}, found '{s}'", .{ string, next_arg });
            };
        }
        return null;
    }
    fn parseAllSafety(args_iter: *ArgsIterator, arg: []const u8) bool {
        if (parseSafety(args_iter, "-fruntime-safety", arg)) |tag| {
            setAll(&Package.Module.runtime_safety, tag);
        } else if (parseSafety(args_iter, "-funwrapped-error", arg)) |tag| {
            Package.Module.runtime_safety.unwrapped_error = tag;
        } else if (parseSafety(args_iter, "-faccessed-null-value", arg)) |tag| {
            Package.Module.runtime_safety.accessed_null_value = tag;
        } else if (parseSafety(args_iter, "-freturned-noreturn", arg)) |tag| {
            Package.Module.runtime_safety.returned_noreturn = tag;
        } else if (parseSafety(args_iter, "-freached-unreachable", arg)) |tag| {
            Package.Module.runtime_safety.reached_unreachable = tag;
        } else if (parseSafety(args_iter, "-faccessed-out-of-bounds", arg)) |tag| {
            Package.Module.runtime_safety.accessed_out_of_bounds = tag;
        } else if (parseSafety(args_iter, "-faccessed-out-of-order", arg)) |tag| {
            Package.Module.runtime_safety.accessed_out_of_order = tag;
        } else if (parseSafety(args_iter, "-faccessed-inactive-field", arg)) |tag| {
            Package.Module.runtime_safety.accessed_inactive_field = tag;
        } else if (parseSafety(args_iter, "-fdivided-by-zero", arg)) |tag| {
            Package.Module.runtime_safety.divided_by_zero = tag;
        } else if (parseSafety(args_iter, "-fmemcpy-argument-aliasing", arg)) |tag| {
            Package.Module.runtime_safety.memcpy_argument_aliasing = tag;
        } else if (parseSafety(args_iter, "-fmismatched-memcpy-argument-lengths", arg)) |tag| {
            Package.Module.runtime_safety.mismatched_memcpy_argument_lengths = tag;
        } else if (parseSafety(args_iter, "-fmismatched-for-loop-capture-lengths", arg)) |tag| {
            Package.Module.runtime_safety.mismatched_for_loop_capture_lengths = tag;
        } else if (parseSafety(args_iter, "-fmismatched-sentinel", arg)) |tag| {
            Package.Module.runtime_safety.mismatched_sentinel = tag;
        } else if (parseSafety(args_iter, "-fshift-amt-overflowed", arg)) |tag| {
            Package.Module.runtime_safety.shift_amt_overflowed = tag;
        } else if (parseSafety(args_iter, "-farith-overflowed", arg)) |tag| {
            Package.Module.runtime_safety.mul_overflowed = tag;
            Package.Module.runtime_safety.add_overflowed = tag;
            Package.Module.runtime_safety.sub_overflowed = tag;
            Package.Module.runtime_safety.dec_overflowed = tag;
            Package.Module.runtime_safety.inc_overflowed = tag;
            Package.Module.runtime_safety.div_overflowed = tag;
            Package.Module.runtime_safety.div_with_remainder = tag;
        } else if (parseSafety(args_iter, "-fcast-truncated-data", arg)) |tag| {
            Package.Module.runtime_safety.cast_truncated_data = tag;
        } else if (parseSafety(args_iter, "-fcast-to-enum-from-invalid", arg)) |tag| {
            Package.Module.runtime_safety.cast_to_enum_from_invalid = tag;
        } else if (parseSafety(args_iter, "-fcast-to-error-from-invalid", arg)) |tag| {
            Package.Module.runtime_safety.cast_to_error_from_invalid = tag;
        } else if (parseSafety(args_iter, "-fcast-to-ptr-from-invalid", arg)) |tag| {
            Package.Module.runtime_safety.cast_to_ptr_from_invalid = tag;
        } else if (parseSafety(args_iter, "-fcast-to-int-from-invalid", arg)) |tag| {
            Package.Module.runtime_safety.cast_to_int_from_invalid = tag;
        } else if (parseSafety(args_iter, "-fcast-to-unsigned-from-negative", arg)) |tag| {
            Package.Module.runtime_safety.cast_to_unsigned_from_negative = tag;
        } else if (mem.eql(u8, "-fanalyze-slice2", arg)) {
            std.builtin.RuntimeSafety.analyze_slice2 = true;
        } else if (mem.eql(u8, "-fno-analyze-slice2", arg)) {
            std.builtin.RuntimeSafety.analyze_slice2 = false;
        } else {
            return false;
        }
        return true;
