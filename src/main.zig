...
fn cmdInit(gpa: Allocator, arena: Allocator, args: []const []const u8) !void {
    ...
    var templates = std.zig.introspect.findTemplates(gpa, arena);
...
fn cmdBuild(gpa: Allocator, arena: Allocator, args: []const []const u8) !void {
    ...
    const cwd_path = try process.getCwdAlloc(arena);
    const build_root = try std.zig.introspect.findBuildRoot(arena, .{
        .cwd_path = cwd_path,
        ...
        // big block here to ensure the cleanup gets run when we extract out our argv.
        {
            ...
            if (build_options.only_core_functionality) {
...

                {
                    ...
                    for (fetches) |f| {
                        const mod: *Package.Module = @ptrCast(@alignCast(f.module orelse continue));
                        ...
                        for (dep_names, man.dependencies.values()) |name, dep| {
                            ...
                            const dep_mod: *Package.Module = @ptrCast(@alignCast(job_queue.table.get(dep_digest).?.module orelse continue));
