...
pub const Tag = enum(u8) {
    ...
    simple_type,
    /// An spirv type.
    /// data is index of Tag.TypeSpirv in extra.
    type_spirv,
    /// An opaque type.
    ...
    const encodings = .{
        ...
        .type_spirv = .{ .summary = .@"spirv_type{%summary#\"}", .payload = Tag.TypeSpirv },
        .type_opaque = .{
            .summary = .@"{.payload.name%summary#\"}",
            .payload = TypeOpaque,
    ...
    pub const TypeSpirv = struct {
        name: NullTerminatedString,
        /// The index of the `reify_spirv` instruction.
        zir_index: TrackedInst.Index,
        ...
        pub const Flags = packed struct(u32) {
            tag: @typeInfo(std.builtin.SpirvType).@"union".tag_type.?,
            // Image type flags
            usage: @typeInfo(std.builtin.SpirvType.Image.Usage).@"union".tag_type.? = undefined,
            format: std.builtin.SpirvType.Image.Format = undefined,
            dim: std.builtin.SpirvType.Image.Dimensionality = undefined,
            depth: std.builtin.SpirvType.Image.Depth = undefined,
            access: std.builtin.SpirvType.Image.Access = undefined,
            is_arrayed: bool = undefined,
            is_multisampled: bool = undefined,
...
pub fn get(ip: *InternPool, gpa: Allocator, tid: Zcu.PerThread.Id, key: Key) Allocator.Error!Index {
    ...
    switch (key) {
        ...
        .spirv_type => unreachable, // use getSpirvType() instead
...
pub const SpirvTypeInit = struct {
    type_hash: u64,
...
pub fn getSpirvType(
    ...
    var gop = try ip.getOrPutKey(gpa, tid, .{ .spirv_type = .{
        .type_hash = ini.type_hash,
        .zir_index = ini.type_spirv.zir_index,
    } });
    defer gop.deinit();
    if (gop == .existing) return gop.existing;
    ...
    try extra.ensureUnusedCapacity(@typeInfo(Tag.TypeSpirv).@"struct".fields.len +
