...
fn dumpStatusReport() !void {
    ...
    const file, const src_base_node = Zcu.LazySrcLoc.resolveBaseNode(block.src_base_inst, zcu) orelse {
        ...
        try stderr.print("Analyzing lost instruction in file '{s}'. This should not happen!\n\n", .{file.displayPath(zcu)});
        return;
    };

    try stderr.writeAll("Analyzing ");
    try stderr.print("Analyzing '{s}'\n", .{file.displayPath(zcu)});
    ...
    while (parent) |curr| {
        ...
        try stderr.print("  in {s}\n", .{cur_block_file.displayPath(zcu)});
