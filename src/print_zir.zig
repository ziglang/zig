...
const Writer = struct {
    ...
    fn writeUnNode(
        ...
        try self.writeInstRef(stream, inst_data.operand);
        try stream.writeAll(") ");
        try self.writeSrc(stream, LazySrcLoc.fromUnNode(inst_data));
    }
    ...
    fn writeUnTok(
        ...
        try self.writeInstRef(stream, inst_data.operand);
        try stream.writeAll(") ");
        try self.writeSrc(stream, LazySrcLoc.fromUnTok(inst_data));
    }
    ...
    fn writeValidateDestructure(
        ...
        try stream.writeAll(") ");
        try self.writeSrc(stream, LazySrcLoc.fromPlNode(inst_data));
    }
    ...
    fn writeFloat128(self: *Writer, stream: anytype, inst: Zir.Inst.Index) !void {
        ...
        const src = LazySrcLoc.fromPlNode(inst_data);
    ...
    fn writeParam(self: *Writer, stream: anytype, inst: Zir.Inst.Index) !void {
        ...
        try stream.writeAll(") ");
        try self.writeSrc(stream, LazySrcLoc.fromPlTok(inst_data));
    }
    ...
    fn writeInstNode(
        ...
        try stream.writeAll(") ");
        try self.writeSrc(stream, LazySrcLoc.fromInstNode(inst_data));
    }
    ...
    fn writeDecls(self: *Writer, stream: anytype, decls_len: u32, extra_start: usize) !usize {
        ...
        while (decl_i < decls_len) : (decl_i += 1) {
            ...
            if (self.recurse_decls) {
                ...
                try self.writeSrc(stream, LazySrcLoc.fromPlNode(decl_block_inst_data));
                try stream.writeAll("\n");
            } else {
    ...
    fn writeStrTok(
        ...
        const str = inst_data.get(self.code);
        try stream.print("\"{}\") ", .{std.zig.fmtEscapes(str)});
        try self.writeSrc(stream, LazySrcLoc.fromStrTok(inst_data));
    }
    ...
    fn writeIntType(self: *Writer, stream: anytype, inst: Zir.Inst.Index) !void {
        ...
        try self.writeSrc(stream, LazySrcLoc.fromIntType(int_type));
    }
    ...
    fn writeUnreachable(self: *Writer, stream: anytype, inst: Zir.Inst.Index) !void {
        ...
        try stream.writeAll(") ");
        try self.writeSrc(stream, LazySrcLoc.fromUnreachable(inst_data));
    }
