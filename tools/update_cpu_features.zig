...
pub fn main() anyerror!void {
    ...
    updateX86CpuModelEnums(arena, zig_exe, zig_src_dir, llvm_src_root);
...
fn updateX86CpuModelEnums(
    ...
    var out_file = try zig_src_dir.createFile("lib/std/zig/system/x86/enums.zig", .{});
...
fn writeEnum(file: fs.File, enum_name: []const u8, enum_field: []const EnumField) !void {
    ...
    for (enum_field) |f| {
        ...
        if (std.ascii.isAlphabetic(f.name[0]) or f.name[0] == '_') {
