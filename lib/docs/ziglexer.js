                ...
                case State.start:
                    switch (c) {
                        ...
                        case '?': {
                            ...
                            this.index += 1; result.loc.end = index;
                            return result;
...
function renderSingleToken(t, needHtmlWs) { 

    if (t.tag == Tag.whitespace) {
        if (needHtmlWs) {
            return escapeHtmlWs(t.src)
        }
        return t.src;
    }
...
function renderTokens(tok_it, needHtmlWs) {
    var html = [];
    ...
    for (const t of tok_it) {
        ...
        html.push(renderSingleToken(t, needHtmlWs));
    }
