...
`);
  ...
  function renderDocTest(decl) {
    ...
    domDocTestsCode.innerHTML = generate_html_for_src(
  ...
  function* ex(expr, opts) {
    switch (Object.keys(expr)[0]) {
      ...
      case "declRef": {
        ...
        const link = findDeclNavLink(name); 
        yield { src: name, tag: Tag.identifier, link };
        return;
      }
      case "refPath": {
        let firstComponent = expr.refPath[0];
        yield * ex(firstComponent, opts);
        for (let i = 1; i < expr.refPath.length; i += 1) {
          yield Tok.period;
          // very simplified
          // TODO: Make this link again
      ...
      case "type": {
        ...
        switch (typeObj.kind) {
          ...
          case typeKinds.Struct: {
            ...
            let field_end = [Tok.comma];

            if (structObj.field_types.length > 1) {
              field_end.push(Tok.enter);
            } else {
              field_end.push(Tok.space);
            }
  ...
  function exprName(expr, opts) {
    switch (Object.keys(expr)[0]) {
      ...
      case "comptimeExpr": {
        const src = zigAnalysis.comptimeExprs[expr.comptimeExpr].code;
        return generate_html_for_src(DecoratedTokenizer(src));
      }
      case "call": {
  ...
  function renderValue(decl) {
    ...
    if (resolvedValue.expr.fieldRef) {
      ...
      domFnProtoCode.innerHTML = generate_html_for_src(
        (function*(){
          yield Tok.const;
          yield Tok.space;
          yield { src: decl.name, tag: Tag.identifier };
          yield Tok.colon;
          yield Tok.space;
          yield { src: type.name, tag: Tag.identifier };
          yield Tok.space;
          yield Tok.eql;
          yield Tok.space;
        ...
        const unused = '<span class="tok-kw">const</span> ' +
      ...
      domFnProtoCode.innerHTML = generate_html_for_src(
        (function*(){
          ...
          yield Tok.colon;
          yield Tok.space;
          yield * ex(typeRef !== null ? typeRef : resolvedValue.expr, {});
          yield Tok.space;
          yield Tok.eql;
          yield Tok.space;
          yield * ex(decl.value.expr, {});
