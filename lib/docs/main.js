...
(function() {
  ...
  function resolveValue(value, trackDecls) {
    ...
    while (true) {
      ...
      if ("import" in value.expr) {
        value = {
          expr: zigAnalysis.exprs[value.expr.import.value]
        };
        continue;
      }
...
`);
  ...
  function* ex(expr, opts) {
    switch (Object.keys(expr)[0]) {
      ...
      case "import": {
        yield { src: "@import", tag: Tag.builtin };
        yield Tok.l_paren;
        yield { src: '"' + expr.import.path + '"', tag: Tag.string_literal };
        yield Tok.r_paren;
        return;
      }
