(function() {
    ...
    WebAssembly.instantiateStreaming(wasm_promise, {
      ...
      sources_promise.then(function(buffer) {
        ...
        // If the user has entered a search term while the WASM is still loading,
        // we will display the search results instead of navigating to the url hash.
        const current_search_value = domSearch.value;
        if (current_search_value != "") {
          curNavSearch = current_search_value;
          render();
        } else {
          onHashChange(null);
        }
