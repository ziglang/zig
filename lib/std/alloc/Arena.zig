...
/// Inner state of Allocator. Can be stored rather than the entire Allocator
/// as a memory-saving optimization.
pub const State = struct {
    buffer_list: std.SinglyLinkedList(usize) = .{},
    end_index: usize = 0,

    pub fn promote(self: State, child_allocator: mem.Allocator) Arena {
        return .{
...
pub fn init(child_allocator: mem.Allocator) Arena {
...
pub fn reset(self: *Arena, mode: ResetMode) bool {
...
fn createNode(self: *Arena, prev_len: usize, minimum_size: usize) ?*BufNode {
...
test "reset with preheating" {
    var arena_allocator = init(std.testing.allocator);
