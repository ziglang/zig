...
pub fn Writer(comptime WriterType: type) type {
    return struct {
        const file_default_mode = 0o664;
        const dir_default_mode = 0o775;
        const sym_link_default_mode = 0o777;
        ...
        pub fn directory(self: *Self, sub_path: []const u8, opt: Options) !void {
            ...
            try self.setName(&header, sub_path);
            try header.setMode(if (opt.mode == 0) dir_default_mode else opt.mode);
            try header.setMtime(if (opt.mtime == 0) @intCast(std.time.timestamp()) else opt.mtime);
        ...
        pub fn file(self: *Self, sub_path: []const u8, size: usize, reader: anytype, opt: Options) !void {
            ...
            try header.setMode(if (opt.mode == 0) file_default_mode else opt.mode);
            try header.setMtime(if (opt.mtime == 0) @intCast(std.time.timestamp()) else opt.mtime);
            try header.write(self.underlying_writer);
        ...
        fn setName(self: *Self, header: *output.Header, sub_path: []const u8) !void {
            header.setName(self.prefix, sub_path) catch |err| switch (err) {
                error.NameTooLong => {
        ...
        fn writeExtendedHeader(self: *Self, kind: output.Header.FileType, buffers: []const []const u8) !void {
            var len: usize = 0;
            for (buffers) |buf|
                len += buf.len;
        ...
        fn addPadding(self: *Self, bytes: usize) !void {
            const padding_bytes = block_size - (bytes % block_size);
        ...
        pub fn symLink(self: *Self, sub_path: []const u8, link_name: []const u8, opt: Options) !void {
            ...
            try header.setMode(if (opt.mode == 0) sym_link_default_mode else opt.mode);
            try header.setMtime(if (opt.mtime == 0) @intCast(std.time.timestamp()) else opt.mtime);
            try header.write(self.underlying_writer);
        }
        ...
        pub fn close(self: *Self) !void {
            try self.underlying_writer.writeAll(&output.zero_header);
            try self.underlying_writer.writeAll(&output.zero_header);
        }
    };
}
...
test "writer" {
    ...
    var w = try writer(file.writer(), "root");
    const buf: [1023]u8 = undefined;
    var fbs = std.io.fixedBufferStream(&buf);
    try w.file("file1", 1023, fbs.reader(), .{});
    try w.directory("01223456789" ** 9, .{});
    try w.directory("dir", .{});
    try w.symLink("dir/sym_link", "../file1", .{});
    try w.close();
}

test "walk" {
    const gpa = testing.allocator;
    var dir = try std.fs.cwd().openDir("/home/ianic/Code/zig", .{ .iterate = true });
    //var dir = try std.fs.cwd().openDir("/home/ianic/Code/tmp/root", .{ .iterate = true });

    var out_dir = try std.fs.cwd().openDir("/home/ianic/Code/tmp", .{});
    var out = try out_dir.createFile("zig.tar.gz", .{});
    defer out.close();

    var gz_out = try std.compress.gzip.compressor(out.writer(), .{});
    defer gz_out.finish() catch {};

    var tar_writer = try writer(gz_out.writer(), "zig34567890/" ** 10 ++ "root");
    defer tar_writer.close() catch {};

    var walker = try dir.walk(gpa);
    defer walker.deinit();
    while (try walker.next()) |entry| {
        if (std.mem.indexOf(u8, entry.path, "zig-cache")) |_| {
            continue;
        }
        if (std.mem.indexOf(u8, entry.path, "build/")) |_| {
            continue;
        }
        if (std.mem.indexOf(u8, entry.path, "build2/")) |_| {
            continue;
        }
        if (std.mem.indexOf(u8, entry.path, ".git/")) |_| {
            continue;
        }
        try tar_writer.walkerEntry(entry);
        // //std.debug.print("{s}\n", .{entry.path});
        // switch (entry.kind) {
        //     .directory => {
        //         try tar_writer.directory(entry.path);
        //     },
        //     .sym_link => {
        //         unreachable;
        //         //try tar_writer.symLink(entry.path, "todo");
        //     },
        //     .file => {
        //         var file = try entry.dir.openFile(entry.basename, .{});
        //         defer file.close();
        //         tar_writer.file(entry.path, (try file.stat()).size, file.reader()) catch |err| switch (err) {
        //             error.NameTooLong => {
        //                 std.debug.print("name too long: {s}\n", .{entry.path});
        //                 continue;
        //             },
        //             else => return err,
        //         };
