...
pub fn PriorityDequeue(comptime T: type, comptime Context: type, comptime compareFn: fn (context: Context, a: T, b: T) Order) type {
    return struct {
        ...
        fn formatHelperRecursive(
            ...
            const left_padding = try formatHelperRecursive(fmt, items, writer, level_to_print, left_child_index, level + 1, element_len);
            const right_padding = try formatHelperRecursive(fmt, items, writer, level_to_print, left_child_index +| 1, level + 1, end_padding);
...
test "std.PriorityDequeue: format makes a pretty picture" {
    var depq = PriorityDequeue(u8, void, struct {
    ...
    try depq.format("{c}", .{}, stream.writer());

    const formatted_depq =
        "       a       \n" ++
        "   ~       s   \n" ++
        " f   e   d   b \n" ++
        "h g i n e i c l\n";

    try testing.expectEqualStrings(formatted_depq, &buffer);
}
