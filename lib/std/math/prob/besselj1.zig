...
test "besselj1" {
    ...
    for (cases) |c| {
        try expectApproxEqRel(besselj1(c[0]), c[1], epsilon);
    }
}
...
pub fn bessely1(x: f64) f64 {
    ...
    if (x <= 5.0) {
        ...
        w += C.TWOOPI * (besselj1(x) * @log(x) - 1.0 / x);
        return w;
    }
...
test "y1" {
    ...
    try expect(math.isNegativeInf(bessely1(0)));

    for (cases) |c| {
        try expectApproxEqRel(bessely1(c[0]), c[1], epsilon);
    }
}
