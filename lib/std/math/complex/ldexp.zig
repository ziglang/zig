...
pub fn ldexp(z: anytype, expt: i32) Complex(@TypeOf(z.re, z.im)) {
    ...
    return switch (T) {
        f32 => ldexp32(z, expt),
        f64 => ldexp64(z, expt),
        else => @compileError("ldexp not implemented for " ++ @typeName(z)),
    };
}
...
fn ldexp32(z: Complex(f32), expt: i32) Complex(f32) {
    ...
    const exp_x = frexp32(z.re, &ex_expt);
...
fn ldexp64(z: Complex(f64), expt: i32) Complex(f64) {
    ...
    const exp_x = frexp64(z.re, &ex_expt);
...
test ldexp32 {
    ...
    try testing.expectApproxEqAbs(20.0, b.re, epsilon);
    try testing.expectApproxEqAbs(12.0, b.im, epsilon);
}
