...
pub fn tanh(z: anytype) Complex(@TypeOf(z.re, z.im)) {
    ...
    return switch (T) {
        f32 => tanh32(z),
        f64 => tanh64(z),
        else => @compileError("tanh not implemented for " ++ @typeName(z)),
    };
}
...
fn tanh32(z: Complex(f32)) Complex(f32) {
    ...
    if (ix >= 0x7f800000) {
        ...
        const xx: f32 = @bitCast(hx - 0x40000000);
        const r =
            if (math.isInf(y))
                y
            else
...
test tanh32 {
    ...
    try testing.expectApproxEqAbs(0.99991274, b.re, epsilon);
    try testing.expectApproxEqAbs(-0.00002536878, b.im, epsilon);
}
...
test tanh64 {
    ...
    try testing.expectApproxEqAbs(0.9999128201513536, b.re, epsilon);
    try testing.expectApproxEqAbs(-0.00002536867620767604, b.im, epsilon);
}
...
test "tanh64 musl" {
    ...
    try testing.expectApproxEqAbs(1, b.re, epsilon);
    try testing.expectApproxEqAbs(0, b.im, epsilon);
}
