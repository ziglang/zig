...
pub fn hypot(x: anytype, y: anytype) Hypot(@TypeOf(x, y)) {
    ...
    if (T == f16) return @floatCast(@sqrt(@mulAdd(f32, x, x, y * y)));
    if (T == f32) return @floatCast(@sqrt(@mulAdd(f64, x, x, y * y)));
    major = @abs(major);
    minor = @abs(minor);
    if (minor > major) {
        const tempo = major;
...
test hypot {
    inline for (.{ f32, f64 }) |T| {
        inline for ([7][3]T{
            ...
            try expect(math.approxEqRel(T, hypot(v[0], v[1]), v[2], floatEps(T)));
        }
