...
fn enumDeclInner(
    ...
    _ = try astgen.scanContainer(&namespace, members, .@"enum");
    ...
    astgen.src_hasher = std.zig.SrcHasher.init(.{});
    if (arg_node != 0) {
        astgen.src_hasher.update(tree.getNodeSource(arg_node));
    }
...
fn containerDecl(
    ...
    switch (token_tags[container_decl.ast.main_token]) {
        ...
        .keyword_enum => {
            ...
            const result = try enumDeclInner(gz, scope, node, container_decl.ast.members, container_decl.ast.arg);
            return rvalue(gz, ri, result, node);
        },
        .keyword_opaque => {
