...
fn expr(gz: *GenZir, scope: *Scope, ri: ResultInfo, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    switch (tree.nodeTag(node)) {
        ...
        // zig fmt: off
        .shl => return shiftOp(gz, scope, ri, node, tree.nodeData(node).lhs.node, tree.nodeData(node).rhs.node, .shl),
        .shr => return shiftOp(gz, scope, ri, node, tree.nodeData(node).lhs.node, tree.nodeData(node).rhs.node, .shr),
        ...
        .array_mult => {
            ...
            const result = try gz.addPlNode(.array_mul, node, Zir.Inst.ArrayMul{
                ...
                .lhs = try expr(gz, scope, .{ .rl = .none }, tree.nodeData(node).lhs.node),
                .rhs = try comptimeExpr(gz, scope, .{ .rl = .{ .coerced_ty = .usize_type } }, tree.nodeData(node).rhs.node, .array_mul_factor),
            });
        ...
        .error_union, .merge_error_sets => |tag| {
            ...
            const lhs = try reachableTypeExpr(gz, scope, tree.nodeData(node).lhs.node, node);
            const rhs = try reachableTypeExpr(gz, scope, tree.nodeData(node).rhs.node, node);
        ...
        .bool_not => return simpleUnOp(gz, scope, ri, node, coerced_bool_ri, tree.nodeData(node).lhs.node, .bool_not),
        .bit_not  => return simpleUnOp(gz, scope, ri, node, .{ .rl = .none }, tree.nodeData(node).lhs.node, .bit_not),

        .negation      => return   negation(gz, scope, ri, node),
        .negation_wrap => return simpleUnOp(gz, scope, ri, node, .{ .rl = .none }, tree.nodeData(node).lhs.node, .negate_wrap),
        ...
        .deref => {
            const lhs = try expr(gz, scope, .{ .rl = .none }, tree.nodeData(node).lhs.node);
        ...
        .address_of => {
            ...
            const result = try expr(gz, scope, .{ .rl = operand_rl }, tree.nodeData(node).lhs.node);
            return rvalue(gz, ri, result, node);
        },
        .optional_type => {
            const operand = try typeExpr(gz, scope, tree.nodeData(node).lhs.node);
            const result = try gz.addUnNode(.optional_type, operand, node);
            return rvalue(gz, ri, result, node);
        },
        .unwrap_optional => switch (ri.rl) {
            .ref, .ref_coerced_ty => {
                const lhs = try expr(gz, scope, .{ .rl = .ref }, tree.nodeData(node).lhs.node);
        ...
        .error_value => return simpleStrTok(gz, ri, tree.nodeData(node).rhs.opt_token.unwrap().?, node, .error_value),
        ...
        .anyframe_type => {
            const return_type = try typeExpr(gz, scope, tree.nodeData(node).rhs.node);
        ...
        .@"catch" => {
            ...
            no_switch_on_err: {
                ...
                const full_switch = tree.fullSwitch(tree.nodeData(node).rhs.node) orelse break :no_switch_on_err;
                if (full_switch.label_token != null) break :no_switch_on_err;
                if (tree.nodeTag(full_switch.ast.condition) != .identifier) break :no_switch_on_err;
            ...
            switch (ri.rl) {
                .ref, .ref_coerced_ty => return orelseCatchExpr(
                    ...
                    ri,
                    node,
                    tree.nodeData(node).lhs.node,
                    .is_non_err_ptr,
                    .err_union_payload_unsafe_ptr,
                    .err_union_code_ptr,
                    tree.nodeData(node).rhs.node,
                    payload_token,
                ),
        ...
        .grouped_expression => return expr(gz, scope, ri, tree.nodeData(node).lhs.node),
        ...
        .@"try" => return tryExpr(gz, scope, ri, node, tree.nodeData(node).lhs.node),
...
fn nosuspendExpr(
    ...
    const astgen = gz.astgen;
    const tree = astgen.tree;
    const body_node = tree.nodeData(node).lhs.node;
...
fn awaitExpr(
    ...
    const astgen = gz.astgen;
    const tree = astgen.tree;
    const rhs_node = tree.nodeData(node).lhs.node;
...
fn breakExpr(parent_gz: *GenZir, parent_scope: *Scope, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    const tree = astgen.tree;
    const opt_break_label = tree.nodeData(node).lhs.opt_token;
    const opt_rhs = tree.nodeData(node).rhs.opt_node;
    ...
    for (statements, 0..) |statement, stmt_idx| {
        ...
        while (true) {
            switch (tree.nodeTag(inner_node)) {
                ...
                .grouped_expression => {
                    inner_node = tree.nodeData(statement).lhs.node;
                    continue;
                },
...
fn deferStmt(
    ...
    const tree = gz.astgen.tree;
    const expr_node = tree.nodeData(node).rhs.node;
    ...
    const sub_scope = if (scope_tag != .defer_error) &defer_gen.base else blk: {
        const payload_token = tree.nodeData(node).lhs.opt_token.unwrap() orelse break :blk &defer_gen.base;
...
fn assign(gz: *GenZir, scope: *Scope, infix_node: Ast.Node.Index) InnerError!void {
    ...
    const lhs = tree.nodeData(infix_node).lhs.node;
    const rhs = tree.nodeData(infix_node).rhs.node;
...
fn assignOp(
    ...
    const lhs_ptr = try lvalExpr(gz, scope, tree.nodeData(infix_node).lhs.node);
    ...
    const rhs = try expr(gz, scope, .{ .rl = .{ .ty = rhs_res_ty } }, tree.nodeData(infix_node).rhs.node);
...
fn assignShift(
    ...
    const rhs = try expr(gz, scope, .{ .rl = .{ .ty = rhs_type } }, tree.nodeData(infix_node).rhs.node);
...
fn assignShiftSat(gz: *GenZir, scope: *Scope, infix_node: Ast.Node.Index) InnerError!void {
    ...
    const rhs = try expr(gz, scope, .{ .rl = .none }, tree.nodeData(infix_node).rhs.node);
...
fn arrayType(gz: *GenZir, scope: *Scope, ri: ResultInfo, node: Ast.Node.Index) !Zir.Inst.Ref {
    ...
    const len_node = tree.nodeData(node).lhs.node;
    ...
    const elem_type = try typeExpr(gz, scope, tree.nodeData(node).rhs.node);
...
fn arrayTypeSentinel(gz: *GenZir, scope: *Scope, ri: ResultInfo, node: Ast.Node.Index) !Zir.Inst.Ref {
    const astgen = gz.astgen;
    const tree = astgen.tree;
    const extra = tree.extraData(tree.nodeData(node).rhs.extra_index, Ast.Node.ArrayTypeSentinel);
...
fn fnDeclInner(
    ...
    const proto_node = tree.nodeData(decl_node).lhs.node;
...
fn usingnamespaceDecl(
    ...
    const type_expr = tree.nodeData(node).lhs.node;
...
fn testDecl(
    ...
    const tree = astgen.tree;
    const body_node = tree.nodeData(node).rhs.node;
    ...
    switch (tree.nodeTag(member_node)) {
        ...
        => {
            ...
            const body: Ast.Node.OptionalIndex = if (tree.nodeTag(member_node) == .fn_decl)
                tree.nodeData(member_node).rhs.node.toOptional()
            else
                .none;
...
fn addFieldAccess(
    ...
    const object_node = tree.nodeData(node).lhs.node;
    const dot_token = tree.nodeMainToken(node);
...
fn arrayAccess(
    ...
    switch (ri.rl) {
        .ref, .ref_coerced_ty => {
            ...
            const rhs = try expr(gz, scope, .{ .rl = .{ .coerced_ty = .usize_type } }, tree.nodeData(node).rhs.node);
            try emitDbgStmt(gz, cursor);
...
fn simpleBinOp(
    ...
    if (op_inst_tag == .cmp_neq or op_inst_tag == .cmp_eq) {
        ...
        if (tree.nodeTag(tree.nodeData(node).lhs.node) == .string_literal or
            tree.nodeTag(tree.nodeData(node).rhs.node) == .string_literal)
            return astgen.failNode(node, "cannot compare strings with {s}", .{str});
    }

    const lhs = try reachableExpr(gz, scope, .{ .rl = .none }, tree.nodeData(node).lhs.node, node);
    const cursor = switch (op_inst_tag) {
        .add, .sub, .mul, .div, .mod_rem => maybeAdvanceSourceCursorToMainToken(gz, node),
        else => undefined,
    };
    const rhs = try reachableExpr(gz, scope, .{ .rl = .none }, tree.nodeData(node).rhs.node, node);
...
fn boolBinOp(
    ...
    const lhs = try expr(gz, scope, coerced_bool_ri, tree.nodeData(node).lhs.node);
    ...
    const rhs = try fullBodyExpr(&rhs_scope, &rhs_scope.base, coerced_bool_ri, tree.nodeData(node).rhs.node, .allow_branch_hint);
    if (!gz.refIsNoReturn(rhs)) {
        _ = try rhs_scope.addBreakWithSrcNode(.break_inline, bool_br, rhs, tree.nodeData(node).rhs.node);
    }
...

    {
        ...
        for (for_full.ast.inputs, indexables, lens) |input, *indexable_ref, *len_refs| {
            ...
            if (tree.nodeTag(input) == .for_range) {
                ...
                const start_node = tree.nodeData(input).lhs.node;
                const start_val = try expr(parent_gz, scope, .{ .rl = .{ .ty = .usize_type } }, start_node);

                const end_node = tree.nodeData(input).rhs.opt_node;
...
fn switchExprErrUnion(
    ...
    const switch_node, const operand_node, const error_payload = switch (node_ty) {
        .@"catch" => .{
            tree.nodeData(catch_or_if_node).rhs.node,
            tree.nodeData(catch_or_if_node).lhs.node,
    ...
    for (switch_full.ast.cases) |case_node| {
        ...
        const body_len_index = if (is_multi_case) blk: {
            ...
            for (case.ast.values) |range| {
                ...
                const first = try comptimeExpr(parent_gz, scope, item_ri, tree.nodeData(range).lhs.node, .switch_item);
                const last = try comptimeExpr(parent_gz, scope, item_ri, tree.nodeData(range).rhs.node, .switch_item);
...
fn ret(gz: *GenZir, scope: *Scope, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    const operand_node = tree.nodeData(node).lhs.opt_node.unwrap() orelse {
    ...
    if (tree.nodeTag(operand_node) == .error_value) {
        ...
        const ident_token = tree.nodeData(operand_node).rhs.opt_token.unwrap().?;
...
fn asmExpr(
    ...
    for (full.outputs, 0..) |output_node, i| {
        ...
        if (has_arrow) {
            ...
            const out_type_node = tree.nodeData(output_node).lhs.opt_node.unwrap().?;
    ...
    for (full.inputs, 0..) |input_node, i| {
        ...
        const constraint_token = symbolic_name + 2;
        const constraint = (try astgen.strLitAsString(constraint_token)).index;
        const operand = try expr(gz, scope, .{ .rl = .none }, tree.nodeData(input_node).lhs.node);
        inputs[i] = .{
            .name = name,
            .constraint = constraint,
            .operand = operand,
    ...
    while (true) {
        switch (tree.nodeTag(node)) {
            ...
            .grouped_expression => {
                ...
                node = tree.nodeData(node).lhs.node;
                continue;
            },
            else => break,
        }
...
fn negation(
    ...
    const operand_node = tree.nodeData(node).lhs.node;
...
fn strLitNodeAsString(astgen: *AstGen, node: Ast.Node.Index) !IndexSlice {
    ...
    const start = tree.nodeData(node).lhs.token;
    const end = tree.nodeData(node).rhs.token;
...
const GenZir = struct {
    ...
    fn addFunc(
        ...
        const src_locs_and_hash: []const u32 = if (args.body_gz != null) src_locs_and_hash: {
            ...
            assert(tree.nodeTag(fn_decl) == .fn_decl or tree.nodeTag(fn_decl) == .test_decl);
            const block = tree.nodeData(fn_decl).rhs.node;
