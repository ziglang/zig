...
pub fn renderTree(buffer: *std.ArrayList(u8), tree: Ast, fixups: Fixups) Error!void {
    ...
    if (r.ais.disabled_offset) |disabled_offset| {
        try writeFixingWhitespace(r.ais.underlying_writer, tree.source[disabled_offset..]);
    }
}
...
fn renderMember(r: Render, decl: Ast.Node.Index, space: Space) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    ...
    switch (tree.nodes.items(.tag)[decl]) {
        ...
        .global_var_decl => return renderVarDecl(r, tree.globalVarDecl(decl)),
        .local_var_decl => return renderVarDecl(r, tree.localVarDecl(decl)),
        .simple_var_decl => return renderVarDecl(r, tree.simpleVarDecl(decl)),
        .aligned_var_decl => return renderVarDecl(r, tree.alignedVarDecl(decl)),
        ...
        .test_decl => {
            ...
            if (test_name_tag == .string_literal or test_name_tag == .identifier) {
                try renderToken(r, test_token + 1, .space);
            }
            try renderExpression(r, datas[decl].rhs, space);
        },

        .container_field_init => return renderContainerField(r, tree.containerFieldInit(decl), space),
        .container_field_align => return renderContainerField(r, tree.containerFieldAlign(decl), space),
        .container_field => return renderContainerField(r, tree.containerField(decl), space),
...
fn renderExpression(r: Render, node: Ast.Node.Index, space: Space) Error!void {
    ...
    switch (node_tags[node]) {
        ...
        .identifier => {
            ...
            return renderSpace(r, token_index, lexeme.len, space);
        },
        ...
        .error_value => {
            ...
            return renderToken(r, main_tokens[node] + 2, space);
        },
        ...
        .@"errdefer" => {
            ...
            if (payload_token != 0) {
                ...
                try renderToken(r, payload_token, .none); // identifier
        ...
        .@"catch" => {
            ...
            if (token_tags[fallback_first - 1] == .pipe) {
                ...
                try renderToken(r, main_token + 2, .none); // payload identifier
        ...
        .field_access => {
            ...
            return renderToken(r, field_access.rhs, space);
        },
        ...
        .array_type => return renderArrayType(r, tree.arrayType(node), space),
        .array_type_sentinel => return renderArrayType(r, tree.arrayTypeSentinel(node), space),

        .ptr_type_aligned => return renderPtrType(r, tree.ptrTypeAligned(node), space),
        .ptr_type_sentinel => return renderPtrType(r, tree.ptrTypeSentinel(node), space),
        .ptr_type => return renderPtrType(r, tree.ptrType(node), space),
        .ptr_type_bit_range => return renderPtrType(r, tree.ptrTypeBitRange(node), space),

        .array_init_one, .array_init_one_comma => {
            var elements: [1]Ast.Node.Index = undefined;
            return renderArrayInit(r, tree.arrayInitOne(&elements, node), space);
        },
        .array_init_dot_two, .array_init_dot_two_comma => {
            var elements: [2]Ast.Node.Index = undefined;
            return renderArrayInit(r, tree.arrayInitDotTwo(&elements, node), space);
        },
        .array_init_dot,
        .array_init_dot_comma,
        => return renderArrayInit(r, tree.arrayInitDot(node), space),
        .array_init,
        .array_init_comma,
        => return renderArrayInit(r, tree.arrayInit(node), space),

        .struct_init_one, .struct_init_one_comma => {
            var fields: [1]Ast.Node.Index = undefined;
            return renderStructInit(r, node, tree.structInitOne(&fields, node), space);
        },
        .struct_init_dot_two, .struct_init_dot_two_comma => {
            var fields: [2]Ast.Node.Index = undefined;
            return renderStructInit(r, node, tree.structInitDotTwo(&fields, node), space);
        },
        .struct_init_dot,
        .struct_init_dot_comma,
        => return renderStructInit(r, node, tree.structInitDot(node), space),
        .struct_init,
        .struct_init_comma,
        => return renderStructInit(r, node, tree.structInit(node), space),

        .call_one, .call_one_comma, .async_call_one, .async_call_one_comma => {
            var params: [1]Ast.Node.Index = undefined;
            return renderCall(r, tree.callOne(&params, node), space);
        },
        ...
        .async_call,
        .async_call_comma,
        => return renderCall(r, tree.callFull(node), space),
        ...
        .slice_open => return renderSlice(r, node, tree.sliceOpen(node), space),
        .slice => return renderSlice(r, node, tree.slice(node), space),
        .slice_sentinel => return renderSlice(r, node, tree.sliceSentinel(node), space),
        ...
        .@"break" => {
            ...
            if (label_token == 0 and target == 0) {
                ...
                try renderToken(r, label_token, space); // identifier
            } else if (label_token != 0 and target != 0) {
                try renderToken(r, main_token, .space); // break keyword
                try renderToken(r, label_token - 1, .none); // colon
                try renderToken(r, label_token, .space); // identifier
        ...
        .@"continue" => {
            ...
            if (label != 0) {
                ...
                return renderToken(r, label, space); // label
            } else {
        ...
        => return renderContainerDecl(r, node, tree.containerDecl(node), space),

        .container_decl_two, .container_decl_two_trailing => {
            var buffer: [2]Ast.Node.Index = undefined;
            return renderContainerDecl(r, node, tree.containerDeclTwo(&buffer, node), space);
        },
        .container_decl_arg,
        .container_decl_arg_trailing,
        => return renderContainerDecl(r, node, tree.containerDeclArg(node), space),

        .tagged_union,
        .tagged_union_trailing,
        => return renderContainerDecl(r, node, tree.taggedUnion(node), space),

        .tagged_union_two, .tagged_union_two_trailing => {
            var buffer: [2]Ast.Node.Index = undefined;
            return renderContainerDecl(r, node, tree.taggedUnionTwo(&buffer, node), space);
        },
        .tagged_union_enum_tag,
        .tagged_union_enum_tag_trailing,
        => return renderContainerDecl(r, node, tree.taggedUnionEnumTag(node), space),
        ...
        .error_set_decl => {
            ...
            if (lbrace + 1 == rbrace) {
                ...
                try renderToken(r, lbrace, .none);
                try renderToken(r, lbrace + 1, .none); // identifier
                return renderToken(r, rbrace, space);
                ...
                while (i < rbrace) : (i += 1) {
                    ...
                    switch (token_tags[i]) {
                        ...
                        .identifier => try renderToken(r, i, .comma),
                        .comma => {},
                        else => unreachable,
                    }
                }
                ...
                while (i < rbrace) : (i += 1) {
                    switch (token_tags[i]) {
                        ...
                        .identifier => try renderToken(r, i, .comma_space),
                        .comma => {},
                        else => unreachable,
                    }
                }
        ...
        .fn_proto_simple => {
            var params: [1]Ast.Node.Index = undefined;
            return renderFnProto(r, tree.fnProtoSimple(&params, node), space);
        },
        .fn_proto_multi => return renderFnProto(r, tree.fnProtoMulti(node), space),
        .fn_proto_one => {
            var params: [1]Ast.Node.Index = undefined;
            return renderFnProto(r, tree.fnProtoOne(&params, node), space);
        },
        .fn_proto => return renderFnProto(r, tree.fnProto(node), space),
        ...
        => {
            ...
            try renderExpression(r, condition, .none); // condtion expression
        ...
        .switch_case_one => return renderSwitchCase(r, tree.switchCaseOne(node), space),
        .switch_case => return renderSwitchCase(r, tree.switchCase(node), space),

        .while_simple => return renderWhile(r, tree.whileSimple(node), space),
        .while_cont => return renderWhile(r, tree.whileCont(node), space),
        .@"while" => return renderWhile(r, tree.whileFull(node), space),
        .for_simple => return renderWhile(r, tree.forSimple(node), space),
        .@"for" => return renderWhile(r, tree.forFull(node), space),

        .if_simple => return renderIf(r, tree.ifSimple(node), space),
        .@"if" => return renderIf(r, tree.ifFull(node), space),

        .asm_simple => return renderAsm(r, tree.asmSimple(node), space),
        .@"asm" => return renderAsm(r, tree.asmFull(node), space),

        .enum_literal => {
            try renderToken(r, main_tokens[node] - 1, .none); // .
            return renderToken(r, main_tokens[node], space); // name
        },
...
fn renderAsmOutput(
    ...
    try renderToken(r, symbolic_name, .none); // ident
    ...
    if (token_tags[symbolic_name + 4] == .arrow) {
        ...
        try renderToken(r, symbolic_name + 4, .none); // ident
...
fn renderVarDecl(r: Render, var_decl: Ast.full.VarDecl) Error!void {
    try renderVarDeclWithoutFixups(r, var_decl);
...
fn renderVarDeclWithoutFixups(r: Render, var_decl: Ast.full.VarDecl) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    ...
    const name_space = if (var_decl.ast.type_node == 0 and
        ...
        Space.none;
    try renderToken(r, var_decl.ast.mut_token + 1, name_space); // name
    ...
    if (var_decl.ast.type_node != 0) {
            ...
            var_decl.ast.section_node != 0 or var_decl.ast.init_node != 0)
        {
            ...
            try renderExpression(r, var_decl.ast.type_node, .none);
    ...
    if (var_decl.ast.init_node != 0) {
        ...
        ais.pushIndentOneShot();
        return renderExpression(r, var_decl.ast.init_node, .semicolon); // ;
    }
    return renderToken(r, var_decl.ast.mut_token + 2, .newline); // ;
}
...
fn renderWhile(r: Render, while_node: Ast.full.While, space: Space) Error!void {
    ...
    if (while_node.label_token) |label| {
        try renderToken(r, label, .none); // label
    ...
    if (while_node.payload_token) |payload_token| {
        ...
        try renderToken(r, ident, .none); // identifier
        const pipe = blk: {
            if (token_tags[ident + 1] == .comma) {
                try renderToken(r, ident + 1, .space); // ,
                try renderToken(r, ident + 2, .none); // index
                break :blk ident + 3;
            } else {
                break :blk ident + 1;
            }
    ...
    if (while_node.ast.else_expr != 0) {
        if (indent_then_expr) {
            ais.pushIndent();
            try renderExpression(r, while_node.ast.then_expr, .newline);
            ais.popIndent();
        } else {
            try renderExpression(r, while_node.ast.then_expr, .space);
        }

        var last_else_token = while_node.else_token;

        if (while_node.error_token) |error_token| {
            try renderToken(r, while_node.else_token, .space); // else
            try renderToken(r, error_token - 1, .none); // |
            try renderToken(r, error_token, .none); // identifier
        ...
        if (indent_else_expr) {
            ...
            ais.popIndent();
            try renderExpressionIndented(r, while_node.ast.else_expr, space);
        } else {
            try renderToken(r, last_else_token, .space);
            try renderExpression(r, while_node.ast.else_expr, space);
        }
    } else {
        if (indent_then_expr) {
            try renderExpressionIndented(r, while_node.ast.then_expr, space);
        } else {
            try renderExpression(r, while_node.ast.then_expr, space);
        }
    }
...
fn renderContainerField(r: Render, field: Ast.full.ContainerField, space: Space) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    ...
    if (field.ast.type_expr == 0 and field.ast.value_expr == 0) {
        return renderTokenComma(r, field.ast.name_token, space); // name
    }
    if (field.ast.type_expr != 0 and field.ast.value_expr == 0) {
        try renderToken(r, field.ast.name_token, .none); // name
        try renderToken(r, field.ast.name_token + 1, .space); // :
    ...
    if (field.ast.type_expr == 0 and field.ast.value_expr != 0) {
        try renderToken(r, field.ast.name_token, .space); // name
        try renderToken(r, field.ast.name_token + 1, .space); // =
...
fn renderFnProto(r: Render, fn_proto: Ast.full.FnProto, space: Space) Error!void {
    ...
    const lparen = if (token_tags[after_fn_token] == .identifier) blk: {
        ...
        try renderToken(r, after_fn_token, .none); // name
    ...
    if (!trailing_comma and !hasComment(tree, lparen, rparen)) {
        ...
        while (true) {
            ...
            {
                try renderToken(r, last_param_token, .none); // name
                last_param_token += 1;
                try renderToken(r, last_param_token, .space); // :
                last_param_token += 1;
            }
...
fn renderSwitchCase(
    ...
    if (switch_case.payload_token) |payload_token| {
        ...
        if (token_tags[payload_token] == .asterisk) {
            ...
            try renderToken(r, payload_token + 1, .none); // identifier
            try renderToken(r, payload_token + 2, pre_target_space); // pipe
        } else {
            try renderToken(r, payload_token, .none); // identifier
            try renderToken(r, payload_token + 1, pre_target_space); // pipe
        }
    }
...
fn renderBlock(
    ...
    {
        try renderToken(r, lbrace - 2, .none);
    ...
    if (statements.len == 0) {
        ...
        for (statements) |stmt, i| {
            ...
            switch (node_tags[stmt]) {
                .global_var_decl => try renderVarDecl(r, tree.globalVarDecl(stmt)),
                .local_var_decl => try renderVarDecl(r, tree.localVarDecl(stmt)),
                .simple_var_decl => try renderVarDecl(r, tree.simpleVarDecl(stmt)),
                .aligned_var_decl => try renderVarDecl(r, tree.alignedVarDecl(stmt)),
...
fn renderStructInit(
    ...
    if (trailing_comma or hasComment(tree, struct_init.ast.lbrace, rbrace)) {
        ...
        try renderToken(r, struct_init.ast.lbrace + 1, .none); // .
        try renderToken(r, struct_init.ast.lbrace + 2, .space); // name
        try renderToken(r, struct_init.ast.lbrace + 3, .space); // =
        ...
        for (struct_init.ast.fields[1..]) |field_init| {
            ...
            try renderToken(r, init_token - 2, .space); // name
...
fn renderContainerDecl(
    ...
    if (!src_has_trailing_comma) one_line: {
        ...
        for (container_decl.ast.members) |member| {
            try renderMember(r, member, .space);
        }
        return renderToken(r, rbrace, space); // rbrace
    }
    ...
    for (container_decl.ast.members) |member, i| {
        ...
        switch (tree.nodes.items(.tag)[member]) {
            ...
            else => try renderMember(r, member, .newline),
        }
    }
    ais.popIndent();
