...
fn renderMember(
    ...
    switch (tree.nodeTag(decl)) {
        .fn_decl => {
            ...
            // pub extern "foo" fn ...
            // export fn ...
            const data = tree.nodeData(decl);
            const fn_proto = data.lhs.node;
        ...
        .@"usingnamespace" => {
            ...
            const expr = tree.nodeData(decl).lhs.node;
            if (main_token > 0 and tree.tokenTag(main_token - 1) == .keyword_pub) {
        ...
        .test_decl => {
            ...
            const test_name_tag = tree.tokenTag(test_token + 1);
            ...
            try renderExpression(r, tree.nodeData(decl).rhs.node, space);
        },
...
fn renderExpression(r: *Render, node: Ast.Node.Index, space: Space) Error!void {
    ...
    switch (tree.nodeTag(node)) {
        ...
        .multiline_string_literal => {
            ...
            for (data.lhs.token..data.rhs.token + 1) |i| {
                try renderToken(r, @intCast(i), .newline);
            }

            while (locked_indents > 0) : (locked_indents -= 1) ais.popIndent();

            const next_token = data.rhs.token + 1;
            const next_token_tag = tree.tokenTag(next_token);
            switch (space) {
                .none, .space, .newline, .skip => {},
                .semicolon => if (next_token_tag == .semicolon) try renderToken(r, next_token, .newline),
                .comma => if (next_token_tag == .comma) try renderToken(r, next_token, .newline),
        ...
        .@"errdefer" => {
            ...
            const data = tree.nodeData(node);
            const expr = data.rhs.node;

            try renderToken(r, defer_token, .space);
            if (data.lhs.opt_token.unwrap()) |payload_token| {
        ...
        .@"defer" => {
            const defer_token = tree.nodeMainToken(node);
            const expr = tree.nodeData(node).rhs.node;
            try renderToken(r, defer_token, .space);
            return renderExpression(r, expr, space);
        },
        .@"comptime", .@"nosuspend" => {
            const comptime_token = tree.nodeMainToken(node);
            const block = tree.nodeData(node).lhs.node;
            try renderToken(r, comptime_token, .space);
            return renderExpression(r, block, space);
        },

        .@"suspend" => {
            const suspend_token = tree.nodeMainToken(node);
            const body = tree.nodeData(node).lhs.node;
        ...
        .@"catch" => {
            const main_token = tree.nodeMainToken(node);
            const data = tree.nodeData(node);
            const fallback_first = tree.firstToken(data.rhs.node);
            ...
            try renderExpression(r, data.lhs.node, .space); // target
            ...
            try renderExpression(r, data.rhs.node, space); // fallback
        },
        ...
        .field_access => {
            ...
            try renderExpression(r, data.lhs.node, .none);

            // Allow a line break between the lhs and the dot if the lhs and rhs
            // are on different lines.
            const lhs_last_token = tree.lastToken(data.lhs.node);
            ...
            return renderIdentifier(r, data.rhs.token, space, .eagerly_unquote); // field
        },

        .error_union,
        .switch_range,
        => {
            const infix = tree.nodeData(node);
            try renderExpression(r, infix.lhs.node, .none);
            try renderToken(r, tree.nodeMainToken(node), .none);
            return renderExpression(r, infix.rhs.node, space);
        },
        .for_range => {
            const infix = tree.nodeData(node);
            try renderExpression(r, infix.lhs.node, .none);
            if (infix.rhs.opt_node.unwrap()) |rhs| {
                try renderToken(r, tree.nodeMainToken(node), .none);
        ...
        => {
            const infix = tree.nodeData(node);
            try renderExpression(r, infix.lhs.node, .space);
        ...
        => {
            try renderToken(r, tree.nodeMainToken(node), .none);
            return renderExpression(r, tree.nodeData(node).lhs.node, space);
        },
        ...
        .array_access => {
            const suffix = tree.nodeData(node);
            const lbracket = tree.firstToken(suffix.rhs.node) - 1;
            const rbracket = tree.lastToken(suffix.rhs.node) + 1;
            const one_line = tree.tokensOnSameLine(lbracket, rbracket);
            const inner_space = if (one_line) Space.none else Space.newline;
            try renderExpression(r, suffix.lhs.node, .none);
            ais.pushIndentNextLine();
            try renderToken(r, lbracket, inner_space); // [
            try renderExpression(r, suffix.rhs.node, inner_space);
            ais.popIndent();
        ...
        .deref => {
            try renderExpression(r, tree.nodeData(node).lhs.node, .none);
            return renderToken(r, tree.nodeMainToken(node), space);
        },
        ...
        .unwrap_optional => {
            ...
            try renderExpression(r, data.lhs.node, .none);
            try renderToken(r, tree.nodeMainToken(node), .none);
            return renderToken(r, data.rhs.token, space);
        },

        .@"break", .@"continue" => {
            const main_token = tree.nodeMainToken(node);
            const data = tree.nodeData(node);
            const opt_label_token = data.lhs.opt_token;
            const opt_target = data.rhs.opt_node;
        ...
        .@"return" => {
            if (tree.nodeData(node).lhs.opt_node.unwrap()) |expr| {
                try renderToken(r, tree.nodeMainToken(node), .space);
        ...
        .grouped_expression => {
            ...
            ais.pushIndentOneShot();
            try renderExpression(r, data.lhs.node, .none);
            return renderToken(r, data.rhs.token, space); // rparen
        },
        ...
        .error_set_decl => {
            ...
            const rbrace = tree.nodeData(node).rhs.token;
        ...
        .anyframe_type => {
            ...
            try renderToken(r, main_token + 1, .none); // ->
            return renderExpression(r, tree.nodeData(node).rhs.node, space);
        },
...
fn renderAsmOutput(
    ...
    if (tree.tokenTag(symbolic_name + 4) == .arrow) {
        ...
        try renderExpression(r, tree.nodeData(asm_output).lhs.opt_node.unwrap().?, Space.none);
        return renderToken(r, tree.nodeData(asm_output).rhs.token, space); // rparen
    } else {
...
fn renderAsmInput(
    ...
    const data = tree.nodeData(asm_input);
    ...
    try renderToken(r, symbolic_name + 2, .space); // "constraint"
    try renderToken(r, symbolic_name + 3, .none); // lparen
    try renderExpression(r, data.lhs.node, Space.none);
    return renderToken(r, data.rhs.token, space); // rparen
}
...
fn renderVarDeclWithoutFixups(
    ...
    ais.pushIndentOneShot();
    return renderExpression(r, init_node, space); // ;
}
        ...
        for (params, 0..) |param_node, i| {
            if (i + 1 < params.len) {
                ...
                const is_multiline_string =
                    tree.tokenTag(tree.firstToken(param_node)) == .multiline_string_literal_line;
...
fn renderExpressionIndented(r: *Render, node: Ast.Node.Index, space: Space) Error!void {
    ...
    const punctuation = switch (space) {
        ...
        .comma => true,
        .comma_space => tree.tokenTag(last_token + 1) == .comma,
        .semicolon => tree.tokenTag(last_token + 1) == .semicolon,
    };
    ...
    switch (space) {
        .none, .space, .newline, .skip => {},
        .comma => {
            if (tree.tokenTag(last_token + 1) == .comma) {
        ...
        .comma_space => if (tree.tokenTag(last_token + 1) == .comma) {
            try renderToken(r, last_token + 1, .skip);
            last_token += 1;
        },
        .semicolon => if (tree.tokenTag(last_token + 1) == .semicolon) {
            try renderToken(r, last_token + 1, .skip);
            last_token += 1;
        },
    }
    ...
    const comment_start = tree.tokenStart(last_token) + tokenSliceForRender(tree, last_token).len;
    const comment = try renderComments(r, comment_start, tree.tokenStart(last_token + 1));
