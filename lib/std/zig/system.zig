...
pub fn abiAndDynamicLinkerFromFile(
    ...
    const header: elf.Header = try elf.Header.parse(&hdr_buf);
    var result: Target = .{
        .cpu = cpu,
        .os = os,
    ...
    if (header.phentsize > @sizeOf(elf.Elf64_Phdr)) return error.InvalidElfFile;

    var got_dyn_section: bool = false;
    var program_headers = header.program_header_iterator(&file);
    while (program_headers.next() catch |err| switch (err) {
        error.OperationAborted => unreachable, // windows-only
        error.WouldBlock => unreachable,
        error.NotOpenForReading => unreachable,
        error.SystemResources, error.Unexpected => |e| return e,
        error.InputOutput => return error.FileSystem,
        else => return error.UnableToReadElfFile,
    }) |*program_header| switch (program_header.p_type) {
        elf.PT_INTERP => {
            got_dyn_section = true;
            if (look_for_ld) {
                const p_offset = program_header.p_offset;
                const p_filesz = program_header.p_filesz;
                ...
                const full_ld_path = result.dynamic_linker.buffer[0..len :0];
        ...
        elf.PT_DYNAMIC => {
            ...
            {
                var dyn_off = program_header.p_offset;
                const p_filesz = program_header.p_filesz;
                const is_64 = header.is_64;
                ...
                dyn: while (dyn_i < dyn_num) {
                    ...
                    while (dyn_buf_i < dyn_read_byte_len and dyn_i < dyn_num) : ({
                        ...
                        const dyn32: *const elf.Elf32_Dyn = @ptrCast(@alignCast(&dyn_buf[dyn_buf_i]));
                        const dyn64: *const elf.Elf64_Dyn = @ptrCast(@alignCast(&dyn_buf[dyn_buf_i]));
                        const tag = elf.int(is_64, dyn32.d_tag, dyn64.d_tag, header.endian);
                        const val = elf.int(is_64, dyn32.d_val, dyn64.d_val, header.endian);
    ...
    if (!got_dyn_section) return error.StaticElfFile;

    if (builtin.target.os.tag == .linux and result.isGnuLibC() and
        query.glibc_version == null)
    {
        var strtab_buf: [4096]u8 = undefined;
        const shstrtab: []const u8 = shstrtab: {
            const section_header = section_header: {
                var sh_buf: [16 * @sizeOf(elf.Elf64_Shdr)]u8 align(@alignOf(elf.Elf64_Shdr)) = undefined;
                const shentsize = header.shentsize;
                if (sh_buf.len < shentsize) return error.InvalidElfFile;

                const str_section_off = header.shoff + @as(u64, shentsize) * @as(u64, header.shstrndx);
                _ = try preadMin(file, &sh_buf, str_section_off, shentsize);

                break :section_header try elf.SectionHeader.parse(sh_buf[0..@sizeOf(elf.Elf64_Shdr)], &header);
            };

            const shstrtab_len = @min(section_header.size, strtab_buf.len);
            const shstrtab_read_len = try preadMin(file, &strtab_buf, section_header.offset, shstrtab_len);
            break :shstrtab strtab_buf[0..shstrtab_read_len];
        };

        var section_headers = header.section_header_iterator(file);
        const dynstr: ?struct { offset: u64, size: u64 } = find_dyn_str: while (section_headers.next() catch |err| switch (err) {
            error.OperationAborted => unreachable, // windows-only
            error.WouldBlock => unreachable,
            error.NotOpenForReading => unreachable,
            error.SystemResources, error.Unexpected => |e| return e,
            ...
            const sh_name = mem.sliceTo(shstrtab[section_header.sh_name..], 0);
...
fn glibcVerFromSoFile(file: fs.File) !std.SemanticVersion {
    ...
    // Empirically, glibc 2.37 libc.so .dynstr section is 35312 bytes on my system.
    // Here I use double this value plus some headroom. This makes it only need
    // a single read syscall here.
    var buf: [85_000]u8 = undefined;
