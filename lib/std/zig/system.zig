...
fn resolveElfFileRecursively(cwd: fs.Dir, start_path: []const u8) error{UnableToFindElfFile}!fs.File {
    ...
    while (true) : (fbs.reset()) {
        const file = cwd.openFile(current_path.constSlice(), .{}) catch return error.UnableToFindElfFile;
        ...
        if (len > 4 and mem.eql(u8, content[0..4], std.elf.MAGIC)) {
            ...
            const interpreter_path = interpreter_path: {
                ...
                const trimmed_line = mem.trimStart(u8, first_line, &.{ ' ', '\t' });
...
fn detectAbiAndDynamicLinker(
    ...
    const elf_file = (try_path: {
        ...
        while (it.next()) |path| : (fbs.reset()) {
            ...
            break :try_path resolveElfFileRecursively(cwd, start_path) catch continue;
        ...
        break :try_hardcoded resolveElfFileRecursively(cwd, hardcoded_file_name) catch null;
