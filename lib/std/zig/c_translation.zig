...
fn PromotedIntType(comptime T: type) type {
    return switch (T) {
        ...
        c_int, c_uint, c_long, c_ulong, c_longlong, c_ulonglong => T,
            ...
            .ComptimeInt => @compileError("Cannot promote `" ++ @typeName(T) ++ "`; a fixed-size number type is required"),
            // promote to c_int if it can represent all values of T
            .Int => |int_info| if (int_info.bits < @typeInfo(c_int).Int.bits)
                c_int
                // otherwise, restore the original C type
            else if (int_info.bits == @typeInfo(c_int).Int.bits)
                if (int_info.signedness == .unsigned) c_uint else c_int
            else if (int_info.bits <= @typeInfo(c_long).Int.bits)
                if (int_info.signedness == .unsigned) c_ulong else c_long
            else if (int_info.bits <= @typeInfo(c_longlong).Int.bits)
