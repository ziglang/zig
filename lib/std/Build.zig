...
pub const LazyPath = struct {
    ...
    pub fn relative(path: []const u8) LazyPath {
        ...
        return LazyPath{ .parent = .{ .path = path } };
    }
    ...
    pub fn dirname(self: LazyPath) LazyPath {
        return .{
            .sub_path = if (self.sub_path) |sub| dirnameAllowEmpty(sub) orelse {
    ...
    pub fn getPath2(self: LazyPath, src_builder: *Build, asking_step: ?*Step) []const u8 {
        const basepath = switch (self.parent) {
            .path => |p| src_builder.pathFromRoot(p),
            ...
            .generated_dirname => |gen| blk: {
                ...
                var path = getPath2(LazyPath{ .parent = .{ .generated = gen.generated } }, src_builder, asking_step);
                var i: usize = 0;
                while (i <= gen.up) : (i += 1) {
                    // path is absolute.
            ...
            .dependency => |dep| dep.builder.build_root.path.?,
        };
        return if (self.sub_path) |sub| src_builder.pathJoin(&.{ basepath, sub }) else basepath;
    }

    /// Sets the sub path and return a new lazy path
    pub fn subPath(self: LazyPath, sub_path: []const u8) LazyPath {
        return .{ .sub_path = sub_path, .parent = self.parent };
    }
    ...
    pub fn dupe(self: LazyPath, b: *Build) LazyPath {
        return .{
            ...
            .sub_path = if (self.sub_path) |sub| b.dupePath(sub) else null,
        };
    }
};
