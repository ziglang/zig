...
pub fn readv(self: File, iovecs: []io.ReadBuffers) ReadError!usize {
    if (is_windows) {
        ...
        return windows.ReadFile(self.handle, first.ptr[0..first.len], null);
    }
...
///   addresses when the length is zero. So this function modifies the ptr fields
...
pub fn preadv(self: File, iovecs: []const posix.iovec, offset: u64) PReadError!usize {
    if (is_windows) {
        ...
        if (iovecs.len == 0) return @as(usize, 0);
        const first = iovecs[0];
        return windows.ReadFile(self.handle, first.ptr[0..first.len], offset);
    }
...
pub fn writev(self: File, iovecs: []io.WriteBuffers) WriteError!usize {
    if (is_windows) {
        ...
        if (iovecs.len == 0) return @as(usize, 0);
        const first = iovecs[0];
        return windows.WriteFile(self.handle, first.ptr[0..first.len], null);
    }
...
pub fn pwritev(self: File, iovecs: []posix.iovec_const, offset: u64) PWriteError!usize {
    if (is_windows) {
        ...
        if (iovecs.len == 0) return @as(usize, 0);
        const first = iovecs[0];
        return windows.WriteFile(self.handle, first.ptr[0..first.len], offset);
    }
