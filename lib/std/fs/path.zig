...
pub fn ComponentIterator(comptime path_type: PathType, comptime T: type) type {
    return struct {
        ...
        /// Like `previous` but skips '.' entries and handles '..' entries
        /// by skipping its parent directory.
        pub fn previousResolveDots(self: *Self) ?Component {
            var dot_dot_count: usize = 0;
            while (true) {
                const component = self.previous() orelse return null;
...
fn realpathMatches(dir: std.fs.Dir, sub_path: []const u8) !bool {
    ...
    var match = true;
    while (true) {
        const sub_path_component = sub_path_it.previousResolveDots() orelse break;
        const real_path_component = real_path_it.previousResolveDots() orelse unreachable;
