...
pub fn pread(fd: i32, buf: [*]u8, count: usize, offset: u64) usize {
    if (@hasField(SYS, .pread64)) {
        ...
        return syscall4(.pread, @bitCast(usize, @as(isize, fd)), @ptrToInt(buf), count, offset);
    }
}
...
pub fn pwrite(fd: i32, buf: [*]const u8, count: usize, offset: usize) usize {
    return syscall4(.pwrite, @bitCast(usize, @as(isize, fd)), @ptrToInt(buf), count, offset);
}
...
pub fn io_uring_register(fd: i32, opcode: u32, arg: ?*const c_void, nr_args: u32) usize {
    return syscall4(.io_uring_register, @bitCast(usize, @as(isize, fd)), opcode, @ptrToInt(arg), nr_args);
}
