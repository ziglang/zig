...
inline fn mmap(length: usize) usize {
    ...
    if (@hasField(linux.SYS, "mmap2")) {
        ...
        return if (native_arch == .s390x) @call(.always_inline, linux.syscall1, .{
            .mmap,
            @intFromPtr(&[_]usize{ 0, length, prot, @as(u32, @bitCast(flags)), @as(usize, @bitCast(@as(isize, -1))), 0 }),
        }) else @call(.always_inline, linux.syscall6, .{ .mmap, 0, length, prot, @as(u32, @bitCast(flags)), @as(usize, @bitCast(@as(isize, -1))), 0 });
    }
}
