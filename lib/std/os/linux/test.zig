...
test "timer" {
    ...
    err = linux.E.init(linux.timerfd_settime(@intCast(timer_fd), .{}, &new_time, null));
    try expect(err == .SUCCESS);
    ...
    err = linux.E.init(linux.epoll_ctl(@intCast(epoll_fd), .ctl_add, @intCast(timer_fd), &event));
    try expect(err == .SUCCESS);

    var events: [8]linux.epoll_event = @splat(undefined);

    err = linux.E.init(linux.epoll_wait(@intCast(epoll_fd), &events, 8, -1));
    try expect(err == .SUCCESS);
}
