...
test "readv" {
    ...
    var iovecs = [_]posix.iovec{posix.iovec{ .ptr = &buffer, .len = buffer.len }};
...
test "writev/fsync/readv" {
    ...
    const iovecs_write = [_]posix.iovec_const{
        posix.iovec_const{ .ptr = &buffer_write, .len = buffer_write.len },
    };
    var buffer_read = [_]u8{0} ** 128;
    var iovecs_read = [_]posix.iovec{
        posix.iovec{ .ptr = &buffer_read, .len = buffer_read.len },
    };
...
test "write_fixed/read_fixed" {
    ...
    var buffers = [2]posix.iovec{
        .{ .ptr = &raw_buffers[0], .len = raw_buffers[0].len },
        .{ .ptr = &raw_buffers[1], .len = raw_buffers[1].len },
    };
    ...
    try testing.expectEqualSlices(u8, "\x00\x00\x00", buffers[1].ptr[0..3]);
    try testing.expectEqualSlices(u8, "foobar", buffers[1].ptr[3..9]);
    try testing.expectEqualSlices(u8, "zz", buffers[1].ptr[9..11]);
}
...
test "sendmsg/recvmsg" {
    ...
    const iovecs_send = [_]posix.iovec_const{
        posix.iovec_const{ .ptr = &buffer_send, .len = buffer_send.len },
    };
    ...
    var iovecs_recv = [_]posix.iovec{
        posix.iovec{ .ptr = &buffer_recv, .len = buffer_recv.len },
    };
    const addr = [_]u8{0} ** 4;
