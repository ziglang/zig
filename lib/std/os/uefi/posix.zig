...
pub fn close(fd: fd_t) void {
    switch (fd) {
        .file => |p| p.close().err() catch {},
        .simple_output => |p| p.reset(true).err() catch {},
        .simple_input => |p| p.reset(true).err() catch {},
        .none => {},
        .cwd => {},
    }
}
...
pub fn lseek_SET(fd: fd_t, pos: u64) std.posix.SeekError!void {
    switch (fd) {
        .file => |p| {
            return p.setPosition(pos).err() catch return error.Unseekable;
        },
...
pub fn lseek_CUR(fd: fd_t, offset: i64) std.posix.SeekError!void {
    switch (fd) {
        .file => |p| {
            const end = p.getEndPosition().err() catch return error.Unseekable;
            const pos = p.getPosition().err() catch return error.Unseekable;
...
pub fn lseek_CUR_get(fd: fd_t) std.posix.SeekError!u64 {
    switch (fd) {
        .file => |p| {
            return p.getPosition().err() catch return error.Unseekable;
        },
        else => return error.Unseekable, // cannot read
    }
}
...
pub fn read(fd: fd_t, buf: []u8) std.posix.ReadError!usize {
    switch (fd) {
        .file => |p| {
            ...
            p.read(&size, buf.ptr).err() catch |err| switch (err) {
                error.NoMedia => return error.InputOutput,
                error.DeviceError => return error.InputOutput,
                error.VolumeCorrupted => return error.InputOutput,
                else => |e| return unexpectedError(e),
...
pub fn write(fd: fd_t, buf: []const u8) std.posix.WriteError!usize {
    switch (fd) {
        .file => |p| {
            var size: usize = buf.len;
            p.write(&size, buf.ptr).err() catch |err| switch (err) {
        ...
        .simple_output => |p| {
            ...
            while (iter.nextCodepoint()) |codepoint| {
                if (index + 2 >= utf16.len) {
                    utf16[index] = 0;
                    p.outputString(utf16[0..index :0]).err() catch |err| switch (err) {
                        error.DeviceError => return error.InputOutput,
                        error.Unsupported => return error.NotOpenForWriting,
