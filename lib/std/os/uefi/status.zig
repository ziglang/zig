...
pub const Status = enum(usize) {
    ...
    /// This protocol is not supported in the remote system.
    protocol_unreachable = high_bit | 102,

    /// No service is available at this port on the remote host.
    port_unreachable = high_bit | 103,

    /// The operation failed because the peer has closed the network connection.
    connection_finished = high_bit | 104,

    /// The operation failed because the network connection was reset.
    connection_reset = high_bit | 105,

    /// The operation failed because the network connection was refused.
    ...
    pub fn err(self: Status) EfiError!void {
        switch (self) {
            .success => return,
            .warn_unknown_glyph => return,
            .warn_delete_failure => return,
            .warn_write_failure => return,
            .warn_buffer_too_small => return,
            .warn_stale_data => return,
            .warn_filesystem => return,
            .warn_reset_required => return,
