...
test "openat smoke test" {
    ...
    if (native_os == .wasi and builtin.link_libc) {
        // .. except on wasm-libc WASI, where it works.
        // See https://github.com/ziglang/zig/issues/5732
        const dfd = try posix.openat(tmp.dir.fd, "some_dir", CommonOpenFlags.lower(.{
            .ACCMODE = .RDWR,
        }), mode);
        posix.close(dfd);
    } else {
