...
/// the pool after the setup is complemented.
fn connectSocks5Tunnel(client: *Client, proxy: *Proxy, tunnel_host: []const u8, tunnel_port: u16, tunnel_protocol: Connection.Protocol) (ConnectError || error{TunnelNotSupported})!*Connection {
    ...
    // socks5 handshake
    {
        ...
        auth_flows.appendAssumeCapacity(@intFromEnum(socks5.KnownAuthentication.none));

        if (proxy.authorization) |authorization| {
            if (std.mem.startsWith(u8, authorization, basic_authorization.prefix)) {
                auth_flows.appendAssumeCapacity(@intFromEnum(socks5.KnownAuthentication.password));
            }
        }

        const hello = socks5.ClientGreeting{
            .auth = auth_flows.constSlice(),
        };

        _ = try hello.serialize(tunnel.data.writer());
        try tunnel.data.flush();
    }

    const authChoice = recvAuthChoice: {
        const choice = socks5.ServerChoice.deserialize(stream.reader()) catch |err| switch (err) {
            error.EndOfStream => return error.ConnectionResetByPeer,
            // error.TlsFailure, error.TlsAlert => unreachable,
            // error.UnexpectedReadFailure => return error.UnexpectedConnectFailure,
            else => return @as(ConnectError, @errorCast(err)),
        };
        if (choice.ver != 5) {
            return error.TunnelNotSupported;
        }

        break :recvAuthChoice choice.cauth;
    };

    switch (authChoice) {
        @intFromEnum(socks5.KnownAuthentication.none) => {},
        @intFromEnum(socks5.KnownAuthentication.password) => {
            const authorization = (proxy.authorization orelse return error.TunnelNotSupported)[basic_authorization.prefix.len..];
            const B64 = std.base64.standard.Decoder;
            const size = B64.calcSizeForSlice(authorization) catch |err| {
                std.log.err("error: {}", .{err});
                return error.TunnelNotSupported;
            };
            const oauth = try client.allocator.alloc(u8, size);
            defer client.allocator.free(oauth);
            ...
            const pwflow = socks5.password;

            _ = try (pwflow.Authenticate{
                .id = username,
                .password = password,
            }).serialize(tunnel.data.writer());
            try tunnel.data.flush();

            const response = pwflow.Response.deserialize(stream.reader()) catch |err| switch (err) {
                error.EndOfStream => {
                    std.log.err("socks5 authentication stops early", .{});
            ...
            // Authentication is done. Continue with ConnectRequest.
        },
        else => return error.TunnelNotSupported,
    }

    if (tunnel_host.len > 255) {
        // TODO: if the host is too long, try SOCKS5 UDP to resovle the address
        return error.TunnelNotSupported;
    }
    const request = socks5.ConnectRequest{
        .cmd = @intFromEnum(socks5.ConnectRequest.KnownCommand.stream_connect),
        .dstaddr = socks5.Address.fromSlice(tunnel_host),
        .dstport = tunnel_port,
    };
    _ = try request.serialize(tunnel.data.writer());
    try tunnel.data.flush();

    const reply = socks5.ConnectReply.deserialize(stream.reader()) catch |err| switch (err) {
        error.EndOfStream => return error.ConnectionResetByPeer,
        // error.TlsAlert, error.TlsFailure => unreachable,
        // error.UnknownAddressType => return error.UnexpectedConnectFailure,
        // error.UnexpectedReadFailure => return error.UnexpectedConnectFailure,
        else => return @as(ConnectError, @errorCast(err)),
    };
    if (reply.ver != 5) {
    ...
    const replyCode = socks5.ConnectReply.KnownRep.from(reply.rep) orelse return error.TunnelNotSupported;
    switch (replyCode) {
        .success => {},
        .general_failure => return ConnectTcpError.UnexpectedConnectFailure,
        .ruleset_rejected => return ConnectTcpError.ConnectionRefused,
        .network_unreachable => return ConnectTcpError.NetworkUnreachable,
        .host_unreachable => return ConnectTcpError.NetworkUnreachable,
        .ttl_expired => return ConnectTcpError.NetworkUnreachable,
        .address_not_supported => return ConnectTcpError.UnknownHostName,
        .command_not_supported => return ConnectTcpError.UnexpectedConnectFailure,
        .connection_refused => return ConnectTcpError.ConnectionRefused,
    }
