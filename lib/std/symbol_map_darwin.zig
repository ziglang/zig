...
const Module = struct {
    ...
    fn readMachODebugInfo(allocator: *mem.Allocator, macho_file: File) !Self {
        const mapped_mem = try debug_info.mapWholeFile(macho_file);
        ...
        return Self{
            ...
            .ofiles = OFileTable.init(allocator),
            .symbols = symbols,
            .strings = strings,
        };
    }
    ...
    pub fn addressToSymbol(self: *Self, address: usize) !SymbolInfo {
        nosuspend {
            ...
            var o_file_di = self.ofiles.get(o_file_path) orelse
                ...
                return if (std.meta.errorInSet(err, debug_info.BaseError || error{FileNotFound}))
            ...
            return debug_info.dwarfAddressToSymbolInfo(&o_file_di, relocated_address);
        }
    }
    ...
    fn loadOFile(self: *Self, o_file_path: []const u8) !DW.DwarfInfo {
        ...
        try DW.openDwarfDebugInfo(&di, self.ofiles.allocator);
