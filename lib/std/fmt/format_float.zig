...
pub fn formatFloat(buf: []u8, v_: anytype, options: FormatOptions) FormatError![]const u8 {
    ...
    return switch (options.mode) {
        ...
        .decimal => formatDecimal(DT, buf, d, options.precision, options.commas),
    };
}
...
fn writeDecimal(buf: []u8, value: anytype, count_: usize, commas: bool) void {
    const count = if (commas) (count_ - 1) / 3 + count_ else count_;
    var i: usize = 0;

    if (!commas) while (i + 2 < count) : (i += 2) {
...
pub fn formatScientific(comptime T: type, buf: []u8, f_: FloatDecimal(T), precision: ?usize) FormatError![]const u8 {
    ...
    // 1.12345
    writeDecimal(buf[index + 2 ..], &output, olength - 1, false);
    ...
    writeDecimal(buf[index..], &uexp, elength, false);
    index += elength;
...
/// always be written.
pub fn formatDecimal(comptime T: type, buf: []u8, f_: FloatDecimal(T), precision: ?usize, commas: bool) FormatError![]const u8 {
    std.debug.assert(buf.len >= min_buffer_size);
    var f = f_;
    ...
    if (dp_offset <= 0) {
        ...
        index += dp_poffset;
        writeDecimal(buf[index..], &output, olength, false);
        index += olength;
        ...
        if (dp_uoffset >= olength) {
            if (commas) {
                ...
                writeDecimal(buf[index..], &output, dp_uoffset, commas);
            ...
            // 12345.6789
            const dp_o = if (commas) dp_uoffset + (dp_uoffset - 1) / 3 else dp_uoffset;
            writeDecimal(buf[index + dp_o + 1 ..], &output, olength - dp_uoffset, false);
            buf[index + dp_o] = '.';
...
fn checkComma(value: f64, expected: []const u8) !void {
    var buf: [100]u8 = undefined;
    const str = try formatFloat(&buf, value, .{ .commas = true, .mode = .decimal });
    try std.testing.expectEqualStrings(expected, str);
}

test "format float with commas" {
    try checkComma(123456, "123,456");
    try checkComma(1234567, "1,234,567");
    try checkComma(12345678, "12,345,678");
    try checkComma(123456789, "123,456,789");
    try checkComma(123456789.123, "123,456,789.123");
    try checkComma(12345678.9123, "12,345,678.9123");
    try checkComma(1234567.89123, "1,234,567.89123");
    try checkComma(123456.789123, "123,456.789123");
    try checkComma(0.123456789, "0.123456789");
    try checkComma(0.12345678, "0.12345678");
    try checkComma(0.1234567, "0.1234567");
}
