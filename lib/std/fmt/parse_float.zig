...
test "fmt.parseFloat" {
    inline for ([_]type{ f16, f32, f64, f128 }) |T| {
        ...
        try expectError(error.InvalidCharacter, parseFloat(T, ".")); // at least on digit is required
        try expectError(error.InvalidCharacter, parseFloat(T, ".e1")); // at least on digit is required
        try expectError(error.InvalidCharacter, parseFloat(T, "0.e")); // at least on digit is required
