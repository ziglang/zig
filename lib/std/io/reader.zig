...
const ReaderVtable = struct {
    context: *const anyopaque,
    readFn: *const fn (context: *const anyopaque, buffer: []u8) anyerror!usize,

    fn read(self: ReaderVtable, buffer: []u8) anyerror!usize {
        return self.readFn(self.context, buffer);
    }
};

pub const AnyReader = GenericReader(ReaderVtable, anyerror, ReaderVtable.read);
...
pub fn GenericReader(
    ...
    return struct {
        ...
        ) (error{StreamTooLong} || mem.Allocator.Error || Error)!void {
        ...
        pub fn readAllAlloc(
            ...
            max_size: usize,
        ) (Error || mem.Allocator.Error || error{StreamTooLong})![]u8 {
        ...
        pub fn readUntilDelimiterArrayList(
            ...
            delimiter: u8,
            max_size: usize,
        ) (NoEofError || mem.Allocator.Error || error{StreamTooLong})!void {
        ...
        pub fn readUntilDelimiterAlloc(
            ...
            delimiter: u8,
            max_size: usize,
        ) (NoEofError || mem.Allocator.Error || error{StreamTooLong})![]u8 {
            var array_list = std.ArrayList(u8).init(allocator);
            defer array_list.deinit();
            try self.streamUntilDelimiter(array_list.writer(), delimiter, max_size);
            return try array_list.toOwnedSlice();
        ...
        pub fn skipUntilDelimiterOrEof(
            self: Self,
            delimiter: u8,
        ) Error!void {
            while (true) {
        ...
        pub fn isBytes(self: Self, slice: []const u8) NoEofError!bool {
            var i: usize = 0;
            var matches = true;
