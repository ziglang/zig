...
pub const system = if (builtin.link_libc or native_os == .windows)
    ...
    .wasi => struct {
        ...
        //   @compileError.
        //
        // - no notion of file modes/permissions (yet).  The POSIX
        //   `mode_t` is void.
        //
        // - Immutable "current working directory".  So APIs to
        //   change the working directory will @compileError.
        //
        // - Currently no signals, mmap, fcntl, pipe, gethostname, fork,
        //   or pthread support.

        pub const PATH_MAX = std.fs.wasi.PATH_MAX;
        pub const STDIN_FILENO = std.fs.wasi.STDIN_FILENO;
        pub const STDOUT_FILENO = std.fs.wasi.STDOUT_FILENO;
        pub const STDERR_FILENO = std.fs.wasi.STDERR_FILENO;
        ...
        pub const O = packed struct(u32) {
            ...
            DSYNC: bool = false,
            NONBLOCK: bool = false,
            RSYNC: bool = false, // This is not very POSIX.
            SYNC: bool = false,
        ...
        // Wasi doesn't support file access modes, so these should be
        // null.  But code tries to do mathy operations with them, so
        // define some constants even if they're not used on Wasi.
