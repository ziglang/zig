...
pub fn parse(allocator: std.mem.Allocator, reader: anytype) (ParseError || @TypeOf(reader).NoEofError)!TimeZone {
...
pub fn deinit(self: *TimeZone, allocator: std.mem.Allocator) void {
    ...
    allocator.free(self.leapseconds);
...
test "slim" {
    ...
    var tz = try TimeZone.parse(std.testing.allocator, in_stream.reader());
    defer tz.deinit(std.testing.allocator);
...
test "fat" {
    const data = @embedFile("TimeZone/antarctica_davis.tzif");
    var in_stream = std.io.fixedBufferStream(data);
