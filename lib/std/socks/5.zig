...
pub const password = @import("./5/flow_password.zig");
...
pub const ClientGreeting = struct {
    ...
    /// The `ClientGreeting.auth` is allocated with `allocator`.
    pub fn deserialize(allocator: std.mem.Allocator, reader: anytype, maxAuthSize: usize) !ClientGreeting {
        const verOnly = try ClientGreeting.deserializeVer(reader);

        return try verOnly.deserializeAuthAlloc(allocator, reader, maxAuthSize);
    }
    ...
    pub fn deserializeAuthAlloc(self: ClientGreeting, allocator: std.mem.Allocator, reader: anytype, maxAuthSize: usize) !ClientGreeting {
        if (self.ver != 5) {
            return self;
        }
        ...
        const readSize = @min(authSize, maxAuthSize);
        const skipSize = authSize - maxAuthSize;

        const auth = try reader.readAlloc(allocator, readSize);
    ...
    pub fn deserializeAndPickAuth(reader: anytype, p: []const u8) !u8 {
        const verOnly = try ClientGreeting.deserializeVer(reader);
        ...
        while (restAuthLen > 0) : (restAuthLen -= 1) {
            ...
            if (std.mem.indexOfScalar(u8, p, auth)) |idx| {
    ...
    ///
    /// The `ClientGreeting.auth` length must be <= 255, or a undefined
