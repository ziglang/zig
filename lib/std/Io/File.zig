...
pub const ReadStreamingError = std.fs.File.ReadError || Io.Cancelable;
pub const ReadPositionalError = std.fs.File.PReadError || Io.Cancelable;
...
pub const Reader = struct {
    ...
    fn stream(io_reader: *Io.Reader, w: *Io.Writer, limit: Io.Limit) Io.Reader.StreamError!usize {
        ...
        switch (r.mode) {
            ...
            .positional_reading => {
                ...
                const n = try r.readPositional(dest);
                w.advance(n);
                return n;
            },
            .streaming_reading => {
                const dest = limit.slice(try w.writableSliceGreedy(1));
                const n = try r.readStreaming(dest);
                w.advance(n);
                return n;
            },
    ...
    fn readVec(io_reader: *Io.Reader, data: [][]u8) Io.Reader.Error!usize {
        ...
        switch (r.mode) {
            .positional, .positional_reading => {
                assert(io_reader.bufferedLen() == 0);
                var iovecs_buffer: [max_buffers_len][]u8 = undefined;
                const dest_n, const data_size = try io_reader.writableVectorSlices(&iovecs_buffer, data);
                const dest = iovecs_buffer[0..dest_n];
                assert(dest[0].len > 0);
                const n = io.vtable.fileRead(io.userdata, r.file, dest, r.pos) catch |err| switch (err) {
                    error.Unseekable => {
                        r.mode = r.mode.toStreaming();
                        const pos = r.pos;
                        if (pos != 0) {
            ...
            .streaming, .streaming_reading => {
                ...
                const dest = iovecs_buffer[0..dest_n];
                assert(dest[0].len > 0);
                const n = io.vtable.fileRead(io.userdata, r.file, dest, -1) catch |err| {
                    r.err = err;
                    return error.ReadFailed;
                };
                if (n == 0) {
    ...
    pub fn readPositional(r: *Reader, dest: []u8) Io.Reader.Error!usize {
    ...
    pub fn read(r: *Reader, dest: []u8) Io.Reader.Error!usize {
        switch (r.mode) {
            .positional, .positional_reading => return r.readPositional(dest),
            .streaming, .streaming_reading => return r.readStreaming(dest),
            .failure => return error.ReadFailed,
        }
    }
