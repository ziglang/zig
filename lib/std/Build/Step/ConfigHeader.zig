...
pub fn create(owner: *std.Build, options: Options) *ConfigHeader {
    ...
    if (options.header_text == null and options.style == .blank) {
        @panic("When using .blank style, header_text must be set to a syntax specific comment text, or explicitly set to the empty string");
    }
...
fn make(step: *Step, prog_node: *std.Progress.Node) !void {
    ...
    man.hash.addOptionalBytes(self.header_text);

    var output = std.ArrayList(u8).init(gpa);
    defer output.deinit();

    if (self.header_text) |t| {
        try output.appendSlice(t);
    } else {
        const default_header_text = "This file was generated by ConfigHeader using the Zig Build System.";
        switch (self.style) {
            .autoconf, .cmake, .blank => try output.appendSlice("/* " ++ default_header_text ++ " */\n"),
            .nasm => try output.appendSlice("; " ++ default_header_text ++ "\n"),
        }
    }
