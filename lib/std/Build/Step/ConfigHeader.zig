...
auto_generated_comment: ?[]const u8,
...
pub const Options = struct {
    ...
    auto_generated_comment: ?[]const u8 = null,
};
...
pub fn create(owner: *std.Build, options: Options) *ConfigHeader {
    ...
    config_header.* = .{
        ...
        .auto_generated_comment = options.auto_generated_comment,
...
fn make(step: *Step, options: Step.MakeOptions) !void {
    ...
    man.hash.addOptionalBytes(config_header.auto_generated_comment);
    ...
    if (config_header.auto_generated_comment) |comment| {
        try output.appendSlice(comment);
    } else {
        switch (config_header.style) {
            .autoconf, .cmake, .blank => try output.appendSlice(c_generated_line),
