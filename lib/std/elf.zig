...
pub const STT = enum(u4) {
    ...
    pub const STT_GNU_IFUNC = os(0x0);

    pub const STT_SPARC_REGISTER = proc(0x0);

    pub const STT_PARISC_MILLICODE = proc(0x0);

    pub const STT_HP_OPAQUE = os(0x1);
    pub const STT_HP_STUB = os(0x2);

    pub const STT_ARM_TFUNC = proc(0x0);
    pub const STT_ARM_16BIT = proc(0x2);
};
...
pub fn ProgramHeaderIterator(comptime ParseSource: anytype) type {
    return struct {
        ...
        pub fn next(self: *@This()) !?elf64.Phdr {
...
pub fn SectionHeaderIterator(comptime ParseSource: anytype) type {
    return struct {
        ...
        pub fn next(self: *@This()) !?elf64.Shdr {
...
pub const native_elf = switch (@sizeOf(usize)) {
    4 => elf32,
    8 => elf64,
