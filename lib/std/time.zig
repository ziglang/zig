...
test "date-day-of-week" {
    ...
    testing.expectEqual(date.dayOfWeek(), Weekday.Wednesday);
    ...
    testing.expectEqual(date.dayOfWeek(), Weekday.Tuesday);
...
pub const Datetime = struct {
    ...
    pub fn gt(self: Datetime, other: Datetime) bool {return self.cmp(other) == Compare.GreaterThan;}
    pub fn gte(self: Datetime, other: Datetime) bool {
        const r = self.cmp(other);
        return r == Compare.Equal or r == Compare.GreaterThan;
    }
    pub fn lt(self: Datetime, other: Datetime) bool {return self.cmp(other) == Compare.LessThan;}
...
test "datetime-subtract" {
     ...
     testing.expectEqual(delta.totalSeconds(), -3 * s_per_day);
     delta = b.sub(a);
     testing.expectEqual(delta.days, 3);
     testing.expectEqual(delta.totalSeconds(), 3 * s_per_day);
     ...
     delta = a.sub(b);
     testing.expectEqual(delta.totalSeconds(), 13 + 51* s_per_min);
}
