...
fn enforceMonotonic(current_now: u64) u64 {
    ...
    const Static = struct {
        var last_now: u64 = 0;
        var last_now_lock = std.sync.Lock{};
    };
    
    // If bi64 atomics are available, its generally cheaper to use them vs locking below.
    ...
    Static.last_now_lock.acquire();
    defer Static.last_now_lock.release();
...
fn readSystemTimer() ?u64 {
    if (target.os.tag == .windows) {
        const Static = struct {
            var frequency = std.sync.Once(os.windows.QueryPerformanceFrequency){};
            var counter_bias = std.sync.Once(os.windows.QueryPerformanceCounter){};
        };

        const frequency = Static.frequency.get();
        const bias = Static.counter_bias.get();
        const counter = os.windows.QueryPerformanceCounter();

        return safeMulDiv(counter -% bias, ns_per_s, frequency);
    }
