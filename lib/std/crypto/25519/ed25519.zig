    ...
    pub const KeyPair = struct {
        ...
        pub fn fromSecretKey(secret_key: SecretKey) (NonCanonicalError || EncodingError || IdentityElementError)!KeyPair {
            ...
            if (std.debug.runtime_safety) {
                ...
                const recomputed_kp = try init(secret_key.seed());
...
test "key pair creation" {
    ...
    const key_pair = try Ed25519.KeyPair.init(seed);
...
test "batch verification" {
    ...
    var i: usize = 0;
    while (i < 100) : (i += 1) {
        const key_pair = try initTestKeypair(Ed25519, &prng);
...
test "with blind keys" {
    ...
    const kp = try initTestKeypair(Ed25519, &prng);
