...
const State128L = struct {
    ...
    fn mac(state: *State128L, adlen: usize, mlen: usize) [16]u8 {
        ...
        return blocks[0].xorBlocks(blocks[1]).xorBlocks(blocks[2]).xorBlocks(blocks[3]).xorBlocks(blocks[4]).xorBlocks(blocks[5]).xorBlocks(blocks[6]).toBytes();
    }
};
...
const State256 = struct {
    ...
    fn mac(state: *State256, adlen: usize, mlen: usize) [16]u8 {
        ...
        return blocks[0].xorBlocks(blocks[1]).xorBlocks(blocks[2]).xorBlocks(blocks[3]).xorBlocks(blocks[4]).xorBlocks(blocks[5]).toBytes();
    }
};
