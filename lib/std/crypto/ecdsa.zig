...
pub fn Ecdsa(comptime Curve: type, comptime Hash: type) type {
    ...
    return struct {
        ...
        pub const Signature = struct {
            ...
            pub const FromDerError = der.Parser.Error || error{InvalidScalar};

            pub fn fromDer(bytes: []const u8) FromDerError!Signature {
...
test EcdsaSecp256k1Sha256oSha256 {
    if (builtin.zig_backend == .stage2_c) return error.SkipZigTest;
...
test "Ecdsa(crypto.ecc.P384, crypto.hash.sha2.Sha256)" {
    if (builtin.zig_backend == .stage2_c) return error.SkipZigTest;
...
test "Test vectors from Project Wycheproof" {
    ...
    for (wycheproof.test_groups) |g| {
        var key_sec1_: [EcdsaP256Sha256.PublicKey.uncompressed_sec1_encoded_length]u8 = undefined;
        const key_sec1 = try fmt.hexToBytes(&key_sec1_, g.key);
        const pk = try EcdsaP256Sha256.PublicKey.fromSec1(key_sec1);
...
fn tryWycheproof(pk: EcdsaP256Sha256.PublicKey, vector: wycheproof.Test) !void {
