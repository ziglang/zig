...
fn verifyRsa(
    ...
    const pk = rsa.PublicKey.fromDer(pub_key_der) catch |err| switch (err) {
        error.InsecureBitCount => return error.CertificateSignatureUnsupportedBitCount,
        else => return error.CertificatePublicKeyInvalid,
    };
    const sig = rsa.PKCS1v1_5(Hash).Signature{ .bytes = sig_bytes };
...
fn verify_ecdsa(
    ...
    switch (sig_named_curve) {
        ...
        => |curve| {
            ...
            const pub_key = Ecdsa.PublicKey.fromSec1(sec1_pub_key) catch return error.CertificateSignatureInvalid;
