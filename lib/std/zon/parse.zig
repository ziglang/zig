...
pub fn fromSlice(
    ...
    return fromSliceAlloc(T, gpa, source, status, options);
}
...
pub fn fromSliceAlloc(
    ...
    if (status) |s| s.* = .{};
    return fromZoirAlloc(T, gpa, ast, zoir, status, options);
}
...
pub fn fromZoirAlloc(
    ...
    return fromZoirNodeAlloc(T, gpa, ast, zoir, .root, status, options);
}
...
test "std.zon ast errors" {
    ...
    try std.testing.expectError(
        error.ParseZon,
        fromSliceAlloc(struct {}, gpa, ".{.x = 1 .y = 2}", &status, .{}),
    );
...
test "std.zon comments" {
    ...
    try std.testing.expectEqual(@as(u8, 10), fromSliceAlloc(u8, gpa,
        \\// comment
        \\10 // comment
        \\// comment
    , null, .{}));
...
test "std.zon fromSliceAlloc syntax error" {
    try std.testing.expectError(
        error.ParseZon,
        fromSliceAlloc(u8, std.testing.allocator, ".{", null, .{}),
    );
}
...
test "std.zon optional" {
    ...
    // Basic usage
    {
        ...
        const some = try fromSliceAlloc(?u32, gpa, "1", null, .{});
...
test "std.zon unions" {
    ...
    // Unions
    {
        ...
        const tagged_x = try fromSliceAlloc(Tagged, gpa, ".{.x = 1.5}", null, .{});
        try std.testing.expectEqual(Tagged{ .x = 1.5 }, tagged_x);
        const tagged_y = try fromSliceAlloc(Tagged, gpa, ".{.@\"y y\" = true}", null, .{});
        try std.testing.expectEqual(Tagged{ .@"y y" = true }, tagged_y);
        const tagged_z_shorthand = try fromSliceAlloc(Tagged, gpa, ".z", null, .{});
        try std.testing.expectEqual(@as(Tagged, .z), tagged_z_shorthand);
        const tagged_zz_shorthand = try fromSliceAlloc(Tagged, gpa, ".@\"z z\"", null, .{});
        try std.testing.expectEqual(@as(Tagged, .@"z z"), tagged_zz_shorthand);

        const untagged_x = try fromSliceAlloc(Untagged, gpa, ".{.x = 1.5}", null, .{});
        try std.testing.expect(untagged_x.x == 1.5);
        const untagged_y = try fromSliceAlloc(Untagged, gpa, ".{.@\"y y\" = true}", null, .{});
    ...
    // Unknown field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Union, gpa, ".{.z=2.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Explicit void field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Union, gpa, ".{.x=1}", &status, .{}),
        );
    ...
    // Extra field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Union, gpa, ".{.x = 1.5, .y = true}", &status, .{}),
        );
    ...
    // No fields
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Union, gpa, ".{}", &status, .{}),
        );
        try std.testing.expectFmt("1:2: error: expected union\n", "{}", .{status});
    }
    ...
    // Enum literals cannot coerce into untagged unions
    {
        ...
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(Union, gpa, ".x", &status, .{}));
        try std.testing.expectFmt("1:2: error: expected union\n", "{}", .{status});
    }
    ...
    // Unknown field for enum literal coercion
    {
        ...
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(Union, gpa, ".y", &status, .{}));
        try std.testing.expectFmt(
...
test "std.zon structs" {
    ...
    // Structs (various sizes tested since they're parsed differently)
    {
        ...
        const zero = try fromSliceAlloc(Vec0, gpa, ".{}", null, .{});
        try std.testing.expectEqual(Vec0{}, zero);

        const one = try fromSliceAlloc(Vec1, gpa, ".{.x = 1.2}", null, .{});
        try std.testing.expectEqual(Vec1{ .x = 1.2 }, one);

        const two = try fromSliceAlloc(Vec2, gpa, ".{.x = 1.2, .y = 3.4}", null, .{});
        try std.testing.expectEqual(Vec2{ .x = 1.2, .y = 3.4 }, two);

        const three = try fromSliceAlloc(Vec3, gpa, ".{.x = 1.2, .y = 3.4, .z = 5.6}", null, .{});
    ...
    // Unknown field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Vec2, gpa, ".{.x=1.5, .z=2.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Duplicate field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Vec2, gpa, ".{.x=1.5, .x=2.5, .x=3.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Ignore unknown fields
    {
        ...
        const parsed = try fromSliceAlloc(Vec2, gpa, ".{ .x = 1.0, .z = 3.0 }", null, .{
    ...
    // Missing field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Vec2, gpa, ".{.x=1.5}", &status, .{}),
        );
    ...
    // Default field
    {
        ...
        const parsed = try fromSliceAlloc(Vec2, gpa, ".{.x = 1.2}", null, .{});
    ...
    // Comptime field assignment
    {
        ...
        var status: Status = .{};
        defer status.deinit(gpa);
        const parsed = fromSliceAlloc(Vec2, gpa, ".{.x = 1.2, .y = 1.5}", &status, .{});
    ...
    // incorrect way that broke for enum values)
    {
        ...
        const parsed = try fromSliceAlloc(Vec0, gpa, ".{ .x = .x }", null, .{});
    ...
    // Enum field and struct field with @
    {
        ...
        const parsed = try fromSliceAlloc(Vec0, gpa, ".{ .@\"x x\" = .@\"x x\" }", null, .{});
    ...
    // Type expressions are not allowed
    {
        // Structs
        {
            var status: Status = .{};
            defer status.deinit(gpa);
            const parsed = fromSliceAlloc(struct {}, gpa, "Empty{}", &status, .{});
            try std.testing.expectError(error.ParseZon, parsed);
            try std.testing.expectFmt(
        ...
        // Arrays
        {
            var status: Status = .{};
            defer status.deinit(gpa);
            const parsed = fromSliceAlloc([3]u8, gpa, "[3]u8{1, 2, 3}", &status, .{});
            try std.testing.expectError(error.ParseZon, parsed);
            try std.testing.expectFmt(
                \\1:1: error: types are not available in ZON
                \\1:1: note: replace the type with '.'
        ...
        // Slices
        {
            var status: Status = .{};
            defer status.deinit(gpa);
            const parsed = fromSliceAlloc([]u8, gpa, "[]u8{1, 2, 3}", &status, .{});
            try std.testing.expectError(error.ParseZon, parsed);
            try std.testing.expectFmt(
                \\1:1: error: types are not available in ZON
                \\1:1: note: replace the type with '.'
        ...
        // Nested
        {
            var status: Status = .{};
            defer status.deinit(gpa);
            const parsed = fromSliceAlloc(struct {}, gpa, ".{ .x = Tuple{1, 2, 3} }", &status, .{});
            try std.testing.expectError(error.ParseZon, parsed);
            try std.testing.expectFmt(
...
test "std.zon tuples" {
    ...
    // Structs (various sizes tested since they're parsed differently)
    {
        ...
        const zero = try fromSliceAlloc(Tuple0, gpa, ".{}", null, .{});
        try std.testing.expectEqual(Tuple0{}, zero);

        const one = try fromSliceAlloc(Tuple1, gpa, ".{1.2}", null, .{});
        try std.testing.expectEqual(Tuple1{1.2}, one);

        const two = try fromSliceAlloc(Tuple2, gpa, ".{1.2, true}", null, .{});
        try std.testing.expectEqual(Tuple2{ 1.2, true }, two);

        const three = try fromSliceAlloc(Tuple3, gpa, ".{1.2, false, 3}", null, .{});
    ...
    // Extra field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Tuple, gpa, ".{0.5, true, 123}", &status, .{}),
        );
    ...
    // Extra field
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Tuple, gpa, ".{0.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Tuple with unexpected field names
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Tuple, gpa, ".{.foo = 10.0}", &status, .{}),
        );
    ...
    // Struct with missing field names
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Struct, gpa, ".{10.0}", &status, .{}),
        );
    ...
    // Comptime field
    {
        ...
        const parsed = try fromSliceAlloc(Vec2, gpa, ".{ 1.2 }", null, .{});
    ...
    // Comptime field assignment
    {
        ...
        var status: Status = .{};
        defer status.deinit(gpa);
        const parsed = fromSliceAlloc(Vec2, gpa, ".{ 1.2, 1.5}", &status, .{});
        try std.testing.expectError(error.ParseZon, parsed);
        try std.testing.expectFmt(
...
test "std.zon arrays and slices" {
    ...
    // Literals
    {
        // Arrays
        {
            const zero = try fromSliceAlloc([0]u8, gpa, ".{}", null, .{});
            try std.testing.expectEqualSlices(u8, &@as([0]u8, .{}), &zero);

            const one = try fromSliceAlloc([1]u8, gpa, ".{'a'}", null, .{});
            try std.testing.expectEqualSlices(u8, &@as([1]u8, .{'a'}), &one);

            const two = try fromSliceAlloc([2]u8, gpa, ".{'a', 'b'}", null, .{});
            try std.testing.expectEqualSlices(u8, &@as([2]u8, .{ 'a', 'b' }), &two);

            const two_comma = try fromSliceAlloc([2]u8, gpa, ".{'a', 'b',}", null, .{});
            try std.testing.expectEqualSlices(u8, &@as([2]u8, .{ 'a', 'b' }), &two_comma);

            const three = try fromSliceAlloc([3]u8, gpa, ".{'a', 'b', 'c'}", null, .{});
            try std.testing.expectEqualSlices(u8, &.{ 'a', 'b', 'c' }, &three);

            const sentinel = try fromSliceAlloc([3:'z']u8, gpa, ".{'a', 'b', 'c'}", null, .{});
    ...
    // Sentinels and alignment
    {
        // Arrays
        {
            const sentinel = try fromSliceAlloc([1:2]u8, gpa, ".{1}", null, .{});
    ...
    // Expect 0 find 3
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([0]u8, gpa, ".{'a', 'b', 'c'}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Expect 1 find 2
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([1]u8, gpa, ".{'a', 'b'}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Expect 2 find 1
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([2]u8, gpa, ".{'a'}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Expect 3 find 0
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([3]u8, gpa, ".{}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Wrong inner type
    {
        // Array
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([3]bool, gpa, ".{'a', 'b', 'c'}", &status, .{}),
            );
        ...
        // Slice
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([]bool, gpa, ".{'a', 'b', 'c'}", &status, .{}),
            );
            try std.testing.expectFmt("1:3: error: expected type 'bool'\n", "{}", .{status});
        }
    }
    ...
    // Complete wrong type
    {
        // Array
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([3]u8, gpa, "'a'", &status, .{}),
            );
        ...
        // Slice
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([]u8, gpa, "'a'", &status, .{}),
            );
            try std.testing.expectFmt("1:1: error: expected array\n", "{}", .{status});
        }
    }
    ...
    // Address of is not allowed (indirection for slices in ZON is implicit)
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([]u8, gpa, "  &.{'a', 'b', 'c'}", &status, .{}),
        );
        try std.testing.expectFmt(
...
test "std.zon string literal" {
    ...
    {
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([]u8, gpa, "\"abcd\"", &status, .{}),
            );
            try std.testing.expectFmt("1:1: error: expected array\n", "{}", .{status});
        }
    ...
    {
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([4:0]u8, gpa, "\"abcd\"", &status, .{}),
            );
            try std.testing.expectFmt("1:1: error: expected array\n", "{}", .{status});
        }
    ...
    {
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([:1]const u8, gpa, "\"foo\"", &status, .{}),
            );
            try std.testing.expectFmt("1:1: error: expected array\n", "{}", .{status});
        }
    ...
    // Expecting string literal, getting something else
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([]const u8, gpa, "true", &status, .{}),
        );
    ...
    // Expecting string literal, getting an incompatible tuple
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([]const u8, gpa, ".{false}", &status, .{}),
        );
    ...
    // Invalid string literal
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc([]const i8, gpa, "\"\\a\"", &status, .{}),
        );
    ...
    {
        {
            ...
            try std.testing.expectError(
                error.ParseZon,
                fromSliceAlloc([]align(2) const u8, gpa, "\"abc\"", &status, .{}),
            );
            try std.testing.expectFmt("1:1: error: expected array\n", "{}", .{status});
        }
...
test "std.zon enum literals" {
    ...
    // Tags that exist
    try std.testing.expectEqual(Enum.foo, try fromSliceAlloc(Enum, gpa, ".foo", null, .{}));
    try std.testing.expectEqual(Enum.bar, try fromSliceAlloc(Enum, gpa, ".bar", null, .{}));
    try std.testing.expectEqual(Enum.baz, try fromSliceAlloc(Enum, gpa, ".baz", null, .{}));
    try std.testing.expectEqual(
        Enum.@"ab\nc",
        try fromSliceAlloc(Enum, gpa, ".@\"ab\\nc\"", null, .{}),
    );
    ...
    // Bad tag
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Enum, gpa, ".qux", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Bad tag that's too long for parser
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Enum, gpa, ".@\"foobarbaz\"", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Bad type
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Enum, gpa, "true", &status, .{}),
        );
    ...
    // Test embedded nulls in an identifier
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(Enum, gpa, ".@\"\\x00\"", &status, .{}),
        );
        try std.testing.expectFmt(
...
test "std.zon parse bool" {
    ...
    // Correct floats
    try std.testing.expectEqual(true, try fromSliceAlloc(bool, gpa, "true", null, .{}));
    try std.testing.expectEqual(false, try fromSliceAlloc(bool, gpa, "false", null, .{}));
    ...
    // Errors
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(bool, gpa, " foo", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    }
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(bool, gpa, "123", &status, .{}));
...
test "std.zon parse int" {
    ...
    try std.testing.expectEqual(@as(u8, 10), try fromSliceAlloc(u8, gpa, "10", null, .{}));
    try std.testing.expectEqual(@as(i16, 24), try fromSliceAlloc(i16, gpa, "24", null, .{}));
    try std.testing.expectEqual(@as(i14, -4), try fromSliceAlloc(i14, gpa, "-4", null, .{}));
    try std.testing.expectEqual(@as(i32, -123), try fromSliceAlloc(i32, gpa, "-123", null, .{}));

    // Test limits
    try std.testing.expectEqual(@as(i8, 127), try fromSliceAlloc(i8, gpa, "127", null, .{}));
    try std.testing.expectEqual(@as(i8, -128), try fromSliceAlloc(i8, gpa, "-128", null, .{}));

    // Test characters
    try std.testing.expectEqual(@as(u8, 'a'), try fromSliceAlloc(u8, gpa, "'a'", null, .{}));
    try std.testing.expectEqual(@as(u8, 'z'), try fromSliceAlloc(u8, gpa, "'z'", null, .{}));

    // Test big integers
    try std.testing.expectEqual(
        @as(u65, 36893488147419103231),
        try fromSliceAlloc(u65, gpa, "36893488147419103231", null, .{}),
    );
    try std.testing.expectEqual(
        @as(u65, 36893488147419103231),
        try fromSliceAlloc(u65, gpa, "368934_881_474191032_31", null, .{}),
    );

    // Test big integer limits
    try std.testing.expectEqual(
        @as(i66, 36893488147419103231),
        try fromSliceAlloc(i66, gpa, "36893488147419103231", null, .{}),
    );
    try std.testing.expectEqual(
        @as(i66, -36893488147419103232),
        try fromSliceAlloc(i66, gpa, "-36893488147419103232", null, .{}),
    );
    {
        var status: Status = .{};
        defer status.deinit(gpa);
    ...
    try std.testing.expectEqual(@as(i8, -1), try fromSliceAlloc(i8, gpa, "-1.0", null, .{}));
    try std.testing.expectEqual(@as(i8, 123), try fromSliceAlloc(i8, gpa, "123.0", null, .{}));

    // Test non-decimal integers
    try std.testing.expectEqual(@as(i16, 0xff), try fromSliceAlloc(i16, gpa, "0xff", null, .{}));
    try std.testing.expectEqual(@as(i16, -0xff), try fromSliceAlloc(i16, gpa, "-0xff", null, .{}));
    try std.testing.expectEqual(@as(i16, 0o77), try fromSliceAlloc(i16, gpa, "0o77", null, .{}));
    try std.testing.expectEqual(@as(i16, -0o77), try fromSliceAlloc(i16, gpa, "-0o77", null, .{}));
    try std.testing.expectEqual(@as(i16, 0b11), try fromSliceAlloc(i16, gpa, "0b11", null, .{}));
    try std.testing.expectEqual(@as(i16, -0b11), try fromSliceAlloc(i16, gpa, "-0b11", null, .{}));

    // Test non-decimal big integers
    try std.testing.expectEqual(@as(u65, 0x1ffffffffffffffff), try fromSliceAlloc(
        u65,
        gpa,
        "0x1ffffffffffffffff",
        null,
    ...
    try std.testing.expectEqual(@as(i66, 0x1ffffffffffffffff), try fromSliceAlloc(
        i66,
        gpa,
        "0x1ffffffffffffffff",
        null,
    ...
    // Number with invalid character in the middle
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "32a32", &status, .{}));
        try std.testing.expectFmt(
    ...
    // Failing to parse as int
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "true", &status, .{}));
    ...
    // Failing because an int is out of range
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "256", &status, .{}));
        try std.testing.expectFmt(
    ...
    // Failing because a negative int is out of range
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(i8, gpa, "-129", &status, .{}));
        try std.testing.expectFmt(
    ...
    // Failing because an unsigned int is negative
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "-1", &status, .{}));
        try std.testing.expectFmt(
            "1:1: error: type 'u8' cannot represent value\n",
            "{}",
            .{status},
    ...
    // Failing because a float is non-whole
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "1.5", &status, .{}));
        try std.testing.expectFmt(
            "1:1: error: type 'u8' cannot represent value\n",
            "{}",
            .{status},
    ...
    // Failing because a float is negative
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "-1.0", &status, .{}));
        try std.testing.expectFmt(
            "1:1: error: type 'u8' cannot represent value\n",
            "{}",
            .{status},
    ...
    // Negative integer zero
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(i8, gpa, "-0", &status, .{}));
        try std.testing.expectFmt(
    ...
    // Negative integer zero casted to float
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(f32, gpa, "-0", &status, .{}));
        try std.testing.expectFmt(
            \\1:2: error: integer literal '-0' is ambiguous
            \\1:2: note: use '0' for an integer zero
            \\1:2: note: use '-0.0' for a floating-point signed zero
    ...
    try std.testing.expect(
        std.math.isNegativeZero(try fromSliceAlloc(f32, gpa, "-0.0", null, .{})),
    );
    try std.testing.expect(std.math.isPositiveZero(try fromSliceAlloc(f32, gpa, "0.0", null, .{})));

    // Double negation is not allowed
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(i8, gpa, "--2", &status, .{}));
        try std.testing.expectFmt(
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(f32, gpa, "--2.0", &status, .{}),
        );
        try std.testing.expectFmt(
            "1:1: error: expected number or 'inf' after '-'\n",
            "{}",
    ...
    // Invalid int literal
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "0xg", &status, .{}));
    ...
    // Notes on invalid int literal
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(u8, gpa, "0123", &status, .{}));
        try std.testing.expectFmt(
...
test "std.zon negative char" {
...

    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(f32, gpa, "-'a'", &status, .{}));
        try std.testing.expectFmt(
            "1:1: error: expected number or 'inf' after '-'\n",
            "{}",
            .{status},
    ...
    }
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(i16, gpa, "-'a'", &status, .{}));
        try std.testing.expectFmt(
            "1:1: error: expected number or 'inf' after '-'\n",
            "{}",
            .{status},
...
test "std.zon parse float" {
    ...
    // Test decimals
    try std.testing.expectEqual(@as(f16, 0.5), try fromSliceAlloc(f16, gpa, "0.5", null, .{}));
    try std.testing.expectEqual(
        @as(f32, 123.456),
        try fromSliceAlloc(f32, gpa, "123.456", null, .{}),
    );
    try std.testing.expectEqual(
        @as(f64, -123.456),
        try fromSliceAlloc(f64, gpa, "-123.456", null, .{}),
    );
    try std.testing.expectEqual(@as(f128, 42.5), try fromSliceAlloc(f128, gpa, "42.5", null, .{}));

    // Test whole numbers with and without decimals
    try std.testing.expectEqual(@as(f16, 5.0), try fromSliceAlloc(f16, gpa, "5.0", null, .{}));
    try std.testing.expectEqual(@as(f16, 5.0), try fromSliceAlloc(f16, gpa, "5", null, .{}));
    try std.testing.expectEqual(@as(f32, -102), try fromSliceAlloc(f32, gpa, "-102.0", null, .{}));
    try std.testing.expectEqual(@as(f32, -102), try fromSliceAlloc(f32, gpa, "-102", null, .{}));

    // Test characters and negated characters
    try std.testing.expectEqual(@as(f32, 'a'), try fromSliceAlloc(f32, gpa, "'a'", null, .{}));
    try std.testing.expectEqual(@as(f32, 'z'), try fromSliceAlloc(f32, gpa, "'z'", null, .{}));

    // Test big integers
    try std.testing.expectEqual(
        @as(f32, 36893488147419103231),
        try fromSliceAlloc(f32, gpa, "36893488147419103231", null, .{}),
    );
    try std.testing.expectEqual(
        @as(f32, -36893488147419103231),
        try fromSliceAlloc(f32, gpa, "-36893488147419103231", null, .{}),
    );
    try std.testing.expectEqual(@as(f128, 0x1ffffffffffffffff), try fromSliceAlloc(
        f128,
        gpa,
        "0x1ffffffffffffffff",
        null,
    ...
    try std.testing.expectEqual(@as(f32, 0x1ffffffffffffffff), try fromSliceAlloc(
        f32,
        gpa,
        "0x1ffffffffffffffff",
        null,
    ...
    try std.testing.expectEqual(
        @as(f32, 123.0E+77),
        try fromSliceAlloc(f32, gpa, "12_3.0E+77", null, .{}),
    );

    // Hexadecimal
    try std.testing.expectEqual(
        @as(f32, 0x103.70p-5),
        try fromSliceAlloc(f32, gpa, "0x103.70p-5", null, .{}),
    );
    try std.testing.expectEqual(
        @as(f32, -0x103.70),
        try fromSliceAlloc(f32, gpa, "-0x103.70", null, .{}),
    );
    try std.testing.expectEqual(
        @as(f32, 0x1234_5678.9ABC_CDEFp-10),
        try fromSliceAlloc(f32, gpa, "0x1234_5678.9ABC_CDEFp-10", null, .{}),
    );

    // inf, nan
    try std.testing.expect(std.math.isPositiveInf(try fromSliceAlloc(f32, gpa, "inf", null, .{})));
    try std.testing.expect(std.math.isNegativeInf(try fromSliceAlloc(f32, gpa, "-inf", null, .{})));
    try std.testing.expect(std.math.isNan(try fromSliceAlloc(f32, gpa, "nan", null, .{})));

    // Negative nan not allowed
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(f32, gpa, "-nan", &status, .{}));
        try std.testing.expectFmt(
            "1:1: error: expected number or 'inf' after '-'\n",
            "{}",
            .{status},
    ...
    // nan as int not allowed
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(i8, gpa, "nan", &status, .{}));
    ...
    // inf as int not allowed
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(i8, gpa, "inf", &status, .{}));
        try std.testing.expectFmt("1:1: error: expected type 'i8'\n", "{}", .{status});
    }
    ...
    // Bad identifier as float
    {
        var status: Status = .{};
        defer status.deinit(gpa);
        try std.testing.expectError(error.ParseZon, fromSliceAlloc(f32, gpa, "foo", &status, .{}));
        try std.testing.expectFmt(
    ...
    // Non float as float
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(f32, gpa, "\"foo\"", &status, .{}),
        );
...
test "std.zon free on error" {
    ...
    try std.testing.expectEqual(
        @as(f32, 1.5),
        (try fromSliceAlloc(union { x: f32 }, std.testing.allocator, ".{ .x = 1.5 }", null, .{})).x,
    );
...
test "std.zon vector" {
    ...
    try std.testing.expectEqual(
        @Vector(0, bool){},
        try fromSliceAlloc(@Vector(0, bool), gpa, ".{}", null, .{}),
    );
    try std.testing.expectEqual(
        @Vector(3, bool){ true, false, true },
        try fromSliceAlloc(@Vector(3, bool), gpa, ".{true, false, true}", null, .{}),
    );

    try std.testing.expectEqual(
        @Vector(0, f32){},
        try fromSliceAlloc(@Vector(0, f32), gpa, ".{}", null, .{}),
    );
    try std.testing.expectEqual(
        @Vector(3, f32){ 1.5, 2.5, 3.5 },
        try fromSliceAlloc(@Vector(3, f32), gpa, ".{1.5, 2.5, 3.5}", null, .{}),
    );

    try std.testing.expectEqual(
        @Vector(0, u8){},
        try fromSliceAlloc(@Vector(0, u8), gpa, ".{}", null, .{}),
    );
    try std.testing.expectEqual(
        @Vector(3, u8){ 2, 4, 6 },
        try fromSliceAlloc(@Vector(3, u8), gpa, ".{2, 4, 6}", null, .{}),
    );
    ...
    // Too few fields
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(@Vector(2, f32), gpa, ".{0.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Too many fields
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(@Vector(2, f32), gpa, ".{0.5, 1.5, 2.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Wrong type fields
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(@Vector(3, f32), gpa, ".{0.5, true, 2.5}", &status, .{}),
        );
        try std.testing.expectFmt(
    ...
    // Wrong type
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(@Vector(3, u8), gpa, "true", &status, .{}),
        );
    ...
    // Elements should get freed on error
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(@Vector(3, *u8), gpa, ".{1, true, 3}", &status, .{}),
        );
...
test "std.zon add pointers" {
    ...
    // Test that optional types are flattened correctly in errors
    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const u8, gpa, "true", &status, .{}),
        );
        try std.testing.expectFmt("1:1: error: expected type '?u8'\n", "{}", .{status});
    }

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const f32, gpa, "true", &status, .{}),
        );
        try std.testing.expectFmt("1:1: error: expected type '?f32'\n", "{}", .{status});
    }

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const @Vector(3, u8), gpa, "true", &status, .{}),
        );
        try std.testing.expectFmt("1:1: error: expected type '?@Vector(3, u8)'\n", "{}", .{status});
    }

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const bool, gpa, "10", &status, .{}),
        );
        try std.testing.expectFmt("1:1: error: expected type '?bool'\n", "{}", .{status});
    }

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const struct { a: i32 }, gpa, "true", &status, .{}),
        );
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const struct { i32 }, gpa, "true", &status, .{}),
        );
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const union { x: void }, gpa, "true", &status, .{}),
        );
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const [3]u8, gpa, "true", &status, .{}),
        );
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(?[3]u8, gpa, "true", &status, .{}),
        );
        try std.testing.expectFmt("1:1: error: expected optional array\n", "{}", .{status});
    }
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(?[]u8, gpa, "true", &status, .{}),
        );
        try std.testing.expectFmt("1:1: error: expected optional array\n", "{}", .{status});
    }

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const []const u8, gpa, "true", &status, .{}),
        );
...

    {
        ...
        try std.testing.expectError(
            error.ParseZon,
            fromSliceAlloc(*const ?*const enum { foo }, gpa, "true", &status, .{}),
        );
