...
pub const Dir = struct {
    ...
    pub const Iterator = switch (builtin.os) {
        .macosx, .ios, .freebsd, .netbsd, .dragonfly => struct {
            ...
            fn nextDragonFly(self: *Self) !?Entry {
                start_over: while (true) {
                    if (self.index >= self.end_index) {
                    ...
                    const dragonfly_entry = @ptrCast(*align(1) os.dirent, &self.buf[self.index]);
                    const next_index = self.index + ((@byteOffsetOf(os.dirent, "d_name") + dragonfly_entry.d_namlen + 1 + 7) & ~usize(7));
                    self.index = next_index;
