...
/// A common format for the Sigaction struct across a variety of C libraries.
const common_libc_Sigaction = extern struct {
...
pub const sigset_t = switch (native_os) {
    ...
    .openbsd, .ios, .tvos, .watchos, .visionos, .serenity => u32,
    .macos => darwin.sigset_t,
...
/// Zig's constant, pre-cleared empty sigset_t, so invoking `sigemptyset`
/// is unnecessary.
pub const empty_sigset: sigset_t = switch (native_os) {
    .linux => [_]c_ulong{0} ** @typeInfo(sigset_t).array.len,
    .emscripten => emscripten.empty_sigset,
    .dragonfly, .netbsd, .solaris, .illumos, .freebsd => .{ .__bits = [_]u32{0} ** SIG.WORDS },
    else => 0,
};

// No pre-filled `filled_sigset` because it requires running C library
// code to initialize.  The C library may reserve some signals for its own
// use and leave them out of the 'filled' set.
