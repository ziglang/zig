...
fn logOverride(
    ...
    const log_file = exec.cache_tmp.createFile("libfuzzer.log", .{
        ...
        panic("failed to open fuzzer log file 'libfuzzer.log': {s}", .{@errorName(e)});
    defer log_file.close();
    log_file.seekFromEnd(0) catch |e|
        panic("failed to seek fuzzer log file 'libfuzzer.log': {s}", .{@errorName(e)});

    const prefix1 = comptime level.asText();
    const prefix2 = if (scope == .default) ": " else "(" ++ @tagName(scope) ++ "): ";
    log_file.writer().print(
        "[{s}] " ++ prefix1 ++ prefix2 ++ format ++ "\n",
        .{current_test_name orelse "none"} ++ args,
    ) catch |e|
        panic("failed to write to fuzzer log file 'libfuzzer.log': {s}", .{@errorName(e)});
}
...
const Executable = struct {
    ...
    fn getCoverageFile(cache_dir: std.fs.Dir, pcs: []const usize, pc_digest: u64) MemoryMappedList {
        ...
        var v = cache_dir.makeOpenPath("v", .{}) catch |e|
            panic("failed to create directory 'v': {s}", .{@errorName(e)});
        defer v.close();
        const coverage_file, const populate = if (v.createFile(&coverage_file_name, .{
            ...
            error.PathAlreadyExists => .{ v.openFile(&coverage_file_name, .{
                ...
                "failed to open existing coverage file '{s}': {s}",
    ...
    pub fn init(cache_dir_path: []const u8) Executable {
        ...
        const cache_dir = std.fs.cwd().makeOpenPath(cache_dir_path, .{}) catch |e| panic(
            ...
            .{ cache_dir_path, @errorName(e) },
        );
        self.cache_tmp = cache_dir.makeOpenPath("tmp", .{}) catch |e|
            panic("failed to open directory 'tmp': {s}", .{@errorName(e)});
        self.cache_f = cache_dir.makeOpenPath("f", .{}) catch |e|
            panic("failed to open directory 'f': {s}", .{@errorName(e)});
        ...
        self.initRodata() catch |e| if (e != error.UnsupportedObjectFormat) std.log.err(
            \\failed to enumerate read-only memory: {s}
...
const Fuzzer = struct {
    ...
    pub fn init(test_one: TestOne, unit_test_name: []const u8) Fuzzer {
        ...
        self.corpus_dir = exec.cache_f.makeOpenPath(unit_test_name, .{}) catch |e|
            panic("failed to open directory '{s}': {s}", .{ unit_test_name, @errorName(e) });
        self.input = in: {
            const f = self.corpus_dir.createFile("in", .{
                .read = true,
                .truncate = false,
            }) catch |e|
                panic("failed to create input file 'in': {s}", .{@errorName(e)});
            const size = f.getEndPos() catch |e|
                panic("failed to stat input file 'in': {s}", .{@errorName(e)});
            const map = (if (size < std.heap.page_size_max)
                ...
                panic("failed to memory map input file 'in': {s}", .{@errorName(e)});
    ...
    pub fn addInput(self: *Fuzzer, bytes: []const u8) void {
        ...
        self.input.ensureTotalCapacity(8 + bytes.len) catch |e|
            panic("could not resize shared input file: {s}", .{@errorName(e)});
