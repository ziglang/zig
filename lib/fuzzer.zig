...
fn logOverride(
    ...
    const f = (log_dir orelse panic(
        "attempt to use log before initialization, message:\n" ++ format,
        args,
    )).createFile("libfuzzer.log", .{
...
const Executable = struct {
    ...
    pub fn init(cache_dir_path: []const u8) Executable {
        ...
        log_dir = cache_dir.makeOpenPath("tmp", .{}) catch |e|
