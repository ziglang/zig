...
fn logOverride(
    ...
    const f = exec.cache_tmp.createFile("libfuzzer.log", .{
        .truncate = false, // appending
        .lock = .exclusive,
    }) catch |e| panic("failed to open fuzzer log file: {t}", .{e});
    defer f.close();
...
const Executable = struct {
        ...
        switch (builtin.object_format) {
            .elf => {
                ...
                while (try shdr_it.next()) |shdr| {
                    ...
                    var read_buf: [rodata_name.len]u8 = undefined;
                    var r = exec_file.reader(&read_buf);
                    try r.seekTo(try math.add(u64, str_tab_off, shdr.sh_name));

                    const section_name = r.interface.take(rodata_name.len) catch return r.err.?;
    ...
    pub fn init(cache_dir_path: []const u8) Executable {
        ...
        self.cache_tmp = cache_dir.makeOpenPath("tmp", .{}) catch |e|
            panic("failed to open directory 'tmp': {t}", .{e});
