        ...
        pub fn processContainerMemberFnsList(c: *ScopeExtraContext, list: std.ArrayList(ContainerMemberFns)) !void {
            for (list.items) |item| {
                ...
                for (item.member_fns.items, 0..) |func, i| {
                    ...
                    const last_name = if (last_index) |index| func_name[index + 1 ..] else func_name;
                    const payload = try c.arena.create(ast.Payload.SimpleVarDecl);
                    payload.* = .{
                        .base = .{ .tag = decl_tag },
                        .data = .{
                    ...
                    func_ref_vars[i] = ast.Node.initPayload(&payload.base);
                }
                container_data.variables = new_variables;
            }
        }
    };
}
