...
fn walkMember(w: *Walk, decl: Ast.Node.Index) Error!void {
    ...
    switch (ast.nodeTag(decl)) {
        .fn_decl => {
            const fn_proto = ast.nodeData(decl).lhs.node;
            try walkExpression(w, fn_proto);
            const body_node = ast.nodeData(decl).rhs.node;
        ...
        .@"usingnamespace" => {
            ...
            const expr = ast.nodeData(decl).lhs.node;
        ...
        .test_decl => {
            try w.transformations.append(.{ .delete_node = decl });
            try walkExpression(w, ast.nodeData(decl).rhs.node);
        },
...
fn walkExpression(w: *Walk, node: Ast.Node.Index) Error!void {
    ...
    switch (ast.nodeTag(node)) {
        ...
        .@"errdefer" => {
            const expr = ast.nodeData(node).rhs.node;
        ...
        .@"comptime", .@"nosuspend" => {
            const block = ast.nodeData(node).lhs.node;
            return walkExpression(w, block);
        },

        .@"suspend" => {
            const body = ast.nodeData(node).lhs.node;
            return walkExpression(w, body);
        },

        .@"catch" => {
            try walkExpression(w, ast.nodeData(node).lhs.node); // target
            try walkExpression(w, ast.nodeData(node).rhs.node); // fallback
        },

        .field_access => {
            const field_access = ast.nodeData(node);
            try walkExpression(w, field_access.lhs.node);
        },

        .error_union,
        .switch_range,
        => {
            const infix = ast.nodeData(node);
            try walkExpression(w, infix.lhs.node);
            return walkExpression(w, infix.rhs.node);
        },
        .for_range => {
            const infix = ast.nodeData(node);
            try walkExpression(w, infix.lhs.node);
        ...
        => {
            const infix = ast.nodeData(node);
            try walkExpression(w, infix.lhs.node);
            try walkExpression(w, infix.rhs.node);
        },
        ...
        .address_of,
        => {
            return walkExpression(w, ast.nodeData(node).lhs.node);
        },
        ...
        .array_access => {
            const suffix = ast.nodeData(node);
            try walkExpression(w, suffix.lhs.node);
            try walkExpression(w, suffix.rhs.node);
        },

        .slice_open, .slice, .slice_sentinel => return walkSlice(w, node, ast.fullSlice(node).?),

        .deref => {
            try walkExpression(w, ast.nodeData(node).lhs.node);
        },
        ...
        .@"break" => {
            const label_token = ast.nodeData(node).lhs.opt_token;
            const target = ast.nodeData(node).rhs.opt_node;
        ...
        .@"continue" => {
            if (ast.nodeData(node).lhs.opt_token.unwrap()) |label| {
                return walkIdentifier(w, label);
            }
        },

        .@"return" => {
            if (ast.nodeData(node).lhs.opt_node.unwrap()) |lhs| {
        ...
        .error_set_decl => {
            ...
            const rbrace = ast.nodeData(node).rhs.token;
        ...
        .anyframe_type => {
            return walkExpression(w, ast.nodeData(node).rhs.node);
        },
...
fn categorizeStmt(ast: *const Ast, stmt: Ast.Node.Index) StmtCategory {
    switch (ast.nodeTag(stmt)) {
        ...
        .assign => {
            const infix = ast.nodeData(stmt);
            if (isDiscardIdent(ast, infix.lhs.node) and ast.nodeTag(infix.rhs.node) == .identifier) {
                const name_bytes = ast.tokenSlice(ast.nodeMainToken(infix.rhs.node));
...
fn isEmptyBlock(ast: *const Ast, node: Ast.Node.Index) bool {
    switch (ast.nodeTag(node)) {
        .block_two => {
            return ast.nodeData(node).lhs.opt_node == .none and ast.nodeData(node).rhs.opt_node == .none;
        },
        else => return false,
    }
}
