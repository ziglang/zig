...
test "wrapping add" {
    ...
    const S = struct {
        fn doTheTest() !void {
            try testWrapAdd(i8, 3, 0, 3);
            try testWrapAdd(i8, -3, 0, -3);
            try testWrapAdd(i8, -3, 10, 7);
            try testWrapAdd(i8, -128, -128, 0);
            if (!spirv) try testWrapAdd(i2, 1, 1, -2);
            try testWrapAdd(i64, maxInt(i64), 1, minInt(i64));
            if (!spirv) try testWrapAdd(i128, maxInt(i128), -maxInt(i128), 0);
            if (!spirv) try testWrapAdd(i128, minInt(i128), maxInt(i128), -1);
            try testWrapAdd(i8, 127, 127, -2);
            try testWrapAdd(u8, 3, 10, 13);
            try testWrapAdd(u8, 255, 255, 254);
            if (!spirv) try testWrapAdd(u2, 3, 2, 1);
            if (!spirv) try testWrapAdd(u3, 7, 1, 0);
            if (!spirv) try testWrapAdd(u128, maxInt(u128), 1, minInt(u128));
        }
...
test "wrapping subtraction" {
    ...
    const S = struct {
        fn doTheTest() !void {
            ...
            if (!spirv) try testWrapSub(i128, maxInt(i128), -1, minInt(i128));
            if (!spirv) try testWrapSub(i128, minInt(i128), -maxInt(i128), -1);
            try testWrapSub(u8, 10, 3, 7);
            try testWrapSub(u8, 0, 255, 1);
            if (!spirv) try testWrapSub(u5, 0, 31, 1);
            if (!spirv) try testWrapSub(u128, 0, maxInt(u128), 1);
        }
...
test "wrapping multiplication" {
    ...
    const S = struct {
        fn doTheTest() !void {
            ...
            if (!spirv) try testWrapMul(i4, 2, 4, -8);
            ...
            if (!spirv) try testWrapMul(i128, maxInt(i128), -1, minInt(i128) + 1);
            if (!spirv) try testWrapMul(i128, minInt(i128), -1, minInt(i128));
            try testWrapMul(u8, 10, 3, 30);
            try testWrapMul(u8, 2, 255, 254);
            if (!spirv) try testWrapMul(u128, maxInt(u128), maxInt(u128), 1);
        }
