...
pub fn build(b: *std.Build) !void {
...

    {
        ...
        run.addFileArg(example_dir.path(b, "build.zig"));
        run.addCheck(.{ .expect_stderr_match = "no_unpack value may not contain '..' components" });
...

    {
        ...
        run.addFileArg(example_dir.path(b, "build.zig"));
        run.addCheck(.{ .expect_stderr_match = "no_unpack value may not contain backslashes" });
        test_step.dependOn(&run.step);
    }
    ...
    }
    {
        ...
        run.addFileArg(b.path("example/example_dep_file.txt"));
        run.addCheck(.{ .expect_stderr_match = "--no-unpack path may not contain '..' components" });
        test_step.dependOn(&run.step);
    }
    {
        const run = b.addSystemCommand(&.{
        ...
        run.addFileArg(b.path("example/example_dep_file.txt"));
        run.addCheck(.{ .expect_stderr_match = "--no-unpack path may not contain backslashes" });
        test_step.dependOn(&run.step);
    }
}
