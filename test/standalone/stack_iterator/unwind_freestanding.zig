...
noinline fn frame3(expected: *[4]usize, addr_buf: *[4]usize) std.builtin.StackTrace {
    ...
    return std.debug.captureCurrentStackTrace(.{
        ...
        // Since we're building for freestanding, we don't have DWARF unwind info or anything like
        // that. As such, we have to reassure the standard library that it *won't* crash if it
        // tries to use FP unwinding, since that typically isn't a guarantee!
